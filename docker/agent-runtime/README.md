...
- Non-root user `valadrien-os` (uid/gid 1000)

**ValadrienOs Binaries:**
- `/usr/local/bin/valadrien-os-agent-shim`: Go binary compiled from `tools/agent-shim/`. Reads `/run/valadrien-os/runtime-command.json` and `syscall.Exec`s the harness CLI.

**Defaults:**
- `USER`: 1000:1000 (valadrien-os, non-root)
- `WORKDIR`: `/workspace` (mount workspace volumes here)
- `ENTRYPOINT`: `/usr/bin/tini --` (PID-1 reaper, forwards signals)
- `CMD`: `/usr/local/bin/valadrien-os-agent-shim`
...
## Agent Container (valadrien-os-agent-shim)

The main agent process runs as the shim (PID 1 under tini). The shim:

1. Reads `/run/valadrien-os/runtime-command.json` (path overridable via `-spec`), a JSON file mounted by whatever schedules the run
