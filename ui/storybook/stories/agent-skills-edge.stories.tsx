...
import type { Agent, AgentSkillSnapshot, CompanySkillListItem } from "@valadrien-os/shared";
...
const library: CompanySkillListItem[] = [
  libSkill("valadrien-os", "ValadrienOs", "Coordination skill: heartbeats, checkout, comments, and routine API patterns."),
  libSkill("design-guide", "Design guide", "ValadrienOs UI design system reference: tokens, typography, status colors."),
];
...
function staleSnapshot(): AgentSkillSnapshot {
  return {
    ...
    supported: true,
    mode: "ephemeral",
    desiredSkills: ["valadrien-os", "legacy-retired-skill"],
    warnings: [],
    entries: [
      {
        key: "valadrien-os",
  ...
  return {
    ...
    entries: [
      ...
      },
      {
        ...
        sourcePath: null,
        targetPath: null,
        detail: "Detected in the adapter's skills directory; managed outside ValadrienOs.",
      },
...
export const StaleWarning: StoryObj = {
  ...
  render: () => (
    ...
    >
      <EdgeState agent={buildAgent("agent-stale", ["valadrien-os", "legacy-retired-skill"])} snapshot={staleSnapshot()} />
    </StoryFrame>
  ),
};
...
export const DetectedAdapter: StoryObj = {
  ...
  render: () => (
    <StoryFrame
      ...
      subtitle="A user-installed skill outside ValadrienOs's management shows in a collapsed, read-only section with a lock icon."
    >
      <EdgeState agent={buildAgent("agent-detected", ["valadrien-os"])} snapshot={detectedSnapshot()} />
    </StoryFrame>
  ),
};
