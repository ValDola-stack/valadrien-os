...
export function HermesGatewayConfigFields({
  ...
  const valadrienOsApiUrl = String(readValue("valadrienOsApiUrl", "") ?? "");
  ...
  return (
    <>
      <Field
        label="API base URL"
        hint="Hermes API server base URL that ValadrienOs can reach, such as http://127.0.0.1:8642 or a private HTTPS URL. Default dashboard root/chat URLs such as http://127.0.0.1:9119/chat are accepted and map to /api."
      >
        <DraftInput
          value={apiBaseUrl}
      ...
      <SecretField
        ...
        placeholder="Hermes API_SERVER_KEY, not VALADRIEN_OS_API_KEY"
        stored={!isCreate && hasStoredApiKey && !editApiKeyValue}
      />

      <Field
        label="ValadrienOs API URL"
        hint="Optional ValadrienOs API URL reachable by the Hermes host. This is not a credential."
      >
        <DraftInput
          value={valadrienOsApiUrl}
          onCommit={(v) => writeValue("valadrienOsApiUrl", v || undefined)}
          immediate
          className={inputClass}
