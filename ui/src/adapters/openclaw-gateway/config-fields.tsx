    ...
    <>
        ...
        <Field label="Session key">
          <DraftInput
            value={
              isCreate
                ? values!.sessionKey ?? ""
                : eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "valadrien-os"))
            }
            onCommit={(v) =>
              isCreate
                ? set!({ sessionKey: v })
      ...
      <Field label="ValadrienOs API URL override">
        <DraftInput
          value={
            isCreate
              ? values!.valadrienOsApiUrl ?? ""
              : eff("adapterConfig", "valadrienOsApiUrl", String(config.valadrienOsApiUrl ?? ""))
          }
          onCommit={(v) =>
            isCreate
              ? set!({ valadrienOsApiUrl: v })
              : mark("adapterConfig", "valadrienOsApiUrl", v || undefined)
          }
          immediate
          className={inputClass}
      ...
      <Field label="Device auth">
        <div className="text-xs text-muted-foreground leading-relaxed">
          When enabled, ValadrienOs persists a device key during onboarding so pairing approvals
