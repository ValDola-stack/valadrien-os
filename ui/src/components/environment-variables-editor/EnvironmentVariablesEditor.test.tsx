...
import type { CompanySecret, EnvBinding } from "@valadrien-os/shared";
...
describe("EnvironmentVariablesEditor", () => {
  ...
  it("keeps long secret names clear of the latest version control", () => {
    const longSecret = makeSecret("long", {
      name: "/valadrien-os-cloud/prod/provider/resend/api-key-with-a-very-long-name",
      latestVersion: 4,
    });
  ...
  it("renders name warnings as a row spanning the name and value columns", () => {
    render(
      <EnvironmentVariablesEditor
        value={{ VALADRIEN_OS_PAGE_BASE_URL: { type: "plain", value: "https://pages.valadrien-os.ing" } }}
        secrets={secrets}
        onChange={() => {}}
        onCreateSecret={async () => secrets[0]}
      />,
  ...
  it("lets slash-delimited secrets be browsed by folder and exposes full paths on hover", async () => {
    const cloudProviderKey = "/valadrien-os-cloud/prod/provider/aws-access-key-id";
    const cloudMigrationKey = "/valadrien-os-cloud/prod/migration/postgres-url";
    ...
    render(
      <SecretPicker
        ...
        secrets={[
          ...
          makeSecret("plain-key", { name: "valadrien-os-page-aws-access-key-id", key: "valadrien-os-page-aws-access-key-id" }),
        ]}
        onSelect={onSelect}
        onCreateNew={() => {}}
        disablePortal
    ...
    expect(rowTitle(itemByText("valadrien-os-cloud"))).toBe("/valadrien-os-cloud");
    expect(rowTitle(itemByText("valadrien-os-page-aws-access-key-id"))).toBe(
      "valadrien-os-page-aws-access-key-id",
    );

    itemByText("valadrien-os-cloud")!.click();
    await flush();
    expect(rowTitle(itemByText("prod"))).toBe("/valadrien-os-cloud/prod");

    itemByText("prod")!.click();
    await flush();
    expect(rowTitle(itemByText("provider"))).toBe("/valadrien-os-cloud/prod/provider");
    expect(rowTitle(itemByText("migration"))).toBe("/valadrien-os-cloud/prod/migration");
  ...
  it("keeps secret search global while browsing starts at slash folders", async () => {
      ...
      <SecretPicker
        ...
        secrets={[
          makeSecret("provider-key", {
            name: "/valadrien-os-cloud/prod/provider/aws-access-key-id",
            key: "/valadrien-os-cloud/prod/provider/aws-access-key-id",
          }),
          makeSecret("migration-key", { name: cloudMigrationKey, key: cloudMigrationKey }),
        ]}
        onSelect={() => {}}
    ...
    combobox.focus();
    await flush();
    expect(document.body.textContent).toContain("valadrien-os-cloud");
