...
import type { CompanySecret, EnvBinding, UserSecretDefinition } from "@valadrien-os/shared";
...
const DEFAULT_RESERVED_PREFIXES = ["VALADRIEN_OS_"];

const DEFAULT_HINT =
  "Set the KEY to the env var name the process expects, for example GH_TOKEN. Choose a secret to resolve a stored value at run start. VALADRIEN_OS_* variables are injected automatically.";
...
export interface EnvironmentVariablesEditorProps {
  ...
  /** Read-only rendering. */
  disabled?: boolean;
  /** Prefixes flagged as reserved/auto-provided. Default `["VALADRIEN_OS_"]`. */
