...
function buildWorkspaceValidationAction(
  ...
  const provenance = {
    ...
    plainLanguageReason:
      'The recorded branch "PAP-522-recorded" is not an ancestor of the checked-out branch "nleach/PAP-1405-live", so ValadrienOs cannot prove a forward-only reconciliation.',
...
describe("IssueRecoveryActionCard repair workspace (quarantine_restore)", () => {
  ...
  it("confirm popover restates the dirty count, live branch, rescue branch and recorded branch, then fires the handler", () => {
    ...
    ).toContain("valadrien-os/rescue/PAP-1405/");
