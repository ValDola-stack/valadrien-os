...
function applyObservabilityHeaders(headers: Headers) {
  if (headers.has("X-ValadrienOs-Tab-Visible")) return; // caller override wins
  const visibility = getPageVisibility();
  headers.set("X-ValadrienOs-Tab-Visible", getVisibilityHeaderValue(visibility));
  if (typeof window !== "undefined" && window.location) {
    headers.set("X-ValadrienOs-Route", window.location.pathname);
  }
}
