...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the ValadrienOs API key");
    ...
    expect(prompt).toContain("agentDefaultsPayload.valadrienOsApiUrl");
    ...
    expect(prompt).toContain("ValadrienOs maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the ValadrienOs host");
    expect(prompt).toContain("Hermes-originated ValadrienOs API calls");
    expect(prompt).toContain("Do not rotate or invent a ValadrienOs key manually");
