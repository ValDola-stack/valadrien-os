...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `ValadrienOs will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "ValadrienOs handles this" mechanics bullets, composed from the
