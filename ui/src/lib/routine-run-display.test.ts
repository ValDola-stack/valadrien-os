...
import type { RoutineVariable } from "@valadrien-os/shared";
...
describe("runRowSubtitle", () => {
    ...
    const subtitle = runRowSubtitle(
      {
        status: "succeeded",
        failureReason: null,
        triggerPayload: { customer: "Acme", retries: 3, VALADRIEN_OS_RUN_ID: "ignored" },
      },
      variables,
    );
  ...
  it("only includes declared routine variables, not builtin payload keys", () => {
    const subtitle = runRowSubtitle(
      { status: "succeeded", failureReason: null, triggerPayload: { VALADRIEN_OS_RUN_ID: "x" } },
      variables,
    );
