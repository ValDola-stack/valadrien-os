 ...
 *
 * Goal: when a user restores many ValadrienOs tabs at once, we do not want every
...
const TAB_ID_STORAGE_KEY = "valadrien-os:shared-poll:tab-id";
...
export function sharedPollingLeaseKey(companyId: string): string {
  return `valadrien-os:shared-poll:${sanitizeCompanyId(companyId)}:leader`;
}

export function sharedPollingChannelName(companyId: string): string {
  return `valadrien-os:shared-poll:${sanitizeCompanyId(companyId)}`;
}
...
function createBrowserLeaseStore(companyId: string): LeaseStore | null {
  ...
  try {
    const probeKey = `valadrien-os:shared-poll:${sanitizeCompanyId(companyId)}:probe`;
