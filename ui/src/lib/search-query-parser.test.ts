...
const context = {
  ...
  projects: [
    { id: "11111111-1111-4111-8111-111111111111", name: "ValadrienOs App", urlKey: "valadrien-os-app" },
  ],
  labels: [
...
describe("parseSearchQuery", () => {
  ...
  it("parses project names", () => {
    expect(parseSearchQuery("project:valadrien-os-app", context).filters).toEqual({
  ...
  it("parses mixed free text and multiple operators", () => {
    expect(parseSearchQuery("auth status:in_progress priority:critical project:valadrien-os-app", context)).toMatchObject({
      query: "auth",
      filters: {
        status: ["in_progress"],
        priority: ["critical"],
