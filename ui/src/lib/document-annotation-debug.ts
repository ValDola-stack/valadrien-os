export const SELECTION_DEBUG_STORAGE_KEY = "valadrienOsDebugSelection";
...
declare global {
  interface Window {
    __valadrienOsSelectionDebug?: SelectionDebugState;
  }
}
...
export function initializeSelectionDebug(): SelectionDebugState {
  if (!window.__valadrienOsSelectionDebug) {
    window.__valadrienOsSelectionDebug = createState();
    console.info(`[valadrien-os selection debug] enabled; set localStorage.${SELECTION_DEBUG_STORAGE_KEY} = "0" and reload to disable`);
  }
  return window.__valadrienOsSelectionDebug;
}
...
function record(type: string, details: SelectionDebugEvent["details"]): void {
  ...
  console.debug("[valadrien-os selection debug]", event);
}
