...
describe("inbox helpers", () => {
  ...
  it("normalizes invalid inbox filter storage back to safe defaults", () => {
    localStorage.setItem("valadrien-os:inbox:filters:company-1", JSON.stringify({
  ...
  it("maps legacy new-tab storage to mine", () => {
    localStorage.setItem("valadrien-os:inbox:last-tab", "new");
    expect(loadLastInboxTab()).toBe("mine");
  });
  ...
  it("groups project sections by latest issue activity while preserving non-issue sections", () => {
    const valadrienOsIssue = makeIssue("valadrien-os", true);
    valadrienOsIssue.projectId = "project-1";
    ...
    const items: InboxWorkItem[] = [
      { kind: "issue", timestamp: 9, issue: valadrienOsIssue },
    ...
    expect(groupInboxWorkItems(items, "project", {
      projectById: new Map([
        ["project-1", { name: "ValadrienOs App" }],
        ["project-2", { name: "Onboarding" }],
      ]),
    })).toEqual([
      { key: "project:project-1", label: "ValadrienOs App", items: [items[0]] },
  ...
  it("returns empty collapsed inbox groups for missing or invalid storage", () => {
    ...
    localStorage.setItem("valadrien-os:inbox:collapsed-groups:company-1", JSON.stringify({ nope: true }));
    expect(loadCollapsedInboxGroupKeys("company-1")).toEqual(new Set());
  });
