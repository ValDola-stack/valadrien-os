...
import type { CompanySkillListItem } from "@valadrien-os/shared";
...
export function buildAgentSkillSourceMeta(skill: SourceSkill): AgentSkillSourceMeta {
  ...
  if (skill.sourceBadge === "valadrien-os") {
    return { icon: Paperclip, label: skill.sourceLabel?.trim() || "ValadrienOs managed" };
  }
