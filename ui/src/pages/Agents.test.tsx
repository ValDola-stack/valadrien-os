...
import type { Agent, Environment, EnvironmentCapabilities } from "@valadrien-os/shared";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";
import { ToastProvider } from "../context/ToastContext";
import type { BuiltInAgentState } from "../api/builtInAgents";
import { Agents } from "./Agents";
import type { AgentOrgChainHealth } from "@valadrien-os/shared";
...
function makeBuiltInAgentState(overrides: Partial<BuiltInAgentState> = {}): BuiltInAgentState {
  return {
    definition: {
      ...
      defaultInstructions: "You are ValadrienOs's built-in Briefs agent.",
...
describe("Agents", () => {
  ...
  it("gives mobile agent names the full row width after the leading status indicator", async () => {
    ...
    mockAgentsApi.list.mockResolvedValue([
      makeAgent({
        id: "agent-mobile",
        name: "ValadrienOs Engineer With A Much Longer Display Name",
        title: "Software Engineer With A Much Longer Specialty Title",
        urlKey: "valadrien-os-engineer-long",
      }),
    ]);
    ...
    const row = findAgentRow(container, "ValadrienOs Engineer With A Much Longer Display Name");
    ...
    const name = row?.querySelector("span[title='ValadrienOs Engineer With A Much Longer Display Name']");
  ...
  it("uses configured names for local-driver environments", async () => {
    ...
    expect(container.textContent).toContain("ValadrienOs host");
  });
