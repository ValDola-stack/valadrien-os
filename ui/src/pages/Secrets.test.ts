...
import type { CompanySecretProviderConfig, SecretProviderDescriptor } from "@valadrien-os/shared";
import {
...
describe("Secrets page provider helpers", () => {
  ...
  it("uses provider health copy when an unconfigured provider reports missing bootstrap inputs", () => {
    ...
    expect(
      ...
      "Deployment default AWS Secrets Manager is not configured. Select a ready provider vault or configure the deployment default. AWS Secrets Manager provider is not ready: missing VALADRIEN_OS_SECRETS_AWS_DEPLOYMENT_ID.",
    );
  });
