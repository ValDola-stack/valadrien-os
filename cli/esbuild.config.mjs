/**
 * esbuild configuration for building the valadrien-os CLI for npm.
 *
 * Bundles all workspace packages (@valadrien-os/*) into a single file.
...
// to npm and resolved at runtime (e.g. @valadrien-os/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@valadrien-os/")) {
      externals.add(name);
    }
  }
