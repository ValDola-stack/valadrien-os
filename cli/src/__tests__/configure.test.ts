...
function writeBaseConfig(configPath: string) {
  const base: ValadrienOsConfig = {
    ...
    storage: {
      provider: "local_disk",
      localDisk: { baseDir: "/tmp/valadrien-os-storage" },
      s3: {
        bucket: "valadrien-os",
        region: "us-east-1",
        prefix: "",
    ...
    secrets: {
      ...
      strictMode: false,
      localEncrypted: { keyFilePath: "/tmp/valadrien-os-secrets/master.key" },
    },
  };
...
describe("configure command", () => {
  it("sets a failing exit code for unknown sections", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-configure-"));
  ...
  it("sets a failing exit code when no config exists", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-configure-missing-"));
