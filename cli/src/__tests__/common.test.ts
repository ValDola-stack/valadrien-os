...
function createTempPath(name: string): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-cli-common-"));
...
describe("resolveCommandContext", () => {
  beforeEach(() => {
    ...
    delete process.env.VALADRIEN_OS_AUTH_STORE;
    delete process.env.VALADRIEN_OS_SERVER_PORT;
  });
  ...
  it("resolves api base by explicit, env, profile, then config/default precedence", () => {
    ...
    process.env.VALADRIEN_OS_API_URL = "http://env:3/";
  ...
  it("prefers explicit and env tokens over profile env and stored board auth", () => {
    ...
    process.env.VALADRIEN_OS_AUTH_STORE = authStorePath;
    ...
    process.env.VALADRIEN_OS_API_KEY = "env-token";
