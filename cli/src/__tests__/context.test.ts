...
function createTempContextPath(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-cli-context-"));
  ...
  it("upserts profile values and switches current profile", () => {
    ...
    upsertProfile(
      "work",
      {
        ...
        agentId: "agent-123",
        agentName: "Agent One",
        apiKeyEnvVarName: "VALADRIEN_OS_AGENT_TOKEN",
      },
      contextPath,
    );
      ...
      JSON.stringify({
        ...
        profiles: {
          legacy: {
            ...
            persona: "board",
            apiKeyEnvVarName: "VALADRIEN_OS_BOARD_TOKEN",
          },
        },
      }),
    );
