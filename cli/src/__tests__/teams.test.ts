...
describe("teams CLI commands", () => {
  ...
  it("browses catalog teams with filters in table output", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://valadrien-os.test/api/teams/catalog?kind=bundled&category=software-development&q=engineering",
  ...
  it("searches catalog teams as JSON", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://valadrien-os.test/api/teams/catalog?kind=bundled&q=engineering",
      expect.objectContaining({ method: "GET" }),
    );
  ...
  it("lists catalog teams with installed status for a company", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "http://valadrien-os.test/api/teams/catalog?kind=bundled",
      expect.objectContaining({ method: "GET" }),
    );
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://valadrien-os.test/api/companies/company-1/teams/catalog/installed",
      expect.objectContaining({ method: "GET" }),
    );
    const rendered = logSpy.mock.calls.map((call: unknown[]) => String(call[0])).join("\n");
  ...
  it("inspects catalog team detail by query ref so keys with slashes work", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://valadrien-os.test/api/teams/catalog/ref?ref=paperclipai%2Fbundled%2Fsoftware-development%2Fproduct-engineering",
      expect.objectContaining({ method: "GET" }),
    );
  ...
  it("previews catalog team installs with trust policy flags", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://valadrien-os.test/api/companies/company-1/teams/catalog/ref/preview?ref=product-engineering",
  ...
  it("installs catalog teams and passes selection options", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://valadrien-os.test/api/companies/company%2F1/teams/catalog/ref/install?ref=product-engineering",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
          agents: ["cto"],
  ...
  it("requests board approval when agent-run install lacks create-agent permission", async () => {
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      1,
      "http://valadrien-os.test/api/companies/company-1/teams/catalog/ref/install?ref=product-engineering",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({
    ...
    expect(fetchMock).toHaveBeenNthCalledWith(
      2,
      "http://valadrien-os.test/api/companies/company-1/approvals",
      expect.objectContaining({
        method: "POST",
  ...
  it("auto-requests board approval for forbidden installs inside a ValadrienOs task run", async () => {
    process.env.VALADRIEN_OS_TASK_ID = "11111111-1111-4111-8111-111111111111";
    const approval = {
      id: "approval-2",
      companyId: "company-1",
      type: "request_board_approval",
  ...
  it("surfaces server blocks for unsafe local-path catalog team sources", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://valadrien-os.test/api/companies/company-1/teams/catalog/ref/install?ref=unsafe-local-team",
      expect.objectContaining({
        method: "POST",
        body: JSON.stringify({}),
      }),
