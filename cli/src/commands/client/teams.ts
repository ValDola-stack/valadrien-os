...
export function registerTeamCommands(program: Command): void {
  ...
  addCommonClientOptions(
    teams
      ...
      .option("--approval-issue-id <id>", "Issue ID to link to the fallback approval request; defaults to VALADRIEN_OS_TASK_ID when set")
...
function shouldRequestInstallApproval(error: unknown, opts: TeamInstallOptions): error is ApiRequestError {
  if (!(opts.requestApprovalOnForbidden || isValadrienOsTaskRun())) return false;
...
function isValadrienOsTaskRun(): boolean {
  return Boolean(process.env.VALADRIEN_OS_TASK_ID?.trim());
}
...
async function requestInstallApproval(
  ...
  const payload = {
    ...
    payload: {
      ...
      summary:
        `A ValadrienOs CLI agent-run attempted to install catalog team "${trimmedRef}" into company "${ctx.companyId}", ` +
...
function resolveApprovalIssueIds(opts: TeamInstallOptions): string[] | undefined {
  const issueId = opts.approvalIssueId?.trim() || process.env.VALADRIEN_OS_TASK_ID?.trim();
