...
/** Result of probing the ValadrienOs instance the CLI is about to talk to. */
 ...
 * developer can confirm *which* ValadrienOs they are about to install into. This
...
export function formatTargetDiagnostics(diag: TargetDiagnostics): string {
  const lines = [pc.dim(`Target ValadrienOs: ${pc.cyan(diag.apiBase)}`)];

  if (!diag.reachable) {
    lines.push(pc.yellow(`  health: unreachable${diag.error ? ` (${diag.error.split("\n")[0]})` : ""}`));
    lines.push(
      pc.dim(
        `  Verify the right instance is running, then pass ${pc.cyan("--api-base <url>")} or set ${pc.cyan("VALADRIEN_OS_API_URL")} if it lives elsewhere.`,
      ),
    );
    return lines.join("\n");
  }
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .option(
        "--no-verify-target",
        "Skip the pre-install probe that reports which ValadrienOs instance the plugin installs into",
      )
  ...
  addCommonClientOptions(
    plugin
      .command("target")
      .description(
        "Show which ValadrienOs instance plugin commands will talk to.\n" +
