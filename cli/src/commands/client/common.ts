...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    ...
    .option("--run-id <id>", "Heartbeat run id for agent-authenticated mutations (checkout/release/interactions/in-progress update); falls back to $VALADRIEN_OS_RUN_ID")
...
export function resolveCommandContext(
  ...
  // in-progress issue) require the X-ValadrienOs-Run-Id header (the server returns
  // "401 Agent run id required" without it). Source it from --run-id, else the
  // VALADRIEN_OS_RUN_ID env the adapter/embodiment context already exports.
  const runId = options.runId?.trim() || process.env.VALADRIEN_OS_RUN_ID?.trim() || undefined;
...
function resolveApiKey(
  ...
  const envValue = process.env.VALADRIEN_OS_API_KEY?.trim();
