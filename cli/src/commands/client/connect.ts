...
import type { Agent, Company } from "@valadrien-os/shared";
import { createAgentKeySchema, createBoardApiKeySchema } from "@valadrien-os/shared";
import { loginBoardCli } from "../../client/board-auth.js";
import { ValadrienOsApiClient } from "../../client/http.js";
...
export function registerConnectCommand(program: Command): void {
  addCommonClientOptions(
    program
      ...
      .option("--api-key-env-var-name <name>", "Env var name to store in the profile", "VALADRIEN_OS_API_KEY")
...
async function connectWizard(opts: ConnectOptions) {
  ...
  const boardApi = new ValadrienOsApiClient({ apiBase, apiKey: boardLogin.token });
  ...
  const apiKeyEnvVarName = opts.apiKeyEnvVarName?.trim() || "VALADRIEN_OS_API_KEY";
...
async function verifyHealth(apiBase: string): Promise<void> {
  const api = new ValadrienOsApiClient({ apiBase });
...
function buildExports(input: {
  ...
  return [
    `export VALADRIEN_OS_API_URL='${escaped(input.apiBase)}'`,
    input.companyId ? `export VALADRIEN_OS_COMPANY_ID='${escaped(input.companyId)}'` : null,
    input.agentId ? `export VALADRIEN_OS_AGENT_ID='${escaped(input.agentId)}'` : null,
