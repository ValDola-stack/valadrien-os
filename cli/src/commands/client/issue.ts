...
async function uploadAttachment(
  ...
  // This multipart upload uses a hand-rolled fetch rather than ValadrienOsApiClient,
  ...
  // (the client injects x-valadrien-os-run-id automatically for JSON requests).
  const headers: Record<string, string> = {};
  if (apiKey) headers.authorization = `Bearer ${apiKey}`;
  if (input.runId) headers["x-valadrien-os-run-id"] = input.runId;
