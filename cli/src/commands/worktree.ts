...
import { ensureAgentJwtSecret, loadValadrienOsEnvFile, mergeValadrienOsEnvEntries, readValadrienOsEnvEntries, resolveValadrienOsEnvFile } from "../config/env.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.VALADRIEN_OS_CONFIG;
...
const WORKTREE_NAME_PREFIX = "valadrien-os-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.VALADRIEN_OS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.VALADRIEN_OS_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromValadrienOsConfig(config: ValadrienOsConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromValadrienOsConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.valadrien-os${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".valadrien-os", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".valadrien-os", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.valadrien-os"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readValadrienOsEnvEntries(resolveValadrienOsEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.VALADRIEN_OS_HOME);
  const instanceId = nonEmpty(envEntries.VALADRIEN_OS_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local ValadrienOs instance. Expected VALADRIEN_OS_HOME and VALADRIEN_OS_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasValadrienOsConfig: existsSync(path.resolve(directPath, ".valadrien-os", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".valadrien-os", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".valadrien-os", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".valadrien-os", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: ValadrienOsConfig, envEntries: Record<string, string>, portOverride?: number): string {
...
export function copySeededSecretsKey(input: {
  ...
  sourceConfig: ValadrienOsConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.VALADRIEN_OS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.VALADRIEN_OS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.VALADRIEN_OS_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: ValadrienOsConfig;
  targetConfig: ValadrienOsConfig;
  ...
  const sourceEnvFile = resolveValadrienOsEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readValadrienOsEnvEntries(sourceEnvFile);
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  if (opts.force) {
    ...
    // <repo>/.valadrien-os/worktrees/ holding every repo-managed worktree
  ...
  const sourceEnvEntries = readValadrienOsEnvEntries(resolveValadrienOsEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.VALADRIEN_OS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.VALADRIEN_OS_AGENT_JWT_SECRET);
  mergeValadrienOsEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { VALADRIEN_OS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadValadrienOsEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run ValadrienOs inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}
...
export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" valadrien-os worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printValadrienOsCliBanner();
  p.intro(pc.bgCyan(pc.black(" valadrien-os worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasValadrienOsConfig: existsSync(path.resolve(worktreePath, ".valadrien-os", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printValadrienOsCliBanner();
  p.intro(pc.bgCyan(pc.black(" valadrien-os worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveValadrienOsEnvFile(configPath);
  const envEntries = readValadrienOsEnvEntries(envPath);
  const out = {
    VALADRIEN_OS_CONFIG: configPath,
    ...(envEntries.VALADRIEN_OS_HOME ? { VALADRIEN_OS_HOME: envEntries.VALADRIEN_OS_HOME } : {}),
    ...(envEntries.VALADRIEN_OS_INSTANCE_ID ? { VALADRIEN_OS_INSTANCE_ID: envEntries.VALADRIEN_OS_INSTANCE_ID } : {}),
    ...(envEntries.VALADRIEN_OS_CONTEXT ? { VALADRIEN_OS_CONTEXT: envEntries.VALADRIEN_OS_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasValadrienOsConfig)
      .map((choice) => path.resolve(choice.worktree, ".valadrien-os", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readValadrienOsEnvEntries(resolveValadrienOsEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start ValadrienOs once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: ValadrienOsConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasValadrienOsConfig ? "valadrien-os" : "no-valadrien-os-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".valadrien-os", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".valadrien-os", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .valadrien-os/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasValadrienOsConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a ValadrienOs worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasValadrienOsConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No ValadrienOs worktrees were found. Run `valadrien-os worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target ValadrienOs configs are the same. Choose different --from/--to worktrees.");
  }
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target ValadrienOs configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop ValadrienOs in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated ValadrienOs DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target ValadrienOs configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readValadrienOsEnvEntries(resolveValadrienOsEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.VALADRIEN_OS_HOME) && nonEmpty(targetEnvEntries.VALADRIEN_OS_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop ValadrienOs in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local ValadrienOs instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated ValadrienOs instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with valadrien-os- if needed (created at ~/valadrien-os-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: VALADRIEN_OS_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: VALADRIEN_OS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source VALADRIEN_OS_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local ValadrienOs instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like ValadrienOs worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another ValadrienOs instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source VALADRIEN_OS_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local ValadrienOs instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .valadrien-os/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: VALADRIEN_OS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source VALADRIEN_OS_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with valadrien-os- if needed")
