...
export async function loginBoardCli(params: {
  ...
  const publicBase = params.publicBaseUrl?.trim() || process.env.VALADRIEN_OS_PUBLIC_URL?.trim();
  ...
  const wantBrowser = params.openBrowser !== false && !isTruthyEnv(process.env.VALADRIEN_OS_NO_BROWSER);
