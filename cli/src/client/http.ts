...
export class ValadrienOsApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-valadrien-os-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the ValadrienOs API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the ValadrienOs server is not running, the configured URL is wrong, or the request is being blocked before it reaches ValadrienOs.",
    "",
    "Try:",
    "- Start ValadrienOs with `pnpm dev` or `pnpm valadrien-os run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If ValadrienOs is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`VALADRIEN_OS_API_URL\`.`,
  );
  return lines.join("\n");
}
