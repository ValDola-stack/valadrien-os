...
Full ValadrienOs CLI/API parity smoke pass against a disposable local source-tree instance.

## Isolation Contract

- Repo: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os`
- Scratch root: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity`
- `VALADRIEN_OS_HOME`: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home`
- `VALADRIEN_OS_INSTANCE_ID`: `cli-api-parity`
- `VALADRIEN_OS_CONFIG`: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
- `VALADRIEN_OS_CONTEXT`: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/context.json`
- `VALADRIEN_OS_AUTH_STORE`: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/auth.json`
- `VALADRIEN_OS_API_URL`: `http://127.0.0.1:3197`
- `VALADRIEN_OS_SERVER_PORT`: `3197`
- `PORT`: `3197`
- `CODEX_HOME`: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/codex-home`
- `CLAUDE_HOME`: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/claude-home`
- `DATABASE_URL`: unset
...
- Command: `sed -n ... valadrien-os-localdev-runbook.md`, `doc/DEVELOPING.md`, `doc/CLI.md`, `doc/DATABASE.md`, `doc/GOAL.md`, `doc/PRODUCT.md`, `doc/SPEC-implementation.md`, `doc/DEPLOYMENT-MODES.md`, `doc/plans/2026-05-23-cli-api-parity-openapi-reference.ts`
...
- Prerequisites/IDs used: none.
- Expected result: All ValadrienOs/Codex/Claude paths point under `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` are unset; port `3197` has no listener.
...
- Status: PASS.
- Output summary: No references to `~/.valadrien-os`, `~/.codex`, `~/.claude`, or `localhost:3100`.
- Follow-up: Start ValadrienOs with the runbook command.
...
- Purpose: Create and start the disposable source-tree ValadrienOs instance.
...
- Actual result: Server started in `local_trusted/private` mode on `127.0.0.1:3197`, with embedded PostgreSQL data at `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/db`.
- Status: PASS.
...
- Actual result: Config path, context path, storage path, secrets key path, and DB directory all resolve under `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity`; health returned `status: ok`, version `0.3.1`, `deploymentMode: local_trusted`, `companyDeletionEnabled: true`.
- Status: PASS.
- Output summary: `paperclipai env` redacted here because it prints the generated `VALADRIEN_OS_AGENT_JWT_SECRET`. Context existed at the scratch path with an empty `default` profile.
...
- Output summary: The assigned `process` adapter agent ran automatically and generated heartbeat runs. The issue later moved to `blocked` via recovery handling because the smoke process printed output without a concrete ValadrienOs disposition.
...
- Command: `token board create --company-id ... --name cli-parity-board --never-expires --json`; `token board list --json`; `whoami --api-key <board-token> --json`; `token agent create --company-id ... --agent ... --name cli-parity-agent --json`; `token agent list --company-id ... --agent ... --json`; `context set --profile cli-agent --persona agent ... --api-key-env-var-name VALADRIEN_OS_API_KEY --json`; `agent me --profile cli-agent --json`; `agent inbox --profile cli-agent --json`; `issue list --profile cli-agent --company-id ... --json`; `company list --profile cli-agent --json`; `token agent revoke ...`; `token board revoke ...`
...
- Command: `kill <valadrien-os pid on 3197>`; `pnpm paperclipai onboard --yes --run --bind loopback`; `curl http://127.0.0.1:3197/api/health`; `issue tree-hold:get <issue-id> null --json`
...
- Status: PASS.
- Output summary: No real `~/.valadrien-os`, `~/.codex`, or `~/.claude` paths were used. The server session is currently running under the isolated environment.
...
- Command: `env -u DATABASE_URL -u DATABASE_MIGRATION_URL ... pnpm paperclipai environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/ValadrienOsAI/valadrien-os"}}' --json`
...
- Prerequisites/IDs used: Isolated `CLAUDE_HOME=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/claude-home`.
...
- Status: PASS.
- Output summary: Mounted `llmRoutes` under `/api` in addition to the existing root mount. Updated available-skill discovery to read `CLAUDE_HOME/skills` when configured, include built-in ValadrienOs repo skills in `available-skill list`, and allow `available-skill get` for safe listed/built-in skill names.
...
- Expected result: LLM docs commands pass; built-in ValadrienOs skills are listed and fetchable; real-user `~/.claude` skills are not listed; `openapi` still documents the unresolved gap if no route exists.
- Actual result: LLM docs commands passed. `available-skill list` returned ValadrienOs repo skills such as `diagnose-why-work-stopped`; `available-skill get valadrien-os` returned markdown. `available-skill get cmux` now returns 404 because `cmux` is no longer listed from the real Claude home. `openapi` still returns `404: API route not found`.
- Status: MIXED.
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Prerequisites/IDs used: Scratch config `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`; scratch worktree root `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/worktrees-home`; company `12e9db4b-f66c-459b-959e-d645002240fb`.
...
- Actual result: `worktree:list` passed and showed the current repo branch `improvement/cli-api-parity` with no ValadrienOs worktree config. `worktree env --config <scratch-config> --json` passed and printed the scratch `VALADRIEN_OS_CONFIG` plus generated env values; the generated JWT secret is intentionally not copied into this log. `worktree:merge-history --from current --to current --company CLI --dry` failed as expected with `Source and target ValadrienOs configs are the same. Choose different --from/--to worktrees.` `cloud push --dry-run` failed as expected with `Cloud sync is disabled. Enable the cloud sync experimental setting before running paperclipai cloud push.`
...
- Purpose: Exercise cloud connect/push without a real external ValadrienOs Cloud stack, finish run subcommands that were not covered by earlier run list/get/events/log checks, and verify `connect` behavior in the non-interactive test runner.
...
- Expected result: Cloud sync can be enabled only in the scratch instance; `cloud connect` stores a fake upstream connection under scratch `VALADRIEN_OS_HOME`; `cloud push --dry-run` exports local company data and posts a preview bundle to the fake upstream; run read/control commands return structured results or controlled 404 for a nonexistent workspace operation; `connect` refuses non-interactive execution with guidance.
...
- Expected result: ValadrienOs remains running on `127.0.0.1:3197`; fake cloud port `3199` is stopped; no active board or agent tokens from the test remain; plugin list is empty; disposable routine is archived; `openapi` remains the one documented unresolved API route gap.
...
- Actual result: Install/list/inspect/health/logs/config/jobs/job:runs/job:trigger/webhook/dashboard/bridge:data/data/action/upgrade/disable/enable/uninstall passed. Config commands initially failed until payloads were corrected to `{"configJson":{...}}`; bridge action initially failed until payload used `key` instead of `action`. `bridge:stream` returned expected `Plugin stream bridge is not enabled`. Local-folder calls returned expected validation because the kitchen-sink manifest declares no local folders. `plugin tools` listed `valadrien-os-kitchen-sink-example:echo`, but `plugin tool:execute` returned `502: worker for plugin "valadrien-os-kitchen-sink-example" is not running` even though bridge calls to the same plugin worker succeeded.
...
- Command: Edited `server/src/services/plugin-tool-dispatcher.ts`, `server/src/services/plugin-loader.ts`, and `server/src/__tests__/plugin-database.test.ts`; ran `pnpm exec vitest run server/src/__tests__/plugin-database.test.ts`; `pnpm --dir server typecheck`; restarted the isolated server; installed kitchen-sink plugin; `plugin tools`; `plugin tool:execute --payload-json '{"tool":"valadrien-os-kitchen-sink-example:echo",...}'`; cleanup `plugin uninstall --force`.
...
- Prerequisites/IDs used: Same scratch server and kitchen-sink plugin; tool `valadrien-os-kitchen-sink-example:echo`.
- Expected result: The listed echo tool dispatches to the running kitchen-sink worker and returns the echo result.
- Actual result: Focused test and server typecheck passed. After restart, `plugin tools` listed `valadrien-os-kitchen-sink-example:echo`; `plugin tool:execute` returned `content: "CLI parity tool after fix"` and the expected run context. The plugin was uninstalled and `plugin list` returned `[]`.
...
- Expected result: Deleting a webhook routine trigger removes the generated valadrien-os-managed secret and binding. No active secrets remain from parity cleanup.
...
- Actual result: Final handoff stated that the full isolated CLI/API parity pass completed and was committed, the scratch ValadrienOs server was still running on `http://127.0.0.1:3197`, and goal usage was 2,625,855 tokens over about 2h 23m.
- Status: PASS.
- Output summary:
...
- Final paths:
  - Scratch home: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home`
  - Config: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/config.json`
  - Context: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/context.json`
  - Auth store: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/auth.json`
  - Embedded DB: `/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/db`, active on pg port `54330`
...
```sh
export VALADRIEN_OS_HOME=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home
export VALADRIEN_OS_INSTANCE_ID=cli-api-parity
export VALADRIEN_OS_CONFIG=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/config.json
export VALADRIEN_OS_CONTEXT=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/context.json
export VALADRIEN_OS_AUTH_STORE=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/auth.json
export VALADRIEN_OS_API_URL=http://127.0.0.1:3197
export VALADRIEN_OS_SERVER_PORT=3197
export PORT=3197
export CODEX_HOME=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/codex-home
export CLAUDE_HOME=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/claude-home
...
- Prerequisites/IDs used: Isolated scratch server restarted on `127.0.0.1:3197`; `DATABASE_URL` and `DATABASE_MIGRATION_URL` unset; `VALADRIEN_OS_HOME`, `VALADRIEN_OS_CONFIG`, `VALADRIEN_OS_CONTEXT`, `VALADRIEN_OS_AUTH_STORE`, `CODEX_HOME`, and `CLAUDE_HOME` all under `tmp/cli-api-parity`.
...
- Command: `pnpm paperclipai health --json`; `pnpm paperclipai openapi --json`; `lsof -nP -iTCP:3197 -sTCP:LISTEN`; `git status --short --branch`, all with the scratch `VALADRIEN_OS_*`, `CODEX_HOME`, and `CLAUDE_HOME` environment and with `DATABASE_URL`/`DATABASE_MIGRATION_URL` unset.
...
- Command: Stopped the foreground scratch server process; started the same runbook command in detached screen session `valadrien-os-cli-parity`; verified `pnpm paperclipai health --json`; checked `lsof -nP -iTCP:3197 -sTCP:LISTEN`; checked `screen -ls`.
- Purpose: Leave the disposable instance running without tying it to the active tool session.
- Prerequisites/IDs used: Same scratch env and unset database variables; detached screen session `91568.valadrien-os-cli-parity`.
- Expected result: Server continues running on non-default port `3197` with the same isolated home/config/context/auth paths.
- Actual result: Health returned `status:"ok"` with deployment mode `local_trusted`; `node` PID `91583` is listening on `127.0.0.1:3197`; `screen -ls` shows detached session `91568.valadrien-os-cli-parity`.
- Status: PASS.
- Output summary: Detached server log is `tmp/cli-api-parity/artifacts/caveat-followup/server-screen.log`.
- Follow-up: Manual continuation can use the same env block and `screen -r valadrien-os-cli-parity` to inspect the server session.
...
- Expected result: The webhook trigger's generated valadrien-os-managed secret and binding are removed when the trigger is deleted.
...
- Reproduction command: Install `packages/plugins/examples/plugin-kitchen-sink-example`, then run `pnpm paperclipai plugin tool:execute --payload-json '{"tool":"valadrien-os-kitchen-sink-example:echo","parameters":{"message":"CLI parity tool"},"runContext":{"companyId":"<company-id>","projectId":"<project-id>","agentId":"<agent-id>","runId":"<run-id>"}}' --json`.
- Expected result: The listed tool dispatches to the running kitchen-sink worker and returns a `ToolResult`.
- Actual result: `plugin tools` listed `valadrien-os-kitchen-sink-example:echo`, and bridge data/action calls to the same plugin worker succeeded, but `tool:execute` returned `502: Cannot execute tool ... worker for plugin "valadrien-os-kitchen-sink-example" is not running`.
...
- Suspected cause: The CLI did not reuse the pnpm executable that launched the current ValadrienOs command, so dependency installation was subject to PATH/shim behavior under an overridden `HOME`.
...
- Verification command: `pnpm exec vitest run cli/src/__tests__/worktree.test.ts`; `pnpm --dir cli typecheck`; live isolated `worktree:cleanup --force` for the partial worktree; live isolated `worktree:make ... --seed-mode minimal`; `worktree:list --json`; `worktree env --config <scratch-worktree-config> --json`; `worktree:merge-history --from valadrien-os-cli-parity-wt --to current --company CLI --dry`.
- Remaining risk: Low. If ValadrienOs is launched outside pnpm, dependency installation still falls back to PATH lookup as before.

### BUG-008 - `configure --section <invalid>` printed an error but exited 0

- Status: Fixed and live-verified.
- Severity: Low command UX/scripting bug.
- Reproduction command: `pnpm paperclipai configure --config /Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/home/instances/cli-api-parity/config.json --section invalid-section`.
...
- Status: Fixed.
- Severity: High for isolated CLI testing; a non-default `apiBase` can be silently removed and later commands may fall back to `http://localhost:3100` if `VALADRIEN_OS_API_URL` is absent.
...
- Fix summary: Mounted LLM docs routes under `/api`; made available-skill discovery honor `CLAUDE_HOME`, include built-in ValadrienOs repo skills, and fetch safe skill markdown consistently; added `/api/openapi.json`, then upgraded it from the initial path inventory to the schema-backed `OpenAPIRegistry`/`OpenApiGeneratorV3` implementation from the parity reference.
...
- Reproduction command: `CLAUDE_HOME=/Users/aronprins/Documents/ValadrienOsAI/valadrien-os/tmp/cli-api-parity/claude-home pnpm paperclipai available-skill list --json`.
- Expected result: Skill discovery uses the isolated Claude home or built-in repo skills only.
- Actual result: Before the fix, the list included `cmux` from the real user Claude skills home, and `available-skill get cmux` failed because only a hardcoded ValadrienOs subset was fetchable.
- Suspected cause: Server code read `HOME/.claude/skills` directly and did not add built-in ValadrienOs skills unless they were present in Claude's skills directory.
- Files changed: `server/src/routes/access.ts`, `doc/bugs/2026-05-24-cli-api-parity-e2e-log.md`.
- Fix summary: Use `CLAUDE_HOME/skills` when `CLAUDE_HOME` is set, include built-in ValadrienOs skills in catalog output, and resolve safe skill markdown from both Claude and ValadrienOs skills directories.
- Verification command: live `available-skill list`, `available-skill get valadrien-os`, and `available-skill get cmux` after restarting the isolated server.
...
- Reproduction command: `pnpm paperclipai environment create --company-id 12e9db4b-f66c-459b-959e-d645002240fb --payload-json '{"name":"CLI parity local env","description":"Disposable CLI parity environment","driver":"local","config":{"cwd":"/Users/aronprins/Documents/ValadrienOsAI/valadrien-os"}}' --json`.
