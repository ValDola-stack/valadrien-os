...
Date: 2026-05-26
Source plan: approved ValadrienOs skills CLI and catalog plan
...
- `paperclipai skills` manages ValadrienOs company skills. It does not manage
...
- Root `skills/` remains reserved for ValadrienOs runtime and operational skills.
...
- Agent desired skills continue to live in
  `adapterConfig.valadrienOsSkillSync.desiredSkills` for the first release. Do not
...
- Catalog skill: an app-shipped skill entry in `@valadrien-os/skills-catalog`.
...
- Company-scoped commands also accept `-C, --company-id <id>` and otherwise use
  `VALADRIEN_OS_COMPANY_ID` or the active context profile.
...
| `skills agent sync <agent-ref> --skill <skill-ref>...` | Replaces the agent's non-required desired skill set with the supplied refs and triggers adapter sync. Required ValadrienOs skills remain enforced by the server. | `AgentSkillSnapshot` |
...
Catalog commands are for the app-shipped ValadrienOs catalog only. External GitHub,
...
Package name: `@valadrien-os/skills-catalog`.
...
- `Company ID is required. Pass --company-id, set VALADRIEN_OS_COMPANY_ID, or set a context profile.`
