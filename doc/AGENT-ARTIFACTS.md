...
must be attached to the ValadrienOs issue before the agent chooses a final
...
```sh
skills/valadrien-os/scripts/valadrien-os-upload-artifact.sh path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The helper uses the authenticated ValadrienOs API from the current heartbeat
environment:
...
`artifact` and `provider` to `valadrien-os`, with metadata canonicalized from the
...
Workspace file links resolve only inside registered ValadrienOs workspaces. The
...
when it carries both that `projectId` and `workspaceId`. ValadrienOs does not
...
2. Upload it with `skills/valadrien-os/scripts/valadrien-os-upload-artifact.sh`.
...
```sh
skills/valadrien-os/scripts/valadrien-os-upload-artifact.sh render.bin \
  --title "Demo video render" \
...
```sh
curl -sS -X POST \
  "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/issues/$VALADRIEN_OS_TASK_ID/attachments" \
...
```sh
curl -sS -X POST \
  "$VALADRIEN_OS_API_URL/api/issues/$VALADRIEN_OS_TASK_ID/work-products" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
  -H "X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID" \
...
Use `type: "artifact"`, `provider: "valadrien-os"`, and metadata containing the
