...
```bash
cd ~/Projects/DEV/valadrien-os
git fetch origin && git worktree add ../valadrien-os-run3 -b design/component-convergence origin/master
cd ../valadrien-os-run3 && pnpm install
```
...
```bash
cd ~/Projects/DEV/valadrien-os-design-simplify
...
```
Converge ValadrienOs's duplicated hand-rolled UI onto the shared
...
4. pnpm check:token-gates 3/3 CLEAN; pnpm typecheck green;
   pnpm --filter @valadrien-os/ui build exit 0.
