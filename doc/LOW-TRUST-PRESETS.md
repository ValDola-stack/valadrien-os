...
ValadrienOs ships core trust preset names so containment decisions are enforced in
...
scope for risky access, ValadrienOs fails closed.
...
agent can read or mutate through the ValadrienOs API and prevents raw untrusted
...
Managed `low_trust_review` runs fail closed unless ValadrienOs can enforce the
runtime boundary:
...
local review, but ValadrienOs-managed low-trust execution requires a sandboxed
