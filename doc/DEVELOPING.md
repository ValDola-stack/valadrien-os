...
or inject external font CSS to make ValadrienOs text render correctly.

Font assets live in Vite's public directory so `pnpm --filter @valadrien-os/ui build`
...
Use the helper bundled with the ValadrienOs skill from the repo root:

```sh
skills/valadrien-os/scripts/valadrien-os-upload-artifact.sh dist/demo.mp4 \
...
```sh
skills/valadrien-os/scripts/valadrien-os-upload-artifact.sh out/walkthrough.webm \
...
For `codex_local`, ValadrienOs assigns new and updated agents an isolated Codex home under the instance root and blocks shared host/company Codex homes:

- `~/.valadrien-os/instances/default/companies/<company-id>/agents/<agent-id>/codex-home`

ValadrienOs also persists an empty `OPENAI_API_KEY` override for those agents so a host-level `OPENAI_API_KEY` cannot leak into Codex runs through process inheritance. If an operator explicitly configures `adapterConfig.env.CODEX_HOME`, it must not point at the shared company `codex-home`, `$CODEX_HOME`, or `~/.codex`.
...
The ValadrienOs app ships a curated catalog of company skills out of the box. The
...
paths at request time. Root `skills/` remains reserved for ValadrienOs runtime
skills and is not part of the catalog.

Validate the catalog without writing the manifest:

```sh
pnpm --filter @valadrien-os/skills-catalog validate
```
...
```sh
pnpm --filter @valadrien-os/skills-catalog build:manifest
```

The package's `build` script runs `build:manifest` and then `tsc`; tests live
under `pnpm --filter @valadrien-os/skills-catalog test`. Validation fails when:
...
CI runs `pnpm --filter @valadrien-os/skills-catalog validate` and the package's
...
```sh
pnpm --filter @valadrien-os/teams-catalog validate
```

Regenerate `generated/catalog.json` after editing catalog team files:

```sh
pnpm --filter @valadrien-os/teams-catalog build:manifest
```
...
- `VALADRIEN_OS_DB_BACKUP_MAX_AGE_HOURS=<hours>` controls the `/api/health`
  stale-backup warning threshold
- `VALADRIEN_OS_DB_BACKUP_ALERT_FILE=/path/to/failure-marker` lets external cron
  wrappers surface the last failed backup in `/api/health`

Without `VALADRIEN_OS_DB_BACKUP_ALERT_FILE`, health checks look for
