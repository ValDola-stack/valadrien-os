...
`pnpm test:hermes-gateway-smoke` does not start Docker, Hermes, or ValadrienOs. It
...
- Board/operator auth is required through `VALADRIEN_OS_AUTH_HEADER`,
  `VALADRIEN_OS_COOKIE`, or a board-capable `VALADRIEN_OS_API_KEY`.
- Diagnostic files are redacted before they are written, except the join output
  file intentionally contains the claimed ValadrienOs agent key and is written
  `chmod 600`.
...
- `VALADRIEN_OS_API_URL`: ValadrienOs URL used by the operator shell.
- `VALADRIEN_OS_API_URL_FOR_HERMES`: ValadrienOs URL used from inside the Hermes
  container or remote Hermes host.
- `HERMES_GATEWAY_API_BASE_URL`: Hermes gateway URL stored on the ValadrienOs
  adapter, reachable by the ValadrienOs server.
...
## Docker Desktop or Linux host ValadrienOs

Use this when ValadrienOs runs on the host at `127.0.0.1:3100` and Docker can
...
## Same Docker network as ValadrienOs

Use this when ValadrienOs is a container on a Docker network and the Hermes smoke
...
VALADRIEN_OS_API_URL=http://127.0.0.1:3100 \
VALADRIEN_OS_AUTH_HEADER='Bearer <board-token>' \
HERMES_CONTAINER_NAME=valadrien-os-hermes-gateway-smoke \
HERMES_SMOKE_NETWORK=valadrien_os_default \
HERMES_DOCKER_ADD_HOST=0 \
HERMES_GATEWAY_API_BASE_URL=http://valadrien-os-hermes-gateway-smoke:8642 \
HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642 \
VALADRIEN_OS_API_URL_FOR_HERMES=http://valadrien-os:3100 \
HERMES_GATEWAY_ALLOW_INSECURE_HTTP=1 \
pnpm smoke:hermes-gateway-e2e
```

Change `valadrien_os_default` and `valadrien-os` to your Compose network and service
name. The unsafe HTTP flag is required because ValadrienOs stores a non-loopback
`http://` gateway URL for private Docker DNS.

## LAN or private-network ValadrienOs

Use this when ValadrienOs is exposed on a private IP or tailnet address and the
Hermes container can reach that address.

```sh
VALADRIEN_OS_API_URL=http://192.168.1.20:3100 \
VALADRIEN_OS_AUTH_HEADER='Bearer <board-token>' \
VALADRIEN_OS_API_URL_FOR_HERMES=http://192.168.1.20:3100 \
...
Use this when ValadrienOs should talk to Hermes through a TLS hostname. The smoke
container still publishes a local port, and your reverse proxy forwards the TLS
hostname to that port.

```sh
VALADRIEN_OS_API_URL=https://valadrien-os.example.com \
VALADRIEN_OS_AUTH_HEADER='Bearer <board-token>' \
VALADRIEN_OS_API_URL_FOR_HERMES=https://valadrien-os.example.com \
...
`HERMES_GATEWAY_API_BASE_URL` to the URL ValadrienOs should store on the adapter.
