...
Use this guide when a Hermes runtime should join ValadrienOs as an external
...
ValadrienOs ships both Hermes adapters as built-ins:
...
`@valadrien-os/hermes-paperclip-adapter` package to override or shadow a built-in
adapter while developing the Hermes package. If the external override is paused
or removed, ValadrienOs restores the built-in `hermes_local` / `hermes_gateway`
adapter.
...
- Hermes gateway key: set `API_SERVER_KEY` before starting Hermes. ValadrienOs
  stores the same value as `agentDefaultsPayload.apiKey` so it can call Hermes.
- ValadrienOs agent key: created after the board approves the join request and
  claimed once by the Hermes agent. Hermes uses this key as
  `VALADRIEN_OS_API_KEY` when it calls ValadrienOs.

Do not reuse the Hermes gateway key as the ValadrienOs agent key. The Hermes
gateway key authenticates ValadrienOs-to-Hermes traffic; the claimed ValadrienOs key
authenticates Hermes-to-ValadrienOs traffic.
...
ValadrienOs can usually store `http://127.0.0.1:8642` as the gateway URL. For
Docker, LAN, tailnet, or reverse-proxy setups, use a URL reachable by the
ValadrienOs server process.
...
- `agentDefaultsPayload.apiBaseUrl` is the Hermes gateway URL that ValadrienOs
  calls.
- `agentDefaultsPayload.valadrienOsApiUrl` is the ValadrienOs base URL that Hermes
  can call after approval and key claim.
- `VALADRIEN_OS_API_URL` / `VALADRIEN_OS_API_KEY` are injected runtime values for
  Hermes-originated ValadrienOs API calls after the agent is approved.

## Approve And Claim

After Hermes submits the join request:

1. In ValadrienOs, review the pending agent join request.
...
4. Store the claimed ValadrienOs key in Hermes runtime state or secrets. The claim
...
agent and wake it through the normal ValadrienOs heartbeat path.

## Local Fresh-State Smoke

For a fresh Docker-backed Hermes gateway and end-to-end ValadrienOs join/run
verification, use:
...
- creates and approves a ValadrienOs agent-only invite
- joins as `hermes_gateway`
- wakes the agent on a smoke issue
- removes ValadrienOs and Docker test state on success
...
- Adapter development override: Adapter manager can install
  `@valadrien-os/hermes-paperclip-adapter` as an external override, but normal
