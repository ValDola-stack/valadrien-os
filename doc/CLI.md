...
1. `--api-base <url>`
2. `VALADRIEN_OS_API_URL`
3. selected context profile `apiBase`
4. local ValadrienOs config server port
...
pnpm paperclipai context set --persona agent --agent-id <agent-id> --api-key-env-var-name VALADRIEN_OS_API_KEY
...
- With agent authentication, `company list` and `company current` are
  ...
  if that is forbidden, it uses `--company-id`, `VALADRIEN_OS_COMPANY_ID`, context,
...
Issue subresources are exposed as ValadrienOs API wrappers. Commands that map to broad server schemas accept JSON payloads and validate them with shared schemas before sending.
...
pnpm paperclipai agent skills:sync <agent-id> --desired-skills valadrien-os,github
...
Agent config, instructions, skills, project env, environment, secret, and workspace edits affect the next run. Active runs finish with the config they started with. When a saved session, reused workspace, or sandbox lease no longer matches the effective next-run config, ValadrienOs may start fresh execution and records non-sensitive freshness categories in run result JSON and workspace operation logs.

`agent local-cli` is the quickest way to run local Claude/Codex manually as a ValadrienOs agent:
...
`paperclipai run` without a subcommand still bootstraps and starts a local ValadrienOs instance. The subcommands below inspect and control API heartbeat runs.
...
Prompt handoff creates ValadrienOs work. It does not create a chat session.

```sh
pnpm paperclipai agent-prompt <agent-name-or-id> <agent-api-key> "Prompt here"
pnpm paperclipai agent prompt --agent <agent-name-or-id> --api-key-env VALADRIEN_OS_API_KEY "Prompt here"
...
Required ValadrienOs runtime skills (heartbeat, etc.) remain server-enforced and
...
The ValadrienOs app ships a curated catalog under `@valadrien-os/skills-catalog`.
...
`skills agent clear` sends an empty desired list. Required ValadrienOs skills are
...
`@valadrien-os/teams-catalog`. Browse, search, inspect, and file reads do not
...
- Under agent authentication, use `paperclipai company list --json`,
  `paperclipai company current --json`, or `VALADRIEN_OS_COMPANY_ID` to select the
...
- `--request-approval-on-forbidden` turns a 403 install denial into a linked
  ...
  `--approval-issue-id <id>` to attach it to a specific issue. During ValadrienOs
  task runs with `VALADRIEN_OS_TASK_ID` set, this fallback is automatic so
...
VALADRIEN_OS_CHALLENGE_SECRET=<challenge-secret> pnpm paperclipai auth challenge get <challenge-id> --token-env VALADRIEN_OS_CHALLENGE_SECRET
VALADRIEN_OS_CHALLENGE_SECRET=<challenge-secret> pnpm paperclipai auth challenge approve <challenge-id> --token-env VALADRIEN_OS_CHALLENGE_SECRET
VALADRIEN_OS_CHALLENGE_SECRET=<challenge-secret> pnpm paperclipai auth challenge cancel <challenge-id> --token-env VALADRIEN_OS_CHALLENGE_SECRET
