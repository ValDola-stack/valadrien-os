...
- Attachment-backed artifact work products use `type: "artifact"`, `provider: "valadrien-os"`, and metadata with `attachmentId`, `contentType`, `byteSize`, `contentPath`, `openPath`, `downloadPath`, and optional `originalFilename`.
...
- heartbeat finalization evaluates liveness from persisted ValadrienOs state; an issue cannot remain healthy `in_progress` solely because the exiting heartbeat started a local/background watcher
...
- when ValadrienOs cannot safely infer the next action, it surfaces the problem through visible blocked/recovery work instead of silently completing or reassigning work
