...
All persisted under your bind mount (`./data/docker-valadrien-os` in the example above).
...
- host port: `3100`
- persistent data dir: `./data/docker-valadrien-os`
...
If you change host port or use a non-local domain, set `VALADRIEN_OS_PUBLIC_URL` to the external URL you will use in browser/auth flows.
...
services:
  valadrien-os:
    environment:
      ...
      VALADRIEN_OS_PUBLIC_URL: https://desk.koker.net
```

`VALADRIEN_OS_PUBLIC_URL` is used as the primary source for:
...
ValadrienOs URL, sign in or create an account, then choose `Claim this instance`
...
Granular overrides remain available if needed (`VALADRIEN_OS_AUTH_PUBLIC_BASE_URL`, `BETTER_AUTH_URL`, `BETTER_AUTH_TRUSTED_ORIGINS`, `VALADRIEN_OS_ALLOWED_HOSTNAMES`).

Set `VALADRIEN_OS_ALLOWED_HOSTNAMES` explicitly only when you need additional hostnames beyond the public URL host (for example Tailscale/LAN aliases or multiple private hostnames).
...
4. Create the data directory and start:
   ...
   ```sh
   mkdir -p ~/.local/share/valadrien-os
   systemctl --user daemon-reload
   systemctl --user start valadrien-os-pod
   ```

### Quadlet management

```sh
journalctl --user -u valadrien-os -f        # App logs
journalctl --user -u valadrien-os-db -f     # DB logs
systemctl --user status valadrien-os-pod    # Pod status
systemctl --user restart valadrien-os-pod   # Restart all
systemctl --user stop valadrien-os-pod      # Stop all
```
...
- `npx valadrien-os onboard --yes` completes
...
```sh
HOST_PORT=3200 VALADRIEN OS_VERSION=latest ./scripts/docker-onboard-smoke.sh
VALADRIEN_OS_DEPLOYMENT_MODE=authenticated VALADRIEN_OS_DEPLOYMENT_EXPOSURE=private ./scripts/docker-onboard-smoke.sh
SMOKE_DETACH=true SMOKE_METADATA_FILE=/tmp/valadrien-os-smoke.env VALADRIEN OS_VERSION=latest ./scripts/docker-onboard-smoke.sh
```
