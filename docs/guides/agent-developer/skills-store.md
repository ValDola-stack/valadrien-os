...
The **Skills Store** is ValadrienOs's library of reusable skills. A skill is a markdown
...
|---|---|---|
| **The catalog** | A curated, read-only set of skills that ships with ValadrienOs | The `@valadrien-os/skills-catalog` package |
...
- **`bundled`** — first-party ValadrienOs skills (e.g. `issue-triage`, `task-planning`,
...
- **`category`** — grouping such as `software-development`, `quality`, `product`,
  `research`, `content`, `browser`, `valadrien-os-operations`, `docs`.
...
|---|---|---|
| `catalog` | ValadrienOs / catalog | Installed from the bundled catalog |
...
`ramp` skill is the model: ValadrienOs ships the stable governance wrapper, source
...
a vendored snapshot would go stale, and when ValadrienOs can keep the safety boundary in
the wrapper. For financial, legal, or account-control domains, the wrapper must require
ValadrienOs approvals before spend, incorporation, account authorization, card issuance,
...
Paste a source and ValadrienOs fetches and imports it. Accepted forms include:
...
The catalog is its own publishable package, `@valadrien-os/skills-catalog`:
