...
- `hermes_local`: starts your local `hermes` CLI through `@valadrien-os/hermes-paperclip-adapter`
- `hermes_gateway`: calls an already-running Hermes API server through `@valadrien-os/hermes-paperclip-adapter/gateway`
...
For local CLI adapters (`claude_local`, `codex_local`, `opencode_local`, `hermes_local`, `droid_local`), ValadrienOs assumes the CLI is already installed and authenticated on the host machine. For `hermes_gateway`, ValadrienOs assumes the Hermes API server is already running, reachable from the ValadrienOs server, and configured with an API key. The older `@valadrien-os/adapter-hermes-gateway` npm package is only a deprecated compatibility shim; the adapter type remains `hermes_gateway`.
