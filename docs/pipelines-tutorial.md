...
Run this against a dev ValadrienOs instance with a board token or an agent token that can manage pipelines, routines, and issues.

```sh
export VALADRIEN_OS_API_URL=http://localhost:3100
export VALADRIEN_OS_COMPANY_ID=<company-id>
export VALADRIEN_OS_API_KEY=<token>
...
curl -sS -X POST \
  ...
  "$VALADRIEN_OS_API_URL/api/cases/$TWEET_CASE/issue-links" >/dev/null
...
# Recovery:
paperclipai pipelines case get -C "$VALADRIEN_OS_COMPANY_ID" "$BLOG_CASE" --json
```
...
```sh
export BLOG_VERSION="$(paperclipai pipelines case get -C "$VALADRIEN_OS_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$HERO_CASE" --to published --expected-version 1 --reason "Hero image done."
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$CARD_CASE" --to dropped --expected-version 1 --reason "Social card not needed."
```

When both asset cases are terminal, move the blog case to `assembly`.

```sh
export BLOG_ASSETS_VERSION="$(paperclipai pipelines case get -C "$VALADRIEN_OS_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$ASSEMBLY_CASE" --to published --expected-version 1 --reason "Assembly complete."
```
...
```sh
export BLOG_REVIEW_VERSION="$(paperclipai pipelines case get -C "$VALADRIEN_OS_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
```sh
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 1 --reason "Draft ready for final review."
...
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 4 --reason "Revised draft ready."
paperclipai pipelines case review -C "$VALADRIEN_OS_COMPANY_ID" "$CHANGELOG_CASE" --approve --expected-version 5
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$CHANGELOG_CASE" --to published --expected-version 6 --reason "Published after request-changes loop."
```
...
```sh
paperclipai pipelines case transition -C "$VALADRIEN_OS_COMPANY_ID" "$TWEET_CASE" --to final_review --expected-version 1 --reason "Blog blocker is now done."
