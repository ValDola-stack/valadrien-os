...
ValadrienOs stores the feature as value-free metadata plus the user's own secret
value record:
...
identity, not from ValadrienOs secrets.
...
- Keep ValadrienOs-managed AWS values under the ValadrienOs managed namespace.
  External refs under that namespace are blocked by guardrails; use the
  ValadrienOs-managed flow when ValadrienOs should create and rotate the value.

### IAM Caveats

ValadrienOs enforces company scoping, responsible-user derivation, declaration
...
For AWS Secrets Manager, the ValadrienOs runtime role needs `GetSecretValue` and
any required KMS decrypt permission for every user-scoped value it may resolve.
If you link user-specific external refs outside the ValadrienOs managed prefix,
...
refs. ValadrienOs can prevent an agent from choosing another credential owner,
but a runtime role with broad external vault read permissions can still read
what IAM allows if another code path is introduced outside ValadrienOs.
...
- `aws_secrets_manager`: back up ValadrienOs's database for vault metadata
  ...
  same ValadrienOs company at the same AWS namespace and confirming the runtime
