...
- Either a host Codex login with `~/.codex/auth.json`, or a per-agent
  `OPENAI_API_KEY` configured in adapter env (ValadrienOs materializes this into
...
- **Genuine external overrides** (a `CODEX_HOME` outside the ValadrienOs-managed company tree) are treated as self-managed and are never seeded or overwritten.
...
3. **External `CODEX_HOME`:** if adapter env points `CODEX_HOME` outside the
   ValadrienOs-managed company tree, that home is self-managed. ValadrienOs does
   not seed or overwrite it, so its own `auth.json` wins.

For sandbox or SSH execution, ValadrienOs uploads the effective managed
...
`$HOME/.codex/auth.json`, and the ValadrienOs host is logged in with a ChatGPT
subscription. For a managed `codex_local` agent, ValadrienOs symlinks the host
