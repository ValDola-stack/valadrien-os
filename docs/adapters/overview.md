...
| Hermes | `hermes_local` | Runs the local Hermes CLI through `@valadrien-os/hermes-paperclip-adapter` |
| Hermes Gateway | `hermes_gateway` | Calls an already-running Hermes API server through `@valadrien-os/hermes-paperclip-adapter/gateway` |
...
Local CLI adapters can run on the ValadrienOs host, SSH targets, or managed
...
|---------|---------------------|-------------------------------------------------------|
| [`codex_local`](/adapters/codex-local) | Host-owns-auth for ValadrienOs-managed `CODEX_HOME` | A host-owned `auth.json` is symlinked into the managed `CODEX_HOME` and uploaded to the sandbox. If a per-agent `OPENAI_API_KEY` is configured, ValadrienOs writes an API-key `auth.json` instead and that file wins. A login baked into the sandbox image is shadowed because Codex runs with ValadrienOs's uploaded `CODEX_HOME`. |
| [`claude_local`](/adapters/claude-local) | Snapshot-owns-auth for managed remote Claude config | ValadrienOs uploads only sanitized settings and skill/runtime assets. When the remote managed config has no Claude credential files, it copies `.credentials.json` or `credentials.json` from the sandbox image's own `$HOME/.claude`, so the image's login wins. |
...
- **Claude sandbox with image login:** ValadrienOs materializes a remote
...
Use `hermes_local` when ValadrienOs should start the local `hermes` CLI on the
same host for each heartbeat. Use `hermes_gateway` when Hermes is already
running as an HTTP/SSE API server and ValadrienOs should call that server instead
...
`@valadrien-os/adapter-hermes-gateway` package remains only as a deprecated
compatibility shim that re-exports the gateway entrypoints for one release.
New plugin overrides should target `@valadrien-os/hermes-paperclip-adapter` and
...
|---------|---------|----------|-------------|
| Droid | `@henkey/droid-valadrien-os-adapter` | `droid_local` | Runs Factory Droid locally |
...
curl -X POST http://localhost:3102/api/adapters \
  -d '{"packageName": "my-valadrien-os-adapter"}'
...
my-adapter/
  src/
    ...
    cli/
      format-event.ts   # Terminal output for `valadrien-os run --watch`
```
