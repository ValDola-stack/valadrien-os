...
What it means: the on-disk Claude Code transcript JSONL for that session contains a malformed (non-`msg_`-prefixed) `previous_message_id`. Anthropic's `/v1/messages` rejects every resume attempt against that transcript with a deterministic 400. Without guards, ValadrienOs would re-persist the same poisoned session id and the issue is stranded permanently — see [RED-976](../../../) / [RED-978](../../../).
...
`CLAUDE_CONFIG_DIR` is configured, ValadrienOs creates a remote
...
its own Claude Code login. ValadrienOs starts a managed remote `claude_local` run,
...
ValadrienOs-managed sandbox run uploads a host-owned `CODEX_HOME/auth.json` and
