...
test.describe("NUX Phase 4 visual QA", () => {
  test("captures every integrated surface", async ({ page }) => {
    ...
    const baseUrl =
      "http://127.0.0.1:" + (process.env.VALADRIEN_OS_E2E_PORT ?? "3199");
    ...
    if (!(await page.getByRole("heading", { name: "Welcome to ValadrienOs" }).count())) {
      await page.getByRole("button", { name: /Back to start/ }).click();
    }
    await expect(
      page.getByRole("heading", { name: "Welcome to ValadrienOs" }),
    ).toBeVisible({ timeout: 10_000 });
    await expect(
