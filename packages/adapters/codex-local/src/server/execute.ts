...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const configuredOpenAiApiKey =
    typeof envConfig.OPENAI_API_KEY === "string" && envConfig.OPENAI_API_KEY.trim().length > 0
      ...
      : null;
  // A configured CODEX_HOME that lives under the ValadrienOs-managed company tree
  ...
  // Merge custom model providers (VALADRIEN_OS_CODEX_PROVIDERS) into the managed
  ...
  try {
    for (const note of preparedRuntimeConfig.notes) {
      await onLog("stdout", `[valadrien-os] ${note}\n`);
    }
    ...
    if (monitorResolution.mode === "disabled") {
      await onLog(
        "stdout",
        `[valadrien-os] Codex output inactivity monitor is DISABLED via adapterConfig.outputInactivityTimeoutMs=null. Hung codex runs will only be detected by the platform-level silent-run safety net.\n`,
      );
    } else if (monitorResolution.mode === "default" && "reason" in monitorResolution) {
      await onLog(
        "stdout",
        `[valadrien-os] Ignoring non-positive adapterConfig.outputInactivityTimeoutMs; falling back to default ${monitorResolution.timeoutMs}ms.\n`,
      );
    }
        ...
        monitorResolution.mode === "disabled"
          ...
          : createCodexOutputInactivityMonitor({
              ...
              onFire: (state) => {
                ...
                const logLine =
                  `[valadrien-os] adapter.invoke ${message}; ` +
    ...
    // Restore the managed config.toml so VALADRIEN_OS_CODEX_PROVIDERS changes
