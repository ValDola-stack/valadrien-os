...
describe("codex managed home", () => {
  ...
  it("treats a concurrently-created expected auth symlink as success", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-home-"));
        ...
        prepareManagedCodexHome(
          {
            ...
            VALADRIEN_OS_HOME: valadrienOsHome,
  ...
  // Regression for #5028: older ValadrienOs versions copied auth.json into the
  ...
  it("replaces a stale regular-file auth.json with a symlink to the live source (#5028)", async () => {
    ...
    try {
      ...
      // Simulate a stale copy left by a previous ValadrienOs version.
  ...
  it("ensureSymlink: unlinks a stale regular file and recreates the symlink", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-ensure-symlink-"));
    try {
  ...
  // this path as operator-owned, not a stale ValadrienOs copy.
  it("ensureSymlink: leaves an unexpected directory in place instead of throwing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-ensure-symlink-dir-"));
    try {
      const source = path.join(root, "live-source.json");
...
describe("isManagedCodexHomePath", () => {
  const env = {
    VALADRIEN_OS_HOME: "/srv/valadrien-os",
    VALADRIEN_OS_INSTANCE_ID: "default",
  } satisfies NodeJS.ProcessEnv;
  const companyRoot = path.resolve(
    "/srv/valadrien-os/instances/default/companies/company-1",
  );
  ...
  it("treats a path outside the company tree as an external override", () => {
    ...
    expect(
      isManagedCodexHomePath(
        env,
        "company-1",
        path.resolve("/srv/valadrien-os/instances/default/companies/company-2/codex-home"),
      ),
    ).toBe(false);
  });
...
describe("codexHomeHasUsableAuth", () => {
  it("is true for credential-bearing auth.json and false when missing", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-auth-"));
    try {
  ...
  it("is false for a dangling auth.json symlink", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-auth-dangling-"));
    try {
...
describe("seedManagedCodexHome", () => {
  it("symlinks auth.json from the shared source into an explicit per-agent home", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-seed-"));
    try {
      const sharedCodexHome = path.join(root, "shared-codex-home");
  ...
  it("writes an API-key auth.json into the home when an apiKey is supplied", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-seed-apikey-"));
    try {
...
describe("reconcileManagedCodexHome", () => {
  async function makeFixture() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-reconcile-"));
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const valadrienOsHome = path.join(root, "valadrien-os-home");
    const agentHome = path.join(
      valadrienOsHome,
...
describe("evaluateCodexCredentialReadiness", () => {
  async function makeFixture() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-readiness-"));
    const sharedCodexHome = path.join(root, "shared-codex-home");
    const valadrienOsHome = path.join(root, "valadrien-os-home");
