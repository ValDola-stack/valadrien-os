...
const {
  ...
  readValadrienOsRuntimeSkillEntries: vi.fn(async () => []),
  ...
  tempCodexHome: "/tmp/valadrien-os-codex-acp-fallback-test-home",
}));

vi.mock("./acp.js", () => ({
  createCodexAcpExecutor: () => executeCodexAcp,
  formatCodexAcpFallbackMessage: (reason: string) =>
    `[valadrien-os] Codex ACP default unavailable; falling back to Codex CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`,
