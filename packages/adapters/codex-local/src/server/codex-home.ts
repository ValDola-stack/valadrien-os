...
/**
 * True when `homePath` lives under the ValadrienOs-managed company tree
 ...
 * external/user-supplied override that ValadrienOs must not seed or overwrite.
 */
...
export async function ensureSymlink(target: string, source: string): Promise<void> {
  ...
  if (!existing.isSymbolicLink()) {
    // A previous ValadrienOs version copied this file into the managed home
    ...
    // ValadrienOs-managed company home, never the user's real ~/.codex.
    ...
    // a ValadrienOs-written stale copy and warrants operator inspection rather
    // than silent removal.
...
/**
 * Seeds auth/config into an explicit ValadrienOs-managed `targetHome`. Symlinks
...
export interface CodexCredentialReadiness {
  /** True when ValadrienOs owns the effective home and is responsible for its auth. */
  managed: boolean;
 ...
 *   is always treated as ready (ValadrienOs must not seed or inspect it).
...
export async function evaluateCodexCredentialReadiness(
  ...
  if (!effectiveHomeIsManaged) {
    // Genuine external override: ValadrienOs never seeds or inspects it.
    return {
      managed: false,
