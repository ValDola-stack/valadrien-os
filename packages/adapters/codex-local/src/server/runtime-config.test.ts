...
async function makeCodexHome(configToml?: string): Promise<string> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-home-"));
...
describe("prepareCodexRuntimeConfig", () => {
  it("is a no-op when VALADRIEN_OS_CODEX_PROVIDERS is unset", async () => {
  ...
  it("merges providers + model_provider into a fresh config.toml and cleans it up", async () => {
    ...
    const prepared = await prepareCodexRuntimeConfig({
      env: { VALADRIEN_OS_CODEX_PROVIDERS: JSON.stringify(BIFROST_PROVIDERS) },
      codexHome: home,
    });
  ...
  it("emits plain-object fields as inline tables and arrays as TOML arrays", async () => {
    ...
    const prepared = await prepareCodexRuntimeConfig({
      env: {
        VALADRIEN_OS_CODEX_PROVIDERS: JSON.stringify({
          providers: {
            gw: {
  ...
  it("expands {env:VAR} placeholders from the run env and process.env, leaving unresolvable ones intact", async () => {
    const home = await makeCodexHome();
    process.env.VALADRIEN_OS_CODEX_TEST_PROCESS_KEY = "from-process-env";
    try {
      const prepared = await prepareCodexRuntimeConfig({
        env: {
          VALADRIEN_OS_CODEX_PROVIDERS: JSON.stringify({
            providers: {
              gw: {
                base_url: "http://gw.example/v1",
                http_headers: {
                  "X-Run": "{env:VALADRIEN_OS_CODEX_TEST_RUN_KEY}",
                  "X-Process": "{env:VALADRIEN_OS_CODEX_TEST_PROCESS_KEY}",
          ...
          VALADRIEN_OS_CODEX_TEST_RUN_KEY: "from-run-env",
        },
        codexHome: home,
      });
      ...
      delete process.env.VALADRIEN_OS_CODEX_TEST_PROCESS_KEY;
    }
  });

  it("reads VALADRIEN_OS_CODEX_PROVIDERS from process.env when absent from the run env", async () => {
    const home = await makeCodexHome();
    process.env.VALADRIEN_OS_CODEX_PROVIDERS = JSON.stringify(BIFROST_PROVIDERS);
    try {
      const prepared = await prepareCodexRuntimeConfig({ env: {}, codexHome: home });
      const content = await readConfigToml(home);
      expect(content).toContain("[model_providers.bifrost]");
      ...
      delete process.env.VALADRIEN_OS_CODEX_PROVIDERS;
    }
  });

  it("surfaces a note for malformed VALADRIEN_OS_CODEX_PROVIDERS without touching config.toml", async () => {
    const home = await makeCodexHome("model = \"gpt-5.1-codex\"\n");
    ...
    for (const [raw, noteFragment] of cases) {
      const prepared = await prepareCodexRuntimeConfig({
        env: { VALADRIEN_OS_CODEX_PROVIDERS: raw },
        codexHome: home,
      });
  ...
  it("stays silent when VALADRIEN_OS_CODEX_PROVIDERS is unset or empty", async () => {
    const home = await makeCodexHome("model = \"gpt-5.1-codex\"\n");
    const envs: Array<Record<string, string>> = [
      {},
      { VALADRIEN_OS_CODEX_PROVIDERS: "" },
      { VALADRIEN_OS_CODEX_PROVIDERS: "  " },
    ];
  ...
  it("restores user provider sections from the pre-run backup after an interrupted run", async () => {
    ...
    await expect(fs.access(path.join(home, "config.toml.valadrien-os-backup"))).rejects.toThrow();
    await prepared.cleanup();
  });
  ...
  it("removes the pre-run backup on cleanup", async () => {
    ...
    const backupPath = path.join(home, "config.toml.valadrien-os-backup");
  ...
  it("rejects the block when model_provider names a filtered or missing provider", async () => {
    ...
    expect(prepared.notes).toContain(
      'VALADRIEN_OS_CODEX_PROVIDERS: model_provider "bad" does not match any usable provider entry; custom providers ignored.',
    );
    const content = await readConfigToml(home);
