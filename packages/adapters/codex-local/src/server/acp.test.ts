...
import type { AdapterExecutionContext, AdapterInvocationMeta } from "@valadrien-os/adapter-utils";
import {
  buildCodexAcpConfig,
...
const originalValadrienOsHome = process.env.VALADRIEN_OS_HOME;
const originalValadrienOsInstanceId = process.env.VALADRIEN_OS_INSTANCE_ID;
...
afterEach(async () => {
  ...
  if (originalValadrienOsHome === undefined) delete process.env.VALADRIEN_OS_HOME;
  else process.env.VALADRIEN_OS_HOME = originalValadrienOsHome;
  if (originalValadrienOsInstanceId === undefined) delete process.env.VALADRIEN_OS_INSTANCE_ID;
  else process.env.VALADRIEN_OS_INSTANCE_ID = originalValadrienOsInstanceId;
...
function buildContext(root: string, overrides: Partial<AdapterExecutionContext> = {}): AdapterExecutionContext {
  return {
    ...
    context: {
      issueId: "issue-1",
      valadrienOsTaskMarkdown: "Task context",
...
describe("codex_local ACP lane", () => {
  it("defaults to ACP when prerequisites pass and falls back to CLI only for auto resolution", async () => {
    const root = await makeTempRoot("valadrien-os-codex-acp-default-");
  ...
  it("reports ACP prerequisites for the ACP lane", async () => {
    const root = await makeTempRoot("valadrien-os-codex-acp-env-");
    const commandPath = path.join(root, "bin", "codex-acp");
    await fs.mkdir(path.dirname(commandPath), { recursive: true });
    await fs.writeFile(commandPath, "#!/usr/bin/env sh\n", "utf8");
    setNodeVersion("v22.13.0");
  ...
  it("executes through ACPX with Codex session config and ephemeral skills", async () => {
    const root = await makeTempRoot("valadrien-os-codex-acp-exec-");
  ...
  it("resumes compatible ACP sessions on later Codex ACP runs", async () => {
    const root = await makeTempRoot("valadrien-os-codex-acp-resume-");
    const runtimes: FakeRuntime[] = [];
    const execute = createCodexAcpExecutor({
      createRuntime: (options: FakeRuntimeOptions) => {
        const runtime = new FakeRuntime(options);
