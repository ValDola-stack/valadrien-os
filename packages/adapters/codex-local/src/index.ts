...
- ValadrienOs injects desired local skills into the effective CODEX_HOME/skills/ directory at execution time so Codex can discover "$valadrien-os" and related skills without polluting the project working directory. For new and updated agents, ValadrienOs assigns an isolated managed home at ~/.valadrien-os/instances/<id>/companies/<companyId>/agents/<agentId>/codex-home/skills/; when CODEX_HOME is explicitly overridden in adapter config, that override is used instead.
...
- Fast mode is supported on GPT-5.6, GPT-5.5, GPT-5.4 and manual model IDs. When enabled for those models, ValadrienOs applies \`service_tier="fast"\` and \`features.fast_mode=true\`.
