...
Use when:
- You want ValadrienOs to run Pi (the AI coding agent) locally as the agent runtime
...
- ValadrienOs requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/valadrien-oss/ and resumed with --session.
