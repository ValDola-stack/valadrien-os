...
Use when:
- You want ValadrienOs to run Cursor Agent CLI locally as the agent runtime
...
- ValadrienOs auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$valadrien-os" and related skills on local runs.
- ValadrienOs auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
