...
function renderApiAccessNote(env: Record<string, string>): string {
  if (!hasNonEmptyEnvValue(env, "VALADRIEN_OS_API_URL") || !hasNonEmptyEnvValue(env, "VALADRIEN_OS_API_KEY")) return "";
  return [
    "ValadrienOs API access note:",
    "Use run_shell_command with curl to make ValadrienOs API requests.",
    "GET example:",
    `  run_shell_command({ command: "curl -s -H \\"Authorization: Bearer $VALADRIEN_OS_API_KEY\\" \\"$VALADRIEN_OS_API_URL/api/agents/me\\"" })`,
    "POST/PATCH example:",
    `  run_shell_command({ command: "curl -s -X POST -H \\"Authorization: Bearer $VALADRIEN_OS_API_KEY\\" -H 'Content-Type: application/json' -H \\"X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID\\" -d '{...}' \\"$VALADRIEN_OS_API_URL/api/issues/{id}/checkout\\"" })`,
    "",
    "",
  ].join("\n");
}
...
/**
 * Inject ValadrienOs skills directly into `~/.gemini/skills/` via symlinks.
...
async function ensureGeminiSkillsInjected(
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[valadrien-os] Failed to prepare Gemini skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const skillName of removedSkills) {
    await onLog(
      "stderr",
      `[valadrien-os] Removed maintainer-only Gemini skill "${skillName}" from ${skillsHome}\n`,
    );
  }
  ...
  for (const entry of selectedEntries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[valadrien-os] ${result === "repaired" ? "Repaired" : "Linked"} Gemini skill: ${entry.key}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[valadrien-os] Failed to link Gemini skill "${entry.key}": ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
async function buildGeminiSkillsDir(
  ...
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-gemini-skills-"));
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const geminiSkillEntries = await readValadrienOsRuntimeSkillEntries(config, __moduleDir);
  const desiredGeminiSkillNames = resolveValadrienOsDesiredSkillNames(config, geminiSkillEntries);
  ...
  if (executionTargetIsRemote) {
    try {
      ...
      await onLog(
        "stdout",
        `[valadrien-os] Syncing workspace and Gemini runtime assets to ${describeAdapterExecutionTarget(executionTarget)}.\n`,
      );
  ...
  if (executionTargetIsRemote && adapterExecutionTargetUsesValadrienOsBridge(executionTarget)) {
  ...
  if (executionTargetIsRemote && runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[valadrien-os] Gemini session "${runtimeSessionId}" does not match the current remote execution identity and will not be resumed in "${effectiveExecutionCwd}". Starting a fresh remote session.\n`,
    );
  } else if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stdout",
      `[valadrien-os] Gemini session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${effectiveExecutionCwd}".\n`,
    );
  }
  ...
  const runAttempt = async (resumeSessionId: string | null) => {
    ...
    const proc = await runAdapterExecutionTargetProcess(runId, runtimeExecutionTarget, command, args, {
      ...
      onRuntimeProgress: ctx.onRuntimeProgress,
      onLog,
      runLogTail: valadrienOsBridge?.runLogTail,
    });
    return {
      proc,
  ...
  try {
    ...
    if (
      ...
      await onLog(
        "stdout",
        `[valadrien-os] Gemini resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
      );
