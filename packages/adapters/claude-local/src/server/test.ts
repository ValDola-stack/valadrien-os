...
export async function testEnvironment(
  ...
  if (targetIsRemote && adapterExecutionTargetUsesManagedHome(target) && !hasExplicitClaudeConfigDir) {
    ...
    try {
      ...
      tempWorkspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-claude-envtest-workspace-"));
      ...
      const runtimeRootDir =
        preparedRuntime.runtimeRootDir ?? path.posix.join(managedRemoteCwd, ".valadrien-os-runtime", "claude");
      ...
      checks.push({
        ...
        level: "info",
        message: "Sandbox probe is using ValadrienOs-managed Claude config materialization.",
      ...
      checks.push({
        ...
        level: "error",
        message: "Could not materialize ValadrienOs-managed Claude config for the sandbox probe.",
