...
} from "@valadrien-os/adapter-utils/acpx-engine/constants";
import type { AcpxEngineExecutorOptions } from "@valadrien-os/adapter-utils/acpx-engine/execute";
import {
  asNumber,
  asString,
  parseObject,
...
export function formatClaudeAcpFallbackMessage(reason: string): string {
  return `[valadrien-os] Claude ACP default unavailable; falling back to Claude CLI. ${reason} Set engine=acp to require ACP or engine=cli to silence this fallback.\n`;
}
...
export function createClaudeAcpExecutor(options: ClaudeAcpExecutorOptions = {}): ClaudeAcpExecutor {
  ...
  return async (ctx) => {
    ...
    if (!currentExecutor) {
      const { createAcpxEngineExecutor } = await import("@valadrien-os/adapter-utils/acpx-engine/execute");
...
export async function testClaudeAcpEnvironment(
  ...
  if (targetIsRemote) {
    checks.push({
      ...
      hint: "Remote ACP requires a bidirectional process target such as SSH or ValadrienOs's sandbox process-session bridge.",
    });
  }
