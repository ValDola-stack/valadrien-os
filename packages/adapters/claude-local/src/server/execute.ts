...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (executionTargetIsSandbox && effort) {
    ...
    if (supportsEffort === false) {
      ...
      await onLog(
        "stderr",
        `[valadrien-os] Claude CLI in the sandbox does not advertise --effort; omitting configured effort "${effort}". Upgrade the sandbox CLI/image to restore reasoning-effort control.\n`,
      );
    }
  }
  ...
  if (runtimeSessionId && !isValidUuid) {
    await onLog(
      "stdout",
      `[valadrien-os] Claude session "${runtimeSessionId}" is not a valid UUID and will not be passed to --resume.\n`,
    );
  }
  if (
    executionTargetIsRemote &&
  ...
  const toAdapterResult = (
    ...
    // successful run to ValadrienOs and the heartbeat stalls silently. See RY-604.
  ...
  try {
    ...
    if (sessionErrorKind !== null) {
      ...
      await onLog(
        "stdout",
        `[valadrien-os] Claude resume session "${sessionId}" ${reason}; retrying with a fresh session.\n`,
      );
      if (sessionErrorKind === "poisoned" && !executionTargetIsRemote) {
        ...
        if (unlinked) {
          try {
            await onLog("stdout", `[valadrien-os] Removed poisoned session file: ${poisonedJsonlPath}\n`);
          } catch {
