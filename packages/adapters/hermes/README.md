# ValadrienOs Adapters for Hermes Agent

A [ValadrienOs](https://valadrien-os.ing) adapter package that lets you run [Hermes Agent](https://github.com/NousResearch/hermes-agent) as a managed employee in a ValadrienOs company.
...
Choose `hermes_local` when ValadrienOs and Hermes run on the same trusted host
and ValadrienOs should start `hermes chat` for each heartbeat. Choose
...
- **Skills integration** — Scans both ValadrienOs-managed and Hermes-native skills (`~/.hermes/skills/`), with sync/list/resolve APIs
- **Structured transcript parsing** — Raw Hermes stdout is parsed into typed `TranscriptEntry` objects so ValadrienOs renders proper tool cards with status icons and expand/collapse
...
This package ships with ValadrienOs core as the built-in `hermes_local` and
`hermes_gateway` adapters. No Adapter manager installation is required for
normal ValadrienOs use.
...
manager, or add an entry to `~/.valadrien-os/adapter-plugins.json` and restart
ValadrienOs. The external package can override either built-in Hermes adapter
while it is enabled:

```json
[
  {
    "packageName": "@valadrien-os/hermes-paperclip-adapter",
    "localPath": "/absolute/path/to/valadrien-os/packages/adapters/hermes",
    "type": "hermes_local",
...
ValadrienOs core imports these same package entrypoints for built-in adapter
registration.

### 2. Create a local Hermes agent in ValadrienOs

In the ValadrienOs UI or via API, create an agent with adapter type `hermes_local`:
...
This mode shells out to the local `hermes` CLI. ValadrienOs injects runtime
environment variables and captures stdout/stderr from the child process.

### 3. Create a Hermes gateway agent in ValadrienOs
...
```json
{
  ...
  "adapterConfig": {
    ...
    "valadrienOsApiUrl": "http://127.0.0.1:3100",
...
`http://127.0.0.1:9119/chat`, ValadrienOs maps it to
`http://127.0.0.1:9119/api` before calling Hermes API routes. `/chat` and the
dashboard root are browser UI routes; ValadrienOs tests `/api/health` and starts
...
`@valadrien-os/adapter-hermes-gateway` remains as a deprecated compatibility shim
for one release. It re-exports the gateway entrypoints from
`@valadrien-os/hermes-paperclip-adapter/gateway` and preserves the legacy exports
for existing plugin installs. New installs and built-in ValadrienOs registrations
should use `@valadrien-os/hermes-paperclip-adapter`; the adapter type remains
`hermes_gateway`.

### Runtime API guidance

Hermes receives ValadrienOs runtime identity through environment variables:
...
`Authorization: Bearer $VALADRIEN_OS_API_KEY` on ValadrienOs API requests and
`X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID` on mutating issue requests. For
multiline comments or status updates, preserve newlines with a heredoc plus
`jq --arg`.

### Hermes-originated ValadrienOs tasks

The package includes a Hermes skill/helper for the reverse direction: a user
starts in Hermes and asks Hermes to create or update ValadrienOs work. This is not
the same as ValadrienOs waking Hermes through `hermes_local` or `hermes_gateway`.

Configure ValadrienOs access in Hermes env/profile secrets, not prompt text:

```bash
VALADRIEN_OS_API_URL=http://127.0.0.1:3100/api
VALADRIEN_OS_BRIDGE_API_KEY=<task-bridge-scoped-agent-api-key>
```
...
The bundled `valadrien-os-task-bridge` skill provides deterministic helper
commands:

```bash
node ./valadrien-os-task.mjs list-assigned
node ./valadrien-os-task.mjs create-task --parent-id "<approved-parent-issue-id>" --title "Investigate checkout failures" --description "Capture failing request and root cause."
node ./valadrien-os-task.mjs comment --issue PAP-123 --body "Found the failing request path."
node ./valadrien-os-task.mjs update-status --issue PAP-123 --status in_review --comment "Ready for review."
```
...
Create issues in ValadrienOs and assign them to your Hermes agent. On each heartbeat, Hermes will:

1. Receive the task instructions
2. Use its full tool suite to complete the work
3. Report results back to ValadrienOs
...
| `valadrienOsApiUrl` | string | `http://127.0.0.1:3100/api` | ValadrienOs API base URL |
...
| `{{valadrienOsApiUrl}}` | ValadrienOs API base URL |
...
6. **Reports** results back to ValadrienOs with cost, usage, and session state
...
- **ValadrienOs-managed skills** — bundled with the adapter, togglable from the UI
...
ValadrienOs UI can display both managed and native skills in one view.

## Development

```bash
git clone https://github.com/paperclipai/paperclip
cd valadrien-os/packages/adapters/hermes
pnpm install
pnpm build
```
...
- [ValadrienOs](https://github.com/paperclipai/paperclip) — The orchestration platform
- [Nous Research](https://nousresearch.com) — The team behind Hermes
- [ValadrienOs Docs](https://docs.valadrien-os.ing) — ValadrienOs documentation
