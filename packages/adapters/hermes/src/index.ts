/**
 * Hermes Agent adapter for ValadrienOs.
 *
 * Runs Hermes Agent (https://github.com/NousResearch/hermes-agent)
 * as a managed employee in a ValadrienOs company. Hermes Agent is a
 ...
 *
 * Hermes supports any model via any provider. The ValadrienOs UI should
...
/**
 * Documentation shown in the ValadrienOs UI when configuring a Hermes agent.
 */
...
This adapter package also ships a Hermes-facing ValadrienOs task bridge skill:
\`valadrien-os-task-bridge\`. Use it when a user starts in Hermes and asks Hermes
to create, comment on, update, or list ValadrienOs tasks.
...
- \`VALADRIEN_OS_BRIDGE_API_KEY\` - ValadrienOs agent API key created with \`scope.kind = "task_bridge"\`
- optional fallback \`VALADRIEN_OS_API_KEY\` - must still be a task_bridge key, never a normal claimed agent key
- optional \`VALADRIEN_OS_COMPANY_ID\`, \`VALADRIEN_OS_AGENT_ID\`, and \`VALADRIEN_OS_RUN_ID\`

The bridge is separate from adapter execution:

- \`hermes_local\` means ValadrienOs shells out to local \`hermes chat\`.
- \`hermes_gateway\` means ValadrienOs wakes remote Hermes through Hermes's API server.
- \`valadrien-os-task-bridge\` means Hermes calls ValadrienOs's REST API to manage tasks.

Create task bridge keys with a parent issue or project boundary. Do not expose
normal claimed ValadrienOs agent API keys to internet-facing Hermes chat/webhook
...
- \`{{companyId}}\` — ValadrienOs company ID
...
/**
 * External adapter plugin entrypoint expected by ValadrienOs's adapter manager.
 */
