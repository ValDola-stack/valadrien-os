...
/**
 * Parse Hermes Agent stdout into TranscriptEntry objects for the ValadrienOs UI.
 *
 ...
 *   System:     "[hermes] ..."
 *
 * We emit structured tool_call/tool_result pairs so ValadrienOs renders proper
 ...
 * ValadrienOs uses this to match them in normalizeTranscript.
 */
 ...
 * ValadrienOs renders proper tool cards with status icons and expand/collapse.
 *
...
export function parseHermesStdoutLine(
  ...
  if (trimmed.startsWith("[hermes]") || trimmed.startsWith("[valadrien-os]")) {
  ...
  // Emit as stderr so ValadrienOs groups them into the amber accordion.
