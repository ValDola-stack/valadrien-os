 ...
 *
 * Translates ValadrienOs's CreateConfigValues into the adapterConfig
...
/**
 * Build a Hermes Agent adapter config from the ValadrienOs UI form values.
 */
export function buildHermesConfig(
  ...
  // NOTE: Provider is NOT set here because the ValadrienOs UI form
  ...
  // Execution limits — let the user configure these from the ValadrienOs UI.
  ...
  // Heartbeat config is handled by ValadrienOs itself
