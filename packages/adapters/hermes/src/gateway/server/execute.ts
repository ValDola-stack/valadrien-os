...
const VALADRIEN_OS_SESSION_KEY_PATTERN =
  /\bvaladrien-os:(?:company:[A-Za-z0-9-]+:agent:[A-Za-z0-9-]+(?::(?:issue|run):[A-Za-z0-9-]+)?|run:[A-Za-z0-9-]+)\b/gi;
...
export function resolveSessionKey(input: {
  ...
  if (input.strategy === "agent") {
    return `valadrien-os:company:${input.companyId}:agent:${input.agentId}`;
  }
  if (input.strategy === "run") {
    return `valadrien-os:run:${input.runId}`;
  }
  const issuePart = input.issueId ? `issue:${input.issueId}` : `run:${input.runId}`;
  return `valadrien-os:company:${input.companyId}:agent:${input.agentId}:${issuePart}`;
}
...
function sanitizeSensitiveText(value: string): string {
  return value
    ...
    .replace(VALADRIEN_OS_SESSION_KEY_PATTERN, "[redacted-session-key]");
}
...
function buildInput(ctx: AdapterExecutionContext, valadrienOsApiUrl: string | null): string {
  const wakePrompt = renderValadrienOsWakePrompt(ctx.context.valadrienOsWake);
  const wakePayloadJson = stringifyValadrienOsWakePayload(ctx.context.valadrienOsWake);
  const taskMarkdown = nonEmpty(ctx.context.valadrienOsTaskMarkdown);
  const sessionHandoff = nonEmpty(ctx.context.valadrienOsSessionHandoffMarkdown);
  const issueWorkMode = readValadrienOsIssueWorkModeFromContext(ctx.context);
  const lines = [
    `You are ${ctx.agent.name}, an AI agent employee in a ValadrienOs-managed company.`,
    "",
    "ValadrienOs runtime identity:",
    `- Agent ID: ${ctx.agent.id}`,
    `- Company ID: ${ctx.agent.companyId}`,
    `- Run ID: ${ctx.runId}`,
    ...(valadrienOsApiUrl ? [`- ValadrienOs API URL: ${valadrienOsApiUrl}`] : []),
    ...
    "- Use X-ValadrienOs-Run-Id on mutating ValadrienOs API requests when a ValadrienOs API key is available.",
    "",
    wakePrompt,
...
function buildRunBody(ctx: AdapterExecutionContext, sessionKey: string | null): Record<string, unknown> {
  const valadrienOsApiUrl = nonEmpty(ctx.config.valadrienOsApiUrl);
  const payloadTemplate = parseObject(ctx.config.payloadTemplate);
  const input = nonEmpty(payloadTemplate.input) ?? buildInput(ctx, valadrienOsApiUrl);
  const instructions =
    nonEmpty(ctx.config.instructions) ??
    nonEmpty(payloadTemplate.instructions) ??
    "Follow the ValadrienOs wake instructions exactly. Do not expose secrets in logs, comments, or final output.";
  return {
    ...payloadTemplate,
    input,
    instructions,
