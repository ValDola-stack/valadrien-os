 ...
 * and returns structured results to ValadrienOs.
 *
...
const HERMES_DEFAULT_PROMPT_TEMPLATE = [
  'You are "{{agent.name}}", an AI agent employee in a ValadrienOs-managed company.',
  "",
  "ValadrienOs runtime identity:",
  "- Agent ID: {{agent.id}}",
  "- Company ID: {{agent.companyId}}",
  "- Run ID: {{run.id}}",
  "- API base: {{valadrienOsApiUrl}}",
  "",
  "ValadrienOs API guidance:",
  "- Use `curl` from the terminal for ValadrienOs API calls; browser/web extraction tools may not reach localhost.",
  "- Use `$VALADRIEN_OS_API_URL`, `$VALADRIEN_OS_API_KEY`, and `$VALADRIEN_OS_RUN_ID`; do not hard-code local ports or copy secrets into comments.",
  "- Displayed command logs may redact secrets; rely on environment variables instead of printed token values.",
  "- Include `-H \"Authorization: Bearer $VALADRIEN_OS_API_KEY\"` on API requests.",
  "- Include `-H \"X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID\"` on mutating issue requests.",
  ...
  "    -H \"X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID\" \\",
...
export function buildPrompt(
  ...
  let valadrienOsApiUrl =
    cfgString(config.valadrienOsApiUrl) ||
    process.env.VALADRIEN_OS_API_URL ||
    "http://127.0.0.1:3100/api";
  // Ensure /api suffix
  if (!valadrienOsApiUrl.endsWith("/api")) {
    valadrienOsApiUrl = valadrienOsApiUrl.replace(/\/+$/, "") + "/api";
  }

  const wakePrompt = renderValadrienOsWakePrompt(context.valadrienOsWake, {
    resumedSession: options.resumedSession === true,
  });
  const valadrienOsTaskMarkdown = cfgString(context.valadrienOsTaskMarkdown)?.trim() || "";
  const sessionHandoffMarkdown = cfgString(context.valadrienOsSessionHandoffMarkdown)?.trim() || "";
  ...
  const vars: Record<string, unknown> = {
    ...
    projectName,
    valadrienOsApiUrl,
    valadrienOsWakePrompt: wakePrompt,
    valadrienOsTaskMarkdown,
    taskContext: valadrienOsTaskMarkdown,
    valadrienOsWakeJson: wakePayloadJson,
    wakePayloadJson,
    valadrienOsApiKeyEnv: "VALADRIEN_OS_API_KEY",
    valadrienOsRunIdEnv: "VALADRIEN_OS_RUN_ID",
  };
...
function cleanResponse(raw: string): string {
  return raw
    ...
    .filter((line) => {
      ...
      if (t.startsWith("[tool]") || t.startsWith("[hermes]") || t.startsWith("[valadrien-os]")) return false;
...
export async function execute(
  ...
  // ── Load agent instructions file (ValadrienOs instruction bundles) ──────
  // ValadrienOs can materialize managed instructions into instructionsFilePath;
  ...
  if (instructionsFilePath) {
    try {
      ...
      // ValadrienOs UI doesn't render this as a red error (stderr output is
  ...
  // ValadrienOs agents run as non-interactive subprocesses with no TTY,
  ...
  if (ctx.runId) env.VALADRIEN_OS_RUN_ID = ctx.runId;

  // BUG FIX: Inject authToken as VALADRIEN_OS_API_KEY (matches adapter-claude-local behavior)
  if ((ctx as any).authToken) env.VALADRIEN_OS_API_KEY = (ctx as any).authToken;
  ...
  if (envTaskId) env.VALADRIEN_OS_TASK_ID = envTaskId;
  const envWakeReason = cfgString(ctxContext.wakeReason) || cfgString(ctx.config?.wakeReason);
  if (envWakeReason) env.VALADRIEN_OS_WAKE_REASON = envWakeReason;
  const envCommentId = cfgString(ctxContext.commentId) || cfgString(ctxContext.wakeCommentId) || cfgString(ctx.config?.commentId);
  if (envCommentId) env.VALADRIEN_OS_WAKE_COMMENT_ID = envCommentId;
  const wakePayloadJson = stringifyValadrienOsWakePayload(ctxContext.valadrienOsWake);
  ...
  // ValadrienOs renders all stderr as red/error in the UI.
  ...
  // Set resultJson so ValadrienOs can persist run metadata (used for UI display + auto-comments)
