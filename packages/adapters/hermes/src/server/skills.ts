...
async function buildSkillEntry(
  ...
  return {
    ...
    readOnly: true, // Hermes manages its own skills — ValadrienOs can't toggle them
...
async function buildHermesSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  // 1. Scan ValadrienOs-managed skills (bundled with the adapter)
  const valadrienOsEntries = await readValadrienOsRuntimeSkillEntries(config, __moduleDir);
  const desiredSkills = resolveValadrienOsDesiredSkillNames(config, valadrienOsEntries);
  const desiredSet = new Set(desiredSkills);
  const availableByKey = new Map(valadrienOsEntries.map((e) => [e.key, e]));
  ...
  // 3. Merge: ValadrienOs skills first (ephemeral), then Hermes skills
  const entries: AdapterSkillEntry[] = [];
  const warnings: string[] = [];

  // ValadrienOs-managed skills
  for (const entry of valadrienOsEntries) {
  ...
  for (const entry of hermesSkillEntries) {
    // Skip if ValadrienOs already manages a skill with the same key
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(
      `Desired skill "${desiredSkill}" is not available in ValadrienOs or Hermes skills.`,
    );
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      ...
      targetPath: null,
      detail:
        "Cannot find this skill in ValadrienOs or ~/.hermes/skills/.",
    });
  }
