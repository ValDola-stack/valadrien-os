...
test("renders standard assignment wake with task authority and no backlog discovery guidance", () => {
  ...
  expect(prompt).toContain("ValadrienOs task context:");
...
test("renders scoped planning wake authority before the Hermes default workflow", () => {
  const prompt = buildPrompt(baseContext(), {
    valadrienOsApiUrl: "http://127.0.0.1:3101/api",
  });
...
test("renders resume deltas instead of full scoped-wake boilerplate when continuing a session", () => {
  ...
  expect(prompt).toContain("## ValadrienOs Resume Delta");
  expect(prompt).toContain("You are resuming an existing ValadrienOs session.");
...
test("renders safe ValadrienOs API examples from environment variables with multiline update preservation", () => {
  const prompt = buildPrompt(baseContext(), {
    valadrienOsApiUrl: "http://valadrien-os.local/api",
  });

  expect(prompt).toContain("Use `$VALADRIEN_OS_API_URL`, `$VALADRIEN_OS_API_KEY`, and `$VALADRIEN_OS_RUN_ID`");
  expect(prompt).toContain("Displayed command logs may redact secrets");
  expect(prompt).toContain('-H "Authorization: Bearer $VALADRIEN_OS_API_KEY"');
  expect(prompt).toContain('-H "X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID"');
...
test("preserves custom prompt templates while exposing runtime and wake variables", () => {
  const prompt = buildPrompt(baseContext(), {
    ...
    promptTemplate: [
      "CUSTOM TEMPLATE",
      "agent={{agent.name}}",
      "api={{valadrienOsApiUrl}}",
      "keyEnv={{valadrienOsApiKeyEnv}}",
      "runEnv={{valadrienOsRunIdEnv}}",
      "wakePrompt={{valadrienOsWakePrompt}}",
      "task={{valadrienOsTaskMarkdown}}",
      "wakeJson={{valadrienOsWakeJson}}",
  ...
  expect(prompt).toContain("api=http://valadrien-os.local/api");
  expect(prompt).toContain("keyEnv=VALADRIEN_OS_API_KEY");
  expect(prompt).toContain("runEnv=VALADRIEN_OS_RUN_ID");
  expect(prompt).toContain("wakePrompt=## ValadrienOs Wake Payload");
  expect(prompt).toContain("task=ValadrienOs task context:");
  expect(prompt).toContain("wakeJson={\"reason\":\"issue_assigned\"");
  expect(prompt).toContain('"reason":"issue_assigned"');
  expect(prompt).toContain("## ValadrienOs Wake Payload");
  expect(prompt).toContain("Issue description:\n```text\nUse the wake payload as runtime authority.\n```");
  expect(prompt).not.toContain("ValadrienOs runtime identity:");
});
