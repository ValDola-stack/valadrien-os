...
test("root package export exposes ValadrienOs external adapter entrypoint", () => {
...
test("Hermes adapter exposes bundled ValadrienOs task bridge skill", async () => {
  const adapter = createServerAdapter();
  ...
  expect(snapshot?.entries.some((entry) => entry.runtimeName === "valadrien-os-task-bridge")).toBe(true);
});
