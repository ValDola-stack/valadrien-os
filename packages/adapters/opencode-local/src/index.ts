...
// retries fail with "model not found". VALADRIEN_OS_OPENCODE_CHEAP_MODEL takes priority;
// VALADRIEN_OS_OPENCODE_SMALL_MODEL (the auxiliary/title model) is reused as a sensible
...
export function buildOpenCodeModelProfiles(
  ...
  const override = (env.VALADRIEN_OS_OPENCODE_CHEAP_MODEL ?? env.VALADRIEN_OS_OPENCODE_SMALL_MODEL)?.trim();
  return [
    {
      key: "cheap",
      label: "Cheap",
