...
describe("opencode remote execution", () => {
  ...
  it("prepares the workspace, syncs OpenCode skills, and restores workspace changes for remote SSH execution", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-opencode-remote-"));
    ...
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.valadrien-os-runtime/opencode/xdgConfig`,
    }));
    expect(syncDirectoryToSsh).toHaveBeenCalledWith(expect.objectContaining({
      remoteDir: `${managedRemoteWorkspace}/.valadrien-os-runtime/opencode/skills`,
      followSymlinks: true,
    }));
    ...
    expect(modelProbeCall?.[3].env.XDG_CONFIG_HOME).toBe(
      `${managedRemoteWorkspace}/.valadrien-os-runtime/opencode/xdgConfig`,
    );
    ...
    expect(call?.[3].env.XDG_CONFIG_HOME).toBe(`${managedRemoteWorkspace}/.valadrien-os-runtime/opencode/xdgConfig`);
  ...
  it("fails before the remote run when the configured model is unavailable on the SSH target", async () => {
    ...
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-opencode-remote-model-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
    await mkdir(workspaceDir, { recursive: true });
    ...
    expect(startAdapterExecutionTargetValadrienOsBridge).not.toHaveBeenCalled();
  });

  it("resumes saved OpenCode sessions for remote SSH execution only when the identity matches", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-opencode-remote-resume-"));
    cleanupDirs.push(rootDir);
    const workspaceDir = path.join(rootDir, "workspace");
