...
import { asBoolean } from "@valadrien-os/adapter-utils/server-utils";
...
function parseProviderConfig(
  ...
  try {
    ...
    notes.push("VALADRIEN_OS_OPENCODE_PROVIDERS contains invalid JSON; custom providers ignored.");
    return null;
  }
  ...
  if (skipped.length > 0) {
    notes.push(
      `VALADRIEN_OS_OPENCODE_PROVIDERS: skipped provider(s) with non-object values: ${skipped.join(", ")}.`,
    );
  }
...
export async function prepareOpenCodeRuntimeConfig(input: {
  ...
  const runtimeConfigHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-opencode-config-"));
  ...
  // Merge gateway/custom provider definitions supplied via VALADRIEN_OS_OPENCODE_PROVIDERS
  ...
  const gatewayProviders = parseProviderConfig(
    input.env.VALADRIEN_OS_OPENCODE_PROVIDERS ?? process.env.VALADRIEN_OS_OPENCODE_PROVIDERS,
    resolveEnv,
    notes,
  );
  ...
  if (gatewayProviders) {
    notes.push(
      `Injected ${Object.keys(gatewayProviders).length} custom OpenCode provider(s) from VALADRIEN_OS_OPENCODE_PROVIDERS: ${Object.keys(gatewayProviders).join(", ")}.`,
    );
  }
  ...
  // other helper tasks) via VALADRIEN_OS_OPENCODE_SMALL_MODEL. OpenCode otherwise
  ...
  const smallModel = (input.env.VALADRIEN_OS_OPENCODE_SMALL_MODEL ?? process.env.VALADRIEN_OS_OPENCODE_SMALL_MODEL)?.trim();
  if (smallModel) {
