...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  try {
    ...
    // log file is unreachable. Toggle via VALADRIEN_OS_OPENCODE_PRINT_LOGS (run env,
    // then process env).
    const printLogs = isTruthyEnvFlag(
      env.VALADRIEN_OS_OPENCODE_PRINT_LOGS ?? process.env.VALADRIEN_OS_OPENCODE_PRINT_LOGS,
    );
