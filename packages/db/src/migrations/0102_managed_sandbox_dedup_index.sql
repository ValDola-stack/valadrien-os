...
-- Keeps the oldest row per (company_id) where driver='sandbox' AND metadata.managedByValadrienOs is true.
-- Scoped to ValadrienOs-managed rows only: tenant-created sandbox envs (without the marker) are untouched.
DO $$
DECLARE
...
BEGIN
  WITH ranked AS (
    ...
    WHERE driver = 'sandbox'
      AND (metadata ->> 'managedByValadrienOs')::boolean = true
  ),
  deleted AS (
