 ...
 *     --config /path/to/valadrien-os/config.json \
 *     [--claude-config-dir ~/.claude] \
 *     [--dry-run] [--json]
 *
 * Or in a ValadrienOs checkout shell:
 *   pnpm --filter @valadrien-os/db exec tsx scripts/clean-poisoned-claude-sessions.ts --dry-run
 *
...
Flags:
  --config <path>            Path to valadrien-os config.json (defaults to
                             $VALADRIEN_OS_HOME/instances/default/config.json or
...
function defaultConfigPath(): string | null {
  ...
  if (process.env.VALADRIEN_OS_HOME) {
    candidates.push(path.join(process.env.VALADRIEN_OS_HOME, "config.json"));
  }
  candidates.push(path.join(home, ".valadrien-os", "instances", "default", "config.json"));
...
async function main(argv: string[]): Promise<void> {
  ...
  if (!databaseUrl) {
    throw new Error(
      "Unable to resolve database URL. Pass --database-url or --config <valadrien-os config.json>.",
    );
  }
