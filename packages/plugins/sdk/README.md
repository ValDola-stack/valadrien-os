...
- **Testing:** `@valadrien-os/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@valadrien-os/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@valadrien-os/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@valadrien-os/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@valadrien-os/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, `useHostNavigation`, slot prop types |
| `@valadrien-os/plugin-sdk/ui/hooks` | Hooks only |
| `@valadrien-os/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@valadrien-os/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@valadrien-os/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@valadrien-os/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@valadrien-os/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@valadrien-os/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @valadrien-os/plugin-sdk
```
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@valadrien-os/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@valadrien-os/plugin-sdk`.
...
metadata. Slow or failing plugins are isolated: ValadrienOs logs the failure and
...
import { usePluginAction, type PluginBridgeError } from "@valadrien-os/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@valadrien-os/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@valadrien-os/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@valadrien-os/plugin-sdk/ui";
...
The host provides selected shared UI components through `@valadrien-os/plugin-sdk/ui`.
...
```tsx
import { FileTree, type FileTreeNode } from "@valadrien-os/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@valadrien-os/plugin-sdk/ui`.
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@valadrien-os/plugin-sdk/ui";
...
```bash
valadrien-os-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@valadrien-os/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__valadrien_os__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__valadrien_os__/events` streams `reload` SSE events on UI build changes
