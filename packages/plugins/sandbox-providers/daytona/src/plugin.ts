...
const WORKSPACE_SENTINEL_RELATIVE_PATH = ".valadrien-os-runtime/reusable-sandbox-lease.json";
...
function buildSandboxLabels(input: {
  ...
  return {
    ...
    ...(input.setupSessionId ? { "valadrien-os-setup-session-id": input.setupSessionId } : {}),
    ...(input.purpose ? { "valadrien-os-purpose": input.purpose } : {}),
  };
}
...
async function resolveSandboxWorkingDirectory(sandbox: Sandbox): Promise<string> {
  const root = (await sandbox.getWorkDir())?.trim()
    ...
    || "/home/daytona";
  const remoteCwd = path.posix.join(root, "valadrien-os-workspace");
...
function leaseMetadata(input: {
  ...
  return {
    ...
    // Record the resources ValadrienOs attempted to request so future diagnosis
...
async function executeOneShot(
  ...
  const stdinPath = params.stdin != null ? `/tmp/valadrien-os-stdin-${randomUUID()}` : null;
...
const plugin = definePlugin({
  ...
  async onEnvironmentCaptureTemplate(
    ...
    const templateRef = sanitizeSnapshotName(
      ...
      `valadrien-os-${params.environmentId}-${randomUUID().slice(0, 8)}`,
    );
