...
describe("buildNetworkPolicyManifests", () => {
  const baseInput = {
    namespace: "valadrien-os-acme",
    valadrienOsServerNamespace: "valadrien-os",
  ...
  it("produces a deny-all + egress allow pair", () => {
    ...
    expect(manifests[0].metadata.name).toBe("valadrien-os-deny-all");
    expect(manifests[1].metadata.name).toBe("valadrien-os-egress-allow");
  });
  ...
  it("egress allow includes kube-dns and valadrien-os-server callback", () => {
    ...
    const valadrienOsRule = rules.find((r: { to: { namespaceSelector?: { matchLabels?: Record<string, string> } }[] }) =>
      r.to.some((t) => t.namespaceSelector?.matchLabels?.["kubernetes.io/metadata.name"] === "valadrien-os"),
    );
    expect(valadrienOsRule).toBeDefined();
  });
  ...
  it("uses valadrien-os-server pod label selector for callback ingress to valadrien-os ns", () => {
    const [, egress] = buildNetworkPolicyManifests(baseInput);
    const callbackRule = egress.spec.egress.find((r: { to: { podSelector?: { matchLabels?: Record<string, string> } }[] }) =>
      r.to.some((t) => t.podSelector?.matchLabels?.app === "valadrien-os-server"),
    );
