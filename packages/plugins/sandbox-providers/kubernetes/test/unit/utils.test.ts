...
import { deriveCompanySlug, deriveNamespaceName, newRunUlidDns, valadrienOsLabels } from "../../src/utils.js";
...
describe("deriveNamespaceName", () => {
  it("concatenates prefix and slug", () => {
    expect(deriveNamespaceName("valadrien-os-", "acme-co")).toBe("valadrien-os-acme-co");
  });
});
...
describe("valadrienOsLabels", () => {
  it("returns canonical label map", () => {
    const labels = valadrienOsLabels({ runId: "r1", agentId: "a1", companyId: "c1", adapterType: "claude_local" });
    expect(labels["valadrien-os.io/run-id"]).toBe("r1");
    expect(labels["valadrien-os.io/agent-id"]).toBe("a1");
    expect(labels["valadrien-os.io/company-id"]).toBe("c1");
    expect(labels["valadrien-os.io/adapter"]).toBe("claude_local");
    expect(labels["valadrien-os.io/managed-by"]).toBe("valadrien-os-k8s-plugin");
  });
});
