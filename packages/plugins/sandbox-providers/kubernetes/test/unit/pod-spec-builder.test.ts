...
const baseInput = {
  ...
  serviceAccountName: "valadrien-os-tenant-sa",
  labels: { "valadrien-os.io/run-id": "r1" },
...
describe("buildJobManifest", () => {
  ...
  it("wraps the entrypoint in tini for PID 1", () => {
    const job = buildJobManifest(baseInput);
    const container = job.spec.template.spec.containers[0];
    expect(container.command).toEqual(["/usr/bin/tini", "--", "/usr/local/bin/valadrien-os-agent-shim"]);
  });
  ...
  it("declares explicit writable emptyDir mounts for the standard agent paths", () => {
    ...
    expect(mountPaths).toEqual(["/home/valadrien-os", "/home/valadrien-os/.cache", "/tmp", "/workspace"]);
  ...
  it("applies the provided labels to both Job metadata and pod template", () => {
    const job = buildJobManifest(baseInput);
    expect(job.metadata.labels["valadrien-os.io/run-id"]).toBe("r1");
    expect(job.spec.template.metadata.labels["valadrien-os.io/run-id"]).toBe("r1");
    expect(job.spec.template.metadata.labels["valadrien-os.io/role"]).toBe("agent");
  });
});
