...
// ValadrienOs-server does NOT push to agent pods — the agent shim makes
// outbound calls to valadrien-os-server via the egress allow-list (port 3100).
...
// add a targeted ingress rule here scoped to the valadrien-os-server pod
// selector.
export function buildNetworkPolicyManifests(input: BuildNetworkPolicyInput): Record<string, unknown>[] {
  ...
  const egressAllow: Record<string, unknown> = {
    ...
    kind: "NetworkPolicy",
    metadata: {
      name: "valadrien-os-egress-allow",
      namespace: input.namespace,
      labels: { "valadrien-os.io/managed-by": "valadrien-os-k8s-plugin" },
    },
    spec: {
      podSelector: { matchLabels: { "valadrien-os.io/role": "agent" } },
      policyTypes: ["Egress"],
      egress: [
        {
          to: [
        ...
        },
        {
          to: [
            {
              namespaceSelector: { matchLabels: { "kubernetes.io/metadata.name": input.valadrienOsServerNamespace } },
              podSelector: { matchLabels: { app: "valadrien-os-server" } },
            },
          ],
