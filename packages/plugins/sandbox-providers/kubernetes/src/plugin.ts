...
// The namespace valadrien-os-server itself runs in. Used when building
// NetworkPolicy manifests so the tenant namespace allows inbound traffic
// from the server pod.
const VALADRIEN_OS_SERVER_NAMESPACE = "valadrien-os";

// Name of the ServiceAccount created inside each tenant namespace by ensureTenant.
const TENANT_SERVICE_ACCOUNT = "valadrien-os-tenant-sa";
...
function generateBootstrapToken(): string {
  ...
  // callback auth scheme; valadrien-os-server's callback auth is out of scope for
...
const plugin = definePlugin({
  ...
  async onEnvironmentAcquireLease(
    ...
    // it in valadrien-os-server logs even if they missed the validation step.
    ...
    await ensureTenant(clients, {
      ...
      valadrienOsServerNamespace: VALADRIEN_OS_SERVER_NAMESPACE,
    ...
    const labels = valadrienOsLabels({
      runId: params.runId,
  ...
  async onEnvironmentExecute(
    ...
    if (leaseBackend === "sandbox-cr") {
      ...
      // The container entrypoint is baked into the Job spec (Tini + valadrien-os-agent-shim).
