...
const PLUGIN_ID = "valadrien-os.kubernetes-sandbox-provider";
...
const manifest: ValadrienOsPluginManifestV1 = {
  ...
  description:
    "Built on kubernetes-sigs/agent-sandbox (v1alpha1). ALPHA — expect breaking changes as the upstream CRD evolves. Falls back to stable batch/v1 Job mode for clusters without agent-sandbox installed. First-party ValadrienOs sandbox-provider plugin for Kubernetes.",
  author: "ValadrienOs",
      ...
      configSchema: {
        ...
        properties: {
          inCluster: {
            type: "boolean",
            description:
              "When true, the plugin uses the in-pod ServiceAccount credentials. Requires valadrien-os-server to be running inside the target cluster.",
          },
          kubeconfig: {
            type: "string",
            format: "secret-ref",
            description:
              "Inline kubeconfig YAML. Paste a kubeconfig or an existing ValadrienOs secret reference; pasted values are stored as company secrets.",
          },
          namespacePrefix: {
            type: "string",
            description: "Prefix for the per-company tenant namespace (default: valadrien-os-).",
          },
          companySlug: {
            type: "string",
