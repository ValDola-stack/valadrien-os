...
describe("Novita sandbox provider plugin", () => {
  ...
  it("parses defaults", () => {
    expect(parseNovitaDriverConfig({})).toMatchObject({
      ...
      domain: null,
      template: null,
      requestedCwd: "/home/user/valadrien-os-workspace",
      timeoutMs: 300_000,
  ...
  it("builds a quoted shell command with cwd, env, args, and stdin", () => {
    ...
    expect(command).toContain("printf '%s' 'input body' > '/tmp/.valadrien-os-stdin-");
    expect(command).toMatch(/< '\/tmp\/\.valadrien-os-stdin-[^']+'/);
    expect(command).toMatch(/rm -f '\/tmp\/\.valadrien-os-stdin-[^']+'/);
  ...
  it("uses a unique stdin path for each command", () => {
    ...
    const firstPath = first.match(/\/tmp\/\.valadrien-os-stdin-[^']+/)?.[0];
    const secondPath = second.match(/\/tmp\/\.valadrien-os-stdin-[^']+/)?.[0];

    expect(firstPath).toBeTruthy();
    expect(secondPath).toBeTruthy();
    expect(firstPath).not.toBe(secondPath);
    expect(first).not.toContain("/tmp/.valadrien-os-stdin <<");
    expect(second).not.toContain("/tmp/.valadrien-os-stdin <<");
  });
