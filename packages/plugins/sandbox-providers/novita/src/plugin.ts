...
export function parseNovitaDriverConfig(raw: Record<string, unknown>): NovitaDriverConfig {
  return {
    ...
    requestedCwd: parseOptionalString(raw.requestedCwd) ?? "/home/user/valadrien-os-workspace",
...
function buildStdinPath(): string {
  return `/tmp/.valadrien-os-stdin-${randomUUID()}`;
}
...
async function createSandbox(params: PluginEnvironmentAcquireLeaseParams | PluginEnvironmentProbeParams, config: NovitaDriverConfig) {
  const metadata = {
    "valadrien-os-provider": "novita",
    "valadrien-os-company-id": params.companyId,
    "valadrien-os-environment-id": params.environmentId,
    ...(params.issueId ? { "valadrien-os-issue-id": params.issueId } : {}),
    ...("runId" in params ? { "valadrien-os-run-id": params.runId } : {}),
  };
  const opts = {
    ...sandboxOpts(config),
    metadata,
