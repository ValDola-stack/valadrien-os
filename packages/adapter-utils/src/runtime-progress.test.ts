...
describe("createRuntimeProgressReporter", () => {
  it("formats the message with phase, label, direction, target, percent and MB", async () => {
    ...
    expect(lines).toEqual(["[valadrien-os] Syncing workspace to sandbox: 40% (12.6/31.4 MB)\n"]);
  });
  ...
  it("omits the label when none is provided (e.g. git history)", async () => {
    ...
    expect(lines).toEqual(["[valadrien-os] Importing git history to ssh: 100% (4.0/4.0 MB)\n"]);
  });
  ...
  it("suppresses intermediate emits that neither cross a step nor exceed the interval", async () => {
    ...
    expect(lines[0]).toBe("[valadrien-os] Syncing workspace to sandbox: 1% (1.0/100.0 MB)\n");
  });
  ...
  it("emits when the percentage crosses a 10% step", async () => {
    ...
    expect(lines[1]).toBe("[valadrien-os] Syncing workspace to sandbox: 15% (15.0/100.0 MB)\n");
  });
  ...
  it("emits on the time threshold even without a step crossing", async () => {
    ...
    expect(lines).toHaveLength(2);
    expect(lines[1]).toBe("[valadrien-os] Syncing workspace to sandbox: 3% (3.0/100.0 MB)\n");
  });
  ...
  it("always emits the terminal 100% line via report reaching the total", async () => {
    ...
    expect(lines[lines.length - 1]).toBe(
      "[valadrien-os] Restoring workspace from sandbox: 100% (100.0/100.0 MB)\n",
    );
  });
  ...
  it("complete() emits the terminal 100% line even when intermediate emits were throttled", async () => {
    ...
    expect(lines[lines.length - 1]).toBe(
      "[valadrien-os] Syncing workspace to sandbox: 100% (100.0/100.0 MB)\n",
    );
  });
  ...
  it("reports MB-only (no percent) when the total is unknown, plus a completion line", async () => {
    ...
    expect(lines).toEqual([
      "[valadrien-os] Restoring workspace from ssh: 2.0 MB\n",
      "[valadrien-os] Restoring workspace from ssh: 8.0 MB\n",
      "[valadrien-os] Restoring workspace from ssh: 8.0 MB\n",
    ]);
  });
  ...
  it("fail() emits a terminal failure marker with the last percent instead of a dangling line", async () => {
    ...
    expect(lines).toEqual([
      "[valadrien-os] Syncing workspace to ssh: 40% (40.0/100.0 MB)\n",
      "[valadrien-os] Syncing workspace to ssh: failed at 40% (40.0/100.0 MB)\n",
    ]);
  });
  ...
  it("fail() falls back to an MB marker when the total is unknown", async () => {
    ...
    expect(lines.at(-1)).toBe("[valadrien-os] Restoring from ssh: failed after 3.0 MB\n");
  });
