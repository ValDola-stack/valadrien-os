...
describe("adapter skill snapshots", () => {
  ...
  it("reports source-missing company runtime skills without orphan warnings", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      availableEntries: [{
        ...
        source: "/valadrien-os/skills/example/__runtime__/reflection-coach--abc123",
  ...
  it("can surface read-only external skills for runtime-mounted adapters", () => {
    const snapshot = buildRuntimeMountedSkillSnapshot({
      ...
      externalDetail: "Installed outside ValadrienOs management in the Claude skills home.",
    });
  ...
  it("reports persistent adapter installed, stale, external, and missing states", () => {
    const snapshot = buildPersistentSkillSnapshot({
      ...
      desiredSkills: [requiredEntry.key, "missing-skill"],
      installed: new Map([
        ["valadrien-os", { targetPath: "/runtime/valadrien-os", kind: "symlink" }],
  ...
  it("reports stale managed persistent skills when ValadrienOs owns an undesired available skill", () => {
    const snapshot = buildPersistentSkillSnapshot({
      adapterType: "cursor",
...
describe("renderValadrienOsWakePrompt", () => {
  ...
  it("renders the execution workspace branch guard only on non-resumed sessions", () => {
    ...
    const firstPrompt = renderValadrienOsWakePrompt(payload);
    ...
    const resumedPrompt = renderValadrienOsWakePrompt(payload, { resumedSession: true });
    expect(resumedPrompt).toContain("## ValadrienOs Resume Delta");
...
describe("renderValadrienOsWakePrompt - task watchdog", () => {
  ...
  it("survives a JSON round-trip through stringifyValadrienOsWakePayload", () => {
    const payload = {
      ...baseWatchdogPayload,
      taskWatchdog: {
        watchedIssueId: "watched-issue-1",
    ...
    const prompt = renderValadrienOsWakePrompt(parsed);
    expect(prompt).toContain("## Task Watchdog Mandate");
  ...
  it("truncates oversized custom instructions and caps terminal leaf summaries", () => {
    ...
    const serialized = stringifyValadrienOsWakePayload({
      ...baseWatchdogPayload,
      taskWatchdog: {
        watchedIssueId: "watched-issue-1",
        watchedIssueIdentifier: "PAP-8000",
