...
export const DEFAULT_VALADRIEN_OS_AGENT_PROMPT_TEMPLATE = [
  ...
  "- Use `VALADRIEN_OS_SCRATCH_DIR` / `VALADRIEN_OS_RUN_SCRATCH_DIR` for temporary scratch files instead of ad hoc `/tmp` paths; ValadrienOs removes that run-owned directory after the run ends.",
...
export const WATCHDOG_DEFAULT_MANDATE = [
  ...
  "- If you discover a ValadrienOs product or platform bug while reviewing the stopped subtree, create a linked engineering follow-up outside the watched source tree using the server-provided watchdog discovery route instead of making it a source child.",
...
type ValadrienOsWakeTaskWatchdogLeaf = {
  id: string | null;
...
type ValadrienOsWakeTaskWatchdogCapabilities = {
  operations: string[];
...
export type ValadrienOsWakeTaskWatchdogContext = {
  watchedIssueId: string | null;
  watchedIssueIdentifier: string | null;
  watchedIssueTitle: string | null;
  stopFingerprint: string | null;
  terminalLeafSummaries: ValadrienOsWakeTaskWatchdogLeaf[];
  customInstructions: string | null;
  capabilities: ValadrienOsWakeTaskWatchdogCapabilities | null;
};
...
export interface ValadrienOsDesiredSkillEntry {
  key: string;
  versionId: string | null;
}
...
interface RuntimeMountedSkillSnapshotOptions {
  ...
  availableEntries: ValadrienOsSkillEntry[];
  desiredSkills: string[];
  configuredDetail: string | ((entry: ValadrienOsSkillEntry) => string | null);
  missingDetail?: string;
  mode?: "ephemeral" | "unsupported";
  supported?: boolean;
  unsupportedDetail?: string | ((entry: ValadrienOsSkillEntry) => string | null);
  warnings?: string[];
...
function isValadrienOsSkillSourceMissing(entry: ValadrienOsSkillEntry) {
  return entry.sourceStatus === "missing";
}

function resolveValadrienOsSkillMissingDetail(
  entry: ValadrienOsSkillEntry,
  fallback: string,
) {
...
function resolveSkillDetail(
  detail: string | ((entry: ValadrienOsSkillEntry) => string | null) | null | undefined,
  entry: ValadrienOsSkillEntry,
): string | null {
...
type ValadrienOsWakePlanReviewAuthor = {
  type: string | null;
  id: string | null;
};

type ValadrienOsWakeAnnotationDelta = {
  id: string | null;
  issueId: string | null;
  ...
  bodyTruncated: boolean;
  createdAt: string | null;
  author: ValadrienOsWakePlanReviewAuthor | null;
};

type ValadrienOsWakePlanReviewComment = {
  id: string | null;
  threadId: string | null;
  body: string;
  bodyTruncated: boolean;
...
type ValadrienOsWakePlanReviewThread = {
  id: string | null;
  documentKey: string | null;
  ...
  author: ValadrienOsWakePlanReviewAuthor | null;
  commentCount: number;
  comments: ValadrienOsWakePlanReviewComment[];
  commentsTruncated: boolean;
  createdAt: string | null;
  updatedAt: string | null;
};

type ValadrienOsWakePlanReviewInteractionTarget = {
  issueId: string | null;
  documentId: string | null;
  key: string | null;
  revisionId: string | null;
...
type ValadrienOsWakePlanReviewInteractionResult = {
  outcome: string | null;
  reason: string | null;
  commentId: string | null;
};

type ValadrienOsWakePlanReviewInteraction = {
  id: string | null;
  kind: string | null;
  status: string | null;
  ...
  target: ValadrienOsWakePlanReviewInteractionTarget | null;
  acceptedTargetRevision: ValadrienOsWakePlanReviewInteractionTarget | null;
  result: ValadrienOsWakePlanReviewInteractionResult | null;
  resolvedAt: string | null;
};

type ValadrienOsWakePlanReviewContext = {
  documentKey: string | null;
  issueId: string | null;
  latestRevisionId: string | null;
  latestRevisionNumber: number | null;
  threads: ValadrienOsWakePlanReviewThread[];
  interaction: ValadrienOsWakePlanReviewInteraction | null;
  totals: {
...
type ValadrienOsWakeCheckboxSelection = {
  prompt: string | null;
...
type ValadrienOsWakeExecutionWorkspace = {
...
type ValadrienOsWakePayload = {
  ...
  planReviewContext: ValadrienOsWakePlanReviewContext | null;
  livenessContinuation: ValadrienOsWakeLivenessContinuation | null;
  taskWatchdog: ValadrienOsWakeTaskWatchdogContext | null;
  interactionKind: string | null;
  interactionStatus: string | null;
  checkboxSelection: ValadrienOsWakeCheckboxSelection | null;
  executionWorkspace: ValadrienOsWakeExecutionWorkspace | null;
  annotationDeltas: ValadrienOsWakeAnnotationDelta[];
...
function normalizeValadrienOsWakePlanReviewAuthor(value: unknown): ValadrienOsWakePlanReviewAuthor | null {
...
function normalizeValadrienOsWakeAnnotationDelta(value: unknown): ValadrienOsWakeAnnotationDelta | null {
  ...
  const author = normalizeValadrienOsWakePlanReviewAuthor(delta.author);
...
function normalizeValadrienOsWakePlanReviewComment(value: unknown): ValadrienOsWakePlanReviewComment | null {
  const comment = parseObject(value);
  const id = asString(comment.id, "").trim() || null;
  const threadId = asString(comment.threadId, "").trim() || null;
  const body = asString(comment.body, "");
  const author = normalizeValadrienOsWakePlanReviewAuthor(comment.author);
...
function normalizeValadrienOsWakePlanReviewThread(value: unknown): ValadrienOsWakePlanReviewThread | null {
  const thread = parseObject(value);
  const comments = Array.isArray(thread.comments)
    ? thread.comments
        .map((entry) => normalizeValadrienOsWakePlanReviewComment(entry))
        .filter((entry): entry is ValadrienOsWakePlanReviewComment => Boolean(entry))
    : [];
  ...
  const author = normalizeValadrienOsWakePlanReviewAuthor(thread.author);
...
function normalizeValadrienOsWakePlanReviewInteractionTarget(
  value: unknown,
): ValadrienOsWakePlanReviewInteractionTarget | null {
...
function normalizeValadrienOsWakePlanReviewInteractionResult(
  value: unknown,
): ValadrienOsWakePlanReviewInteractionResult | null {
...
function normalizeValadrienOsWakePlanReviewInteraction(value: unknown): ValadrienOsWakePlanReviewInteraction | null {
  ...
  const target = normalizeValadrienOsWakePlanReviewInteractionTarget(interaction.target);
  const acceptedTargetRevision = normalizeValadrienOsWakePlanReviewInteractionTarget(interaction.acceptedTargetRevision);
  const result = normalizeValadrienOsWakePlanReviewInteractionResult(interaction.result);
...
function normalizeValadrienOsWakePlanReviewContext(value: unknown): ValadrienOsWakePlanReviewContext | null {
  const context = parseObject(value);
  const threads = Array.isArray(context.threads)
    ? context.threads
        .map((entry) => normalizeValadrienOsWakePlanReviewThread(entry))
        .filter((entry): entry is ValadrienOsWakePlanReviewThread => Boolean(entry))
    : [];
  const interaction = normalizeValadrienOsWakePlanReviewInteraction(context.interaction);
...
function normalizeValadrienOsWakeCheckboxSelection(value: unknown): ValadrienOsWakeCheckboxSelection | null {
...
function normalizeValadrienOsWakeTaskWatchdogLeaf(value: unknown): ValadrienOsWakeTaskWatchdogLeaf | null {
...
function normalizeValadrienOsWakeTaskWatchdogCapabilities(value: unknown): ValadrienOsWakeTaskWatchdogCapabilities | null {
...
function normalizeValadrienOsWakeTaskWatchdog(value: unknown): ValadrienOsWakeTaskWatchdogContext | null {
  ...
  const terminalLeafSummaries = Array.isArray(watchdog.terminalLeafSummaries)
    ? watchdog.terminalLeafSummaries
        ...
        .map((entry) => normalizeValadrienOsWakeTaskWatchdogLeaf(entry))
        .filter((entry): entry is ValadrienOsWakeTaskWatchdogLeaf => Boolean(entry))
    : [];
  const capabilities = normalizeValadrienOsWakeTaskWatchdogCapabilities(watchdog.capabilities);
...
function normalizeValadrienOsWakeExecutionWorkspace(value: unknown): ValadrienOsWakeExecutionWorkspace | null {
...
export function normalizeValadrienOsWakePayload(value: unknown): ValadrienOsWakePayload | null {
  ...
  const planReviewContext = normalizeValadrienOsWakePlanReviewContext(payload.planReviewContext);
  const annotationDeltas = Array.isArray(payload.annotationDeltas)
    ? payload.annotationDeltas
        .map((entry) => normalizeValadrienOsWakeAnnotationDelta(entry))
        .filter((entry): entry is ValadrienOsWakeAnnotationDelta => Boolean(entry))
    : [];
  const livenessContinuation = normalizeValadrienOsWakeLivenessContinuation(payload.livenessContinuation);
  const taskWatchdog = normalizeValadrienOsWakeTaskWatchdog(payload.taskWatchdog);
  ...
  const checkboxSelection = normalizeValadrienOsWakeCheckboxSelection(payload.checkboxSelection);
  const executionWorkspace = normalizeValadrienOsWakeExecutionWorkspace(payload.executionWorkspace);
  if (comments.length === 0 && commentIds.length === 0 && annotationDeltas.length === 0 && childIssueSummaries.length === 0 && unresolvedBlockerIssueIds.length === 0 && unresolvedBlockerSummaries.length === 0 && !activeTreeHold && !executionStage && !continuationSummary && !planReviewContext && !livenessContinuation && !taskWatchdog && !checkboxSelection && !executionWorkspace && !normalizeValadrienOsWakeIssue(payload.issue)) {
    return null;
  }
...
export function renderValadrienOsWakePrompt(
  ...
  const planReviewTargetLabel = (target: ValadrienOsWakePlanReviewInteractionTarget | null) => {
  ...
  const planReviewAuthorLabel = (author: ValadrienOsWakePlanReviewAuthor | null) => {
...
export function buildRuntimeMountedSkillSnapshot(
  ...
  const {
    ...
    desiredSkills,
    configuredDetail,
    missingDetail = "ValadrienOs cannot find this skill in the local runtime skills directory.",
    mode = "ephemeral",
    externalInstalled,
  ...
  for (const available of availableEntries) {
    ...
    if (isValadrienOsSkillSourceMissing(available)) {
      entries.push({
        key: available.key,
        ...
        sourcePath: null,
        targetPath: null,
        detail: resolveValadrienOsSkillMissingDetail(available, missingDetail),
        ...
        ? configured
          ...
          : resolveSkillDetail(
              options.unsupportedDetail
                ?? "Desired state is stored in ValadrienOs only; this adapter cannot apply skills at runtime.",
              available,
            )
        : null,
      ...buildManagedSkillOrigin(),
  ...
  for (const available of availableEntries) {
    ...
    if (isValadrienOsSkillSourceMissing(available)) {
      entries.push({
        ...
        detail: resolveValadrienOsSkillMissingDetail(
          available,
          missingDetail,
        ),
        ...buildManagedSkillOrigin(),
...
export function readValadrienOsSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
  desiredSkillEntries: ValadrienOsDesiredSkillEntry[];
} {
  ...
  const desired = Array.isArray(desiredValues)
    ? desiredValues.flatMap((value): ValadrienOsDesiredSkillEntry[] => {
    ...
    : [];
  const byKey = new Map<string, ValadrienOsDesiredSkillEntry>();
...
export function writeValadrienOsSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: Array<string | ValadrienOsDesiredSkillEntry>,
  ...
  const current =
    typeof raw === "object" && raw !== null && !Array.isArray(raw)
      ...
      : {};
  const entries = desiredSkills.flatMap((value): ValadrienOsDesiredSkillEntry[] => {
    if (typeof value === "string") {
      const key = value.trim();
      return key ? [{ key, versionId: null }] : [];
    }
