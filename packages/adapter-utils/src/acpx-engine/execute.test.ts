...
import { DEFAULT_REMOTE_SANDBOX_ADAPTER_TIMEOUT_SEC } from "@valadrien-os/adapter-utils/execution-target";
import {
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-acpx-skills-"));
  tempRoots.push(root);
  return root;
}
...
describe("shared ACPX engine runtime behavior", () => {
  it("includes ValadrienOs env and API access notes in the ACPX prompt without leaking the token", async () => {
    ...
    expect(prompt).toContain("ValadrienOs runtime note:");
    expect(prompt).toContain("VALADRIEN_OS_AGENT_ID");
    expect(prompt).toContain("VALADRIEN_OS_API_KEY");
    expect(prompt).toContain("VALADRIEN_OS_WAKE_PAYLOAD_JSON");
    expect(prompt).toContain("ValadrienOs API access note:");
    expect(prompt).toContain('VALADRIEN_OS_API_BASE="${VALADRIEN_OS_API_URL%/}"; VALADRIEN_OS_API_BASE="${VALADRIEN_OS_API_BASE%/api}"');
    expect(prompt).toContain("$VALADRIEN_OS_API_BASE/api/agents/me");
    expect(prompt).toContain("$VALADRIEN_OS_API_BASE/api/issues/$VALADRIEN_OS_TASK_ID");
    expect(prompt).toContain("X-ValadrienOs-Run-Id");
    expect(prompt).not.toContain("$VALADRIEN_OS_API_URL/api/");
  ...
  it("does not show a scoped issue API command when the task id is unavailable", async () => {
    ...
    expect(prompt).not.toContain("$VALADRIEN_OS_API_BASE/api/issues/$VALADRIEN_OS_TASK_ID");
  });
  ...
  it.skipIf(process.platform === "win32")("materializes ACPX Claude skills without symlinked descendants", async () => {
    ...
    const { meta } = await runExecutor({
      agent: "claude",
      stateDir,
      valadrienOsRuntimeSkills: [skill],
      valadrienOsSkillSync: { desiredSkills: [skill.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("revokes removed ACPX Codex skills and skips symlinked descendants", async () => {
    ...
    const baseConfig = {
      ...
      valadrienOsRuntimeSkills: [keep, remove],
    };

    await runExecutor({
      ...baseConfig,
      valadrienOsSkillSync: { desiredSkills: [keep.key, remove.key] },
    });
    expect(await pathExists(path.join(codexHome, "skills", remove.runtimeName, "SKILL.md"))).toBe(true);

    await runExecutor({
      ...baseConfig,
      valadrienOsSkillSync: { desiredSkills: [keep.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("removes legacy ACPX Codex skill symlinks when a skill is no longer desired", async () => {
    ...
    await runExecutor({
      ...
      stateDir: path.join(root, "state"),
      env: { CODEX_HOME: codexHome },
      valadrienOsRuntimeSkills: [legacy],
      valadrienOsSkillSync: { desiredSkills: [] },
    });
  ...
  it.skipIf(process.platform === "win32")("replaces stale managed Codex auth files with source symlinks", async () => {
    ...
    const valadrienOsInstanceId = "test-instance";
    const managedCodexHome = path.join(
      valadrienOsHome,
      "instances",
      valadrienOsInstanceId,
      "companies",
      "company-1",
      "codex-home",
    );
    ...
    try {
      ...
      process.env.VALADRIEN_OS_INSTANCE_ID = valadrienOsInstanceId;
      await runExecutor({
        agent: "codex",
        stateDir: path.join(root, "state"),
  ...
  it("keeps fresh credential wrapper scripts across ACPX agent changes", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      agent: "custom-a",
      env: { VALADRIEN_OS_API_KEY: "old-key" },
    });
    await runExecutor({
      ...baseConfig,
      agent: "custom-b",
      env: { VALADRIEN_OS_API_KEY: "new-key" },
    });
    ...
    expect(wrapper).not.toContain("VALADRIEN_OS_API_KEY");
    expect(wrapper).not.toContain("new-key");
    expect(wrapper).not.toContain("old-key");
    expect(env).toContain("VALADRIEN_OS_API_KEY='new-key'");
  ...
  it("shapes ACPX wrapper workspace env for remote execution identities", async () => {
    ...
    expect(env).toContain("VALADRIEN_OS_WORKSPACE_CWD='/remote/workspace'");
    expect(env).not.toContain("VALADRIEN_OS_WORKSPACE_WORKTREE_PATH=");
  });
  ...
  it("keeps distinct wrapper env files for concurrent runs with different credentials", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      env: { VALADRIEN_OS_API_KEY: "first-key" },
    });
    await runExecutor({
      ...baseConfig,
      env: { VALADRIEN_OS_API_KEY: "second-key" },
    });
    ...
    expect(envFiles.filter((contents) => contents.includes("VALADRIEN_OS_API_KEY='first-key'"))).toHaveLength(1);
    expect(envFiles.filter((contents) => contents.includes("VALADRIEN_OS_API_KEY='second-key'"))).toHaveLength(1);
  });
  ...
  it("writes wrapper that redirects child stderr to a per-run log file", async () => {
    ...
    expect(wrapper).toContain("VALADRIEN_OS_RUN_ID");
  ...
  it("starts sandbox ACP process sessions in the remote execution cwd", async () => {
    ...
    const runner = createLocalSandboxRunner(
      (input: { args?: string[]; env?: Record<string, string> }) => {
        if (input.env?.VALADRIEN_OS_SANDBOX_EXEC_CHANNEL === "bridge") {
          const script = input.args?.[1] ?? "";
          const match = script.match(/VALADRIEN_OS_PROCESS_SESSION_COMMAND_B64='([^']+)'/);
          if (match) {
    ...
    expect(String(payloadEnv.VALADRIEN_OS_API_URL ?? "")).toMatch(
      /^http:\/\/127\.0\.0\.1:\d+$/,
    );
    expect(payloadEnv.VALADRIEN_OS_API_KEY).toBeTruthy();
    expect(payloadEnv.VALADRIEN_OS_API_KEY).not.toBe("real-run-jwt");
  });
  ...
  it.skipIf(process.platform === "win32")("drops benign ACP nes/close cleanup stderr but keeps it in the run log", async () => {
    ...
    const { stderr } = await execFileAsync("bash", [wrapperPath], {
      env: { ...process.env, VALADRIEN_OS_RUN_ID: "run-nes-close-1" },
    });
  ...
  it("passes ValadrienOs env through the ACP agent wrapper instead of process.env", async () => {
      ...
      createRuntime: () => ({
        ...
        startTurn: () => ({
          events: (async function* () {
            ...
            observedApiKeyDuringStream = process.env.VALADRIEN_OS_API_KEY;
            yield { type: "done", stopReason: "end_turn" };
          })(),
          result: Promise.resolve({ status: "completed", stopReason: "end_turn" }),
          cancel: async () => {},
    ...
    const previousApiKey = process.env.VALADRIEN_OS_API_KEY;
    try {
      delete process.env.VALADRIEN_OS_API_KEY;
      const result = await execute({
        runId: "run-1",
        agent: {
          id: "agent-1",
      ...
      if (previousApiKey === undefined) delete process.env.VALADRIEN_OS_API_KEY;
      else process.env.VALADRIEN_OS_API_KEY = previousApiKey;
    }
  });

  it("writes a ValadrienOs-managed .claude/settings.local.json for the claude agent so it can reach the ValadrienOs API", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    ...
    const { meta } = await runExecutor(
      ...
      { context: { valadrienOsWorkspace: { cwd, agentHome: path.join(root, "agent-home") } } },
    );
    ...
    expect(allow).toContain(`Bash(${cwd}/scripts/valadrien-os-issue-update.sh:*)`);
    ...
    const note = (meta[0]?.commandNotes as string[] | undefined)?.find((entry) =>
      entry.includes("ValadrienOs-managed Claude settings"),
    );
    expect(note).toBeTruthy();
  });

  it("merges ValadrienOs allowlist into an existing .claude/settings.local.json without losing user entries", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const cwd = path.join(root, "worktree");
    ...
    await runExecutor(
      { agent: "claude", stateDir, cwd },
      { context: { valadrienOsWorkspace: { cwd } } },
    );
...
describe("shared ACP engine execution timeouts", () => {
  it("applies the 4h sandbox backstop when timeoutSec is unset on a sandbox execution target", async () => {
    ...
    expect(startLine!.text).toContain(
      `[valadrien-os] Adapter execution timeout: timeoutSec=${DEFAULT_REMOTE_SANDBOX_ADAPTER_TIMEOUT_SEC} ` +
  ...
  it("reports a self-describing timeout error when the wall-clock timer kills a turn", async () => {
    ...
    const execute = createAcpxEngineExecutor({
      createRuntime: () => ({
        ...
        startTurn: () => ({
          // Never yields on its own: only the ValadrienOs wall-clock timer's
