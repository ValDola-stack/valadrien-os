...
import {
  ...
  type AdapterExecutionTargetValadrienOsBridgeHandle,
...
interface AcpxPreparedRuntime {
  ...
  valadrienOsBridge: AdapterExecutionTargetValadrienOsBridgeHandle | null;
...
function defaultStateDir(companyId: string, agentId: string): string {
  return path.join(defaultValadrienOsInstanceDir(), "companies", companyId, "acp-engine", "agents", agentId);
}
...
async function resolveSelectedRuntimeSkills(
  ...
  const allSkills = await readValadrienOsRuntimeSkillEntries(config, moduleDir);
...
async function prepareGeminiSkillRuntime(input: {
  ...
  for (const skillName of removedSkills) {
    await input.onLog("stdout", `[valadrien-os] Removed maintainer-only ACPX Gemini skill "${skillName}" from ${skillsHome}\n`);
  }
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result === "created" || result === "repaired") {
        await input.onLog(
          "stdout",
          `[valadrien-os] ${result === "repaired" ? "Repaired" : "Linked"} ACPX Gemini skill "${entry.runtimeName}" into ${skillsHome}\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[valadrien-os] Failed to link ACPX Gemini skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
async function writeAgentWrapper(input: {
  ...
  const script = [
    ...
    // ACP nes/close cleanup RPC error to ValadrienOs's live stderr stream.
    "  exec 2> >(tee -a \"$stderr_dir/$VALADRIEN_OS_RUN_ID.log\" | grep -Ev \"method: ['\\\"]nes/close['\\\"].*-32601\" >&2 || true)",
    "fi",
...
async function buildRuntime(input: {
  ...
  const secretsContext = parseObject(context.valadrienOsSecrets);
  ...
  if (acpxAgent === "claude") {
    ...
    const desired = resolveValadrienOsDesiredSkillNames(
      config,
      await readValadrienOsRuntimeSkillEntries(config, input.engine.moduleDir),
    );
  ...
  let valadrienOsBridge: AdapterExecutionTargetValadrienOsBridgeHandle | null = null;
  if (
    ...
    if (valadrienOsBridge) {
      ...
      await input.ctx.onLog("stdout", "[valadrien-os] Sandbox ACP API callback bridge enabled for this run.\n");
    }
  }
  ...
  try {
    ...
    await valadrienOsBridge?.stop().catch(() => {});
    throw err;
  }
...
async function cleanupRemoteBridges(prepared: AcpxPreparedRuntime): Promise<void> {
  await Promise.allSettled([
    ...
    prepared.valadrienOsBridge?.stop(),
  ]);
}

function renderValadrienOsEnvNote(env: Record<string, string>): string {
  const valadrienOsKeys = Object.keys(env)
    .filter((key) => key.startsWith("VALADRIEN_OS_"))
    .sort();
  if (valadrienOsKeys.length === 0) return "";
  return [
    "ValadrienOs runtime note:",
    `The following VALADRIEN_OS_* environment variables are available in this run: ${valadrienOsKeys.join(", ")}`,
    "Do not assume these variables are missing without checking your shell environment.",
  ].join("\n");
}

function renderApiAccessNote(env: Record<string, string>): string {
  if (!env.VALADRIEN_OS_API_URL || !env.VALADRIEN_OS_API_KEY) return "";
  const lines = [
    "ValadrienOs API access note:",
    "Use terminal commands with curl to make ValadrienOs API requests.",
    "Normalize the base URL before adding API paths:",
    `  VALADRIEN_OS_API_BASE="\${VALADRIEN_OS_API_URL%/}"; VALADRIEN_OS_API_BASE="\${VALADRIEN_OS_API_BASE%/api}"`,
    "GET example:",
    `  curl -s -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" "$VALADRIEN_OS_API_BASE/api/agents/me"`,
  ];
  if (env.VALADRIEN_OS_TASK_ID) {
    lines.push(
      "Scoped issue comment example:",
      `  curl -s -X POST -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" -H "Content-Type: application/json" -H "X-ValadrienOs-Run-Id: $VALADRIEN_OS_RUN_ID" -d '{"body":"Status update from agent."}' "$VALADRIEN_OS_API_BASE/api/issues/$VALADRIEN_OS_TASK_ID/comments"`,
    );
  } else {
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean, env: Record<string, string>): Promise<{
  ...
  const valadrienOsEnvNote = renderValadrienOsEnvNote(env);
  ...
  return {
    ...
    promptMetrics: {
      ...
      runtimeNoteChars: valadrienOsEnvNote.length + apiAccessNote.length,
...
export function createAcpxEngineExecutor(deps: AcpxEngineExecutorOptions = {}) {
  ...
  return async function executeAcpxEngine(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    await ctx.onLog(
      "stderr",
      `[valadrien-os] ${formatAdapterExecutionTimeoutStartLogLine(prepared.timeoutResolution)}\n`,
    );
