    ...
    sink: async (line) => {
      ...
      if (runtimeStatus?.sink) {
        await emitRuntimeStatus(
          runtimeStatus.sink,
          runtimeStatus.phase,
          line.replace(/^\[valadrien-os\]\s*/, "").trim(),
        );
      }
    },
    phase,
...
export async function prepareSandboxManagedRuntime(input: {
  ...
  const runtimeRootDir = path.posix.join(workspaceRemoteDir, ".valadrien-os-runtime", input.adapterKey);
  ...
  await withTempDir("valadrien-os-sandbox-sync-", async (tempDir) => {
        ...
        await input.client.run(
          `sh -c ${shellQuote(
            `mkdir -p ${shellQuote(workspaceRemoteDir)} && ` +
              `find ${shellQuote(workspaceRemoteDir)} -mindepth 1 -maxdepth 1 ${preserveFindArgs([".valadrien-os-runtime"])} -exec rm -rf -- {} + && ` +
  ...
  return {
    ...
    restoreWorkspace: async (onProgress?: RuntimeProgressSink) => {
      ...
      await withTempDir("valadrien-os-sandbox-restore-", async (tempDir) => {
