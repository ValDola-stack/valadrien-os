...
export async function withShallowGitWorkspaceClone<T>(
  ...
  const cloneDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-git-workspace-"));
  const tempRef = `refs/valadrien-os/git-sync/import/${randomUUID()}`;
  try {
...
export function createImportedGitRef(scope = "remote"): string {
  return `refs/valadrien-os/git-sync/imported/${scope}/${randomUUID()}`;
}

export function createRemoteGitExportRef(scope = "remote"): string {
  return `refs/valadrien-os/git-sync/export/${scope}/${randomUUID()}`;
}
  ...
  for (let attempt = 0; attempt < 5; attempt += 1) {
    ...
    const mergeCommit = await runLocalGit(
      ...
      [
        ...
        input.importedHead,
        "-m",
        `ValadrienOs remote git sync merge ${input.importedHead.slice(0, 12)}`,
      ],
      {
        timeout: 60_000,
        maxBuffer: 64 * 1024,
