...
describe("sandbox adapter execution targets", () => {
  ...
  function encodeTailTick(stdout: Buffer, stderr: Buffer): string {
    return [
      "__VALADRIEN_OS_RUN_LOG_STDOUT__",
      stdout.toString("base64"),
      "__VALADRIEN_OS_RUN_LOG_STDERR__",
      stderr.toString("base64"),
      "__VALADRIEN_OS_RUN_LOG_END__",
      "",
    ].join("\n");
  }
  ...
  it("bridges bidirectional sandbox process sessions through a local ACPX-spawnable proxy", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-process-session-"));
    ...
    const bridge = await startAdapterExecutionTargetProcessSessionBridge({
      ...
      target,
      runtimeRootDir: path.posix.join(rootDir, ".valadrien-os-runtime", "acpx"),
      adapterKey: "acpx",
  ...
  it("buffers sandbox process session output until the local proxy connects", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-process-session-buffer-"));
    cleanupDirs.push(rootDir);
  ...
  it("delivers full output when the sandbox child exits immediately after writing", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-process-session-fast-exit-"));
    cleanupDirs.push(rootDir);
  ...
  it("ignores unauthenticated connections to the process session bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-process-session-auth-"));
    cleanupDirs.push(rootDir);
  ...
  it("streams sandbox process session output before the remote child exits", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-process-session-stream-"));
    cleanupDirs.push(rootDir);
  ...
  it("treats SSH targets as bridge-only", () => {
    ...
    expect(adapterExecutionTargetUsesValadrienOsBridge(target)).toBe(true);
    expect(adapterExecutionTargetSessionIdentity(target)).toEqual({
      transport: "ssh",
      host: "ssh.example.test",
      port: 22,
  ...
  it("starts a localhost ValadrienOs bridge for sandbox targets in bridge mode", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-target-bridge-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".valadrien-os-runtime", "codex");
    ...
    const apiServer = createServer((req, res) => {
      requests.push({
        ...
        runId: typeof req.headers["x-valadrien-os-run-id"] === "string" ? req.headers["x-valadrien-os-run-id"] : null,
      });
    ...
    const bridge = await startAdapterExecutionTargetValadrienOsBridge({
      runId: "run-bridge",
      target,
      runtimeRootDir,
      adapterKey: "codex",
    ...
    try {
      expect(bridge).not.toBeNull();
      expect(bridge?.env.VALADRIEN_OS_API_URL).toMatch(/^http:\/\/127\.0\.0\.1:\d+$/);
      expect(bridge?.env.VALADRIEN_OS_API_KEY).not.toBe("real-run-jwt");
      expect(bridge?.env.VALADRIEN_OS_API_BRIDGE_MODE).toBe("queue_v1");

      const response = await fetch(`${bridge!.env.VALADRIEN_OS_API_URL}/api/agents/me`, {
        headers: {
          authorization: `Bearer ${bridge!.env.VALADRIEN_OS_API_KEY}`,
  ...
  it("creates a sandbox run log tail factory when bridge streaming is enabled", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-target-bridge-stream-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".valadrien-os-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("defaults sandbox run log streaming on and honors the explicit opt-out", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-target-bridge-stream-default-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".valadrien-os-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    const defaultBridge = await startAdapterExecutionTargetValadrienOsBridge({
    ...
    const optOutBridge = await startAdapterExecutionTargetValadrienOsBridge({
  ...
  it("tails sandbox run log chunks with byte offsets and dedupes the final batch", async () => {
    ...
    const tail = createSandboxRunLogTailFactory({
      runner,
      remoteCwd: "/workspace",
      logsDir: "/workspace/.valadrien-os-runtime/codex/valadrien-os-bridge/queue/logs",
      pollIntervalMs: 1,
    ...
    expect(runner.execute).toHaveBeenCalledWith(expect.objectContaining({
      command: "sh",
      cwd: "/workspace",
      env: { VALADRIEN_OS_SANDBOX_EXEC_CHANNEL: "bridge" },
      timeoutMs: 50,
    }));
  });
  ...
  it("delivers the final batch and a warning when run log polling degrades", async () => {
    ...
    expect(combinedStream(events, "stderr")).toBe(
      "final err\n[valadrien-os] Run log streaming degraded during the run; remaining output was delivered at completion.\n",
    );
  });

  it("exposes the ValadrienOs bridge to the sandbox shell surface", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-target-bridge-shell-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".valadrien-os-runtime", "claude");
    await mkdir(runtimeRootDir, { recursive: true });
    ...
    try {
      expect(bridge).not.toBeNull();
      const shellProbe = [
        "const url = `${process.env.VALADRIEN_OS_API_URL}/api/agents/me`;",
        "fetch(url, { headers: { authorization: `Bearer ${process.env.VALADRIEN_OS_API_KEY}`, accept: 'application/json' } })",
        "  .then(async (response) => {",
        "    const body = await response.json();",
        "    process.stdout.write(JSON.stringify({",
        "      status: response.status,",
        "      body,",
        "      bridgeMode: process.env.VALADRIEN_OS_API_BRIDGE_MODE,",
        "    }));",
        "  })",
        "  .catch((error) => {",
      ...
      expect(`${result.stdout}\n${result.stderr}`).not.toContain(bridge!.env.VALADRIEN_OS_API_KEY);
      ...
      expect(runnerCommandText).not.toContain(bridge!.env.VALADRIEN_OS_API_KEY);
      const runtimeFiles = (await readRuntimeTextFiles(runtimeRootDir)).join("\n");
      expect(runtimeFiles).not.toContain("real-run-jwt");
      expect(runtimeFiles).not.toContain(bridge!.env.VALADRIEN_OS_API_KEY);
      expect(requests).toEqual([{
        method: "GET",
        url: "/api/agents/me",
        auth: "Bearer real-run-jwt",
  ...
  it("uses the effective adapter timeout when starting the sandbox callback bridge", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-target-bridge-timeout-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".valadrien-os-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
  ...
  it("fails oversized host responses with a 502 before returning them to the sandbox client", async () => {
    const rootDir = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-target-bridge-limit-"));
    cleanupDirs.push(rootDir);
    const remoteCwd = path.join(rootDir, "workspace");
    const runtimeRootDir = path.join(remoteCwd, ".valadrien-os-runtime", "codex");
    await mkdir(runtimeRootDir, { recursive: true });
