 ...
 *
 * The ValadrienOs extension keywords below are recognised by the ValadrienOs UI
 * but are otherwise ignored by standard JSON Schema validators.
 */
export type JsonSchema = {
  /**
   * When true, the ValadrienOs config UI hides this property behind an
   * "Advanced options" disclosure. Defaults to false (always visible).
   */
  "x-valadrien-os-advanced"?: boolean;
  /**
   ...
   * `x-valadrien-os-advanced` is not true.
   */
  "x-valadrien-os-group"?: string;
  [key: string]: unknown;
};
...
// ---------------------------------------------------------------------------
// Manifest sub-types — nested declarations within ValadrienOsPluginManifestV1
// ---------------------------------------------------------------------------
...
export interface PluginEnvironmentDriverDeclaration {
  ...
  /**
   * How ValadrienOs should apply a captured template ref back into this provider's
...
/**
 * Declares a normal ValadrienOs agent that a plugin can provision and later
...
export interface PluginManagedAgentDeclaration {
  ...
  /** Suggested ValadrienOs runtime configuration. */
...
/**
 * Declares a normal ValadrienOs project that a plugin can provision and later
 * resolve by stable key within each company.
 */
...
/**
 * Lower-bound semver requirement for the ValadrienOs host.
 *
 ...
 */
export interface ValadrienOsPluginManifestV1 {
   ...
   */
  minimumValadrienOsVersion?: PluginMinimumHostVersion;
