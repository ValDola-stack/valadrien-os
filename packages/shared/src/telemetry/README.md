...
This document explains how contributors should use ValadrienOs's public telemetry
...
`packages/shared/src/telemetry/generated/valadrien-os-telemetry.ts`.
...
| --- | --- |
| First-party event names | `ValadrienOsEventName` in `generated/valadrien-os-telemetry.ts` |
| Per-event dimensions and optionality | `EventDimensionsMap` in `generated/valadrien-os-telemetry.ts` |
| Enum descriptions for telemetry dimensions | `VALADRIEN_OS_ENUM_DESCRIPTIONS` in `generated/valadrien-os-telemetry.ts` |
| Schema version and event envelope helpers | `SCHEMA_VERSION`, `makeEvent()`, and `makeBatch()` in `generated/valadrien-os-telemetry.ts` |
...
ValadrienOs telemetry uses named events with explicit dimension fields. Treat
...
1. Start from `generated/valadrien-os-telemetry.ts`. The generated types are what
