---
name: task-planning
description: Turn a ValadrienOs issue or request into a structured implementation plan with child task graph, blockers, owners, and acceptance criteria, then save it as the issue `plan` document.
key: paperclipai/bundled/valadrien-os-operations/task-planning
recommendedForRoles:
  - manager
  - engineer
  - product
...
Produce implementation plans that the ValadrienOs executor can actually run: explicit child issues, real blockers, named owners, and a defined acceptance bar. Avoid plans that read well but cannot be split into work.
...
Use the ValadrienOs API to write the plan document, then comment:
