...
- **No reflection on yourself.** If `targetAgentId == VALADRIEN_OS_AGENT_ID`, refuse and ask for another coach.
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/agents/<targetAgentId>" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```

Record `name`, `role`, `reportsTo`, `adapterType`, `adapterConfig.instructionsFilePath` (where `AGENTS.md` lives), and current assigned skills via `GET /api/agents/<targetAgentId>/skills`. Refuse and exit if `targetAgentId == $VALADRIEN_OS_AGENT_ID`.

### 2) Pull the recent record

```sh
curl -sS "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/issues?assigneeAgentId=<targetAgentId>&status=done,in_review,blocked&limit=25" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/issues/<issueId>" -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
curl -sS "$VALADRIEN_OS_API_URL/api/issues/<issueId>/comments" -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"
```

Keep status transitions, blocker reasons, reviewer comments, approval outcomes, human corrections, and PR-link comments. Comments are the closest thing ValadrienOs has to an execution trace — treat them as first-class evidence.
...
- [ ] `targetAgentId != $VALADRIEN_OS_AGENT_ID`
