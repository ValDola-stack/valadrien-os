...
const EXPECTED_BUNDLED_KEYS = [
  ...
  "paperclipai/bundled/product/valadrien-os-capsules",
...
describe("shipped skills catalog", () => {
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@valadrien-os/skills-catalog");
  ...
  it("keeps the Ramp wrapper fail-closed on mixed-provenance playbooks", () => {
    ...
    expect(rampSkill).toContain("do not execute them inside ValadrienOs unless a ValadrienOs approval explicitly names the playbook");
