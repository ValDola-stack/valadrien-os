...
const CATALOG_PACKAGE_NAME = "@valadrien-os/teams-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @valadrien-os/teams-catalog build:manifest.");
    }
  }
...
async function loadCatalogSkills(packageDir: string, errors: string[]): Promise<CatalogSkillSummary[]> {
  try {
    const catalogPackageName = "@valadrien-os/skills-catalog";
    ...
    try {
      ...
      errors.push(`Could not load @valadrien-os/skills-catalog for skill requirement validation: ${errorMessage(error)}`);
      return [];
    }
  }
}
...
function resolveSkillRequirement(
  ...
  errors.push(`${prefix} skill reference "${ref}" does not resolve to a local team skill or @valadrien-os/skills-catalog skill.`);
  return {
    type: "catalog",
    ref,
    agentSlugs: agentSlugs.sort(),
...
function classifyCatalogFile(relativePath: string): CatalogTeamFileKind {
  ...
  if (relativePath === ".valadrien-os.yaml") return "extension";
