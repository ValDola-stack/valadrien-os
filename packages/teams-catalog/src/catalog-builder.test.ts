...
const catalogSkills = [
  ...
  {
    id: "paperclipai:bundled:valadrien-os-operations:task-planning",
...
describe("teams catalog manifest", () => {
  ...
  it("detects stale generated manifests", async () => {
    ...
    await fs.writeFile(
      ...
      formatCatalogManifest({
        schemaVersion: 1,
        packageName: "@valadrien-os/teams-catalog",
    ...
    expect(result.errors).toContain(
      "generated/catalog.json is stale. Run pnpm --filter @valadrien-os/teams-catalog build:manifest.",
    );
  });
});
