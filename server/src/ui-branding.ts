const FAVICON_BLOCK_START = "<!-- VALADRIEN_OS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- VALADRIEN_OS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- VALADRIEN_OS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- VALADRIEN_OS_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.VALADRIEN_OS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.VALADRIEN_OS_WORKTREE_NAME) ?? nonEmpty(env.VALADRIEN_OS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.VALADRIEN_OS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
  ...
  return {
    ...
    instanceId: nonEmpty(env.VALADRIEN_OS_INSTANCE_ID),
  };
}
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  const tags = [
    '<meta name="valadrien-os-worktree-enabled" content="true" />',
    `<meta name="valadrien-os-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="valadrien-os-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="valadrien-os-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ];
  if (branding.instanceId) {
    tags.push(`<meta name="valadrien-os-instance-id" content="${escapeHtmlAttribute(branding.instanceId)}" />`);
  }
  return tags.join("\n");
}
