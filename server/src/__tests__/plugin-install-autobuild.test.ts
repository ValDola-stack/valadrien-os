...
import { createDb, plugins } from "@valadrien-os/db";
import {
...
async function createBundledPluginFixture(
  ...
  const packageName = `@valadrien-os/${slug}`;
  const pluginKey = `valadrien-os.${slug.replace(/^plugin-/, "").replace(/-/g, "_")}`;
...
describe("ensureLocalPluginBuilt", () => {
  ...
  it("skips auto-build for local plugin paths outside the repo", async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-plugin-outside-"));
    ...
    await ensureLocalPluginBuilt(
      packageRoot,
      {
        name: "@valadrien-os/plugin-outside",
        valadrienOsPlugin: {
          manifest: "./dist/manifest.js",
          worker: "./dist/worker.js",
        },
  ...
  it("skips auto-build when VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD=1", async () => {
    ...
    await ensureLocalPluginBuilt(
      ...
      {
        processEnv: { VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD: "1" },
        execFileAsyncImpl: execStub,
      },
    );
...
describeEmbeddedPostgres("plugin install auto-build route", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-plugin-autobuild-");
  ...
  afterEach(async () => {
    ...
    cleanupPaths.clear();
    delete process.env["VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD"];
  });
  ...
  it("returns the manual build command when auto-build is disabled and dist is missing", async () => {
    process.env["VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD"] = "1";
    ...
    expect(res.body.error).toContain("does not appear to be a ValadrienOs plugin (no manifest found)");
