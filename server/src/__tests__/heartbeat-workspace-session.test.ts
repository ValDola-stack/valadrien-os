...
async function createGitCheckout(options: { withRemote: boolean }) {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-push-preflight-"));
...
describe("assertGitSensitiveAdapterWorkspaceValid", () => {
  ...
  it("rejects a git worktree persisted workspace when the checked-out branch differs from the recorded branch", async () => {
    ...
    const worktreeParent = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-branch-worktree-"));
  ...
  it("rejects a workspace-linked issue when adapter cwd has no git metadata", async () => {
    const input = buildWorkspaceValidationInput();
    const cwd = "/tmp/valadrien-os-workspace-without-git-metadata";
...
describe("assertGitWorktreeBaseWorkspaceReady", () => {
  ...
  it("rejects isolated git worktrees when the resolved base is not a git checkout", async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-non-git-workspace-"));
    try {
      await expect(assertGitWorktreeBaseWorkspaceReady({
        requestedExecutionWorkspaceMode: "isolated_workspace",
        config: { workspaceStrategy: { type: "git_worktree" } },
  ...
  it("does not require git for shared project-primary workspaces", async () => {
    const cwd = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-shared-workspace-"));
    try {
      await expect(assertGitWorktreeBaseWorkspaceReady({
  ...
  it("resets when configured model differs from persisted session model", () => {
    expect(
      shouldResetTaskSessionForModelChange({
        ...
        taskSessionParams: {
          sessionId: "thread-1",
          __valadrienOsConfiguredModel: "opencode/mimo-v2-pro-free",
        },
      }),
    ).toBe(true);
  });
  ...
  it("does not reset when models match", () => {
    expect(
      shouldResetTaskSessionForModelChange({
        ...
        taskSessionParams: {
          sessionId: "thread-1",
          __valadrienOsConfiguredModel: "gpt-5.4-mini",
        },
      }),
    ).toBe(false);
  });
    ...
    runtimeSkills: [
      {
        ...
        source: "/tmp/valadrien-os/runtime-skills/valadrien-os",
        versionId: null,
...
function sessionParamsWithConfigMetadata(
  ...
  return {
    sessionId: "thread-1",
    __valadrienOsConfiguredModel: configuredModel,
    __valadrienOsConfigFingerprint: metadata.fingerprint,
    __valadrienOsConfigFingerprintVersion: metadata.version,
    __valadrienOsConfigCategories: metadata.categories,
    __valadrienOsConfigCategoryFingerprints: metadata.categoryFingerprints,
  };
}
...
describe("effective run session config freshness", () => {
  ...
  it("detects instructions content drift without storing the contents", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-session-fingerprint-"));
...
describe("stripConfiguredModelFromSessionParams", () => {
  ...
  it("returns a copy without mutating the input", () => {
    const input = { sessionId: "thread-1", __valadrienOsConfiguredModel: "gpt-5.4-mini" };
    const result = stripConfiguredModelFromSessionParams(input);
    expect(result).not.toBe(input);
    expect(input.__valadrienOsConfiguredModel).toBe("gpt-5.4-mini");
  });
...
describe("stripValadrienOsSessionMetadataFromSessionParams", () => {
  it("removes all internal ValadrienOs session metadata before adapter invocation", () => {
    expect(
      stripValadrienOsSessionMetadataFromSessionParams({
        sessionId: "thread-1",
        cwd: "/tmp/project",
        __valadrienOsConfiguredModel: "gpt-5.4-mini",
        __valadrienOsConfigFingerprint: "v1:sha256:abc",
        __valadrienOsConfigFingerprintVersion: 1,
        __valadrienOsConfigCategories: ["adapterConfig"],
        __valadrienOsConfigCategoryFingerprints: { adapterConfig: "v1:sha256:def" },
      }),
    ).toEqual({
      sessionId: "thread-1",
      cwd: "/tmp/project",
