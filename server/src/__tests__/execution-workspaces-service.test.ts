...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "ValadrienOs Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@valadrien-os.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-execution-workspaces-service-");
  ...
  it("reconciles a forward branch record, comments on the source issue, and resolves matching workspace recovery", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-reconcile-${randomUUID()}`);
  ...
  it("quarantine_restore rescues dirty live-branch work, resolves recovery, and returns the source issue to todo", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-quarantine-restore-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    expect(result.rescueRef).toMatchObject({
      branchName: expect.stringMatching(/^valadrien-os\/rescue\/PAP-124\/\d{8}T\d{6}Z$/),
      fileCount: 2,
    });
  ...
  it("quarantine_restore rejects active runtime services before creating a rescue branch", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-quarantine-running-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    await expect(readGit(
      repoRoot,
      ["for-each-ref", "--format=%(refname:short)", "refs/heads/valadrien-os/rescue"],
    )).resolves.toBeNull();
  ...
  it.each(["review", "approval"] as const)(
    ...
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-quarantine-${stageType}-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it.each([
    ...
    async ({ claimantIssueIdentifier, claimantHasActiveRun, expectedReason }) => {
      const repoRoot = await createTempRepo();
      tempDirs.add(repoRoot);
      const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-quarantine-claimant-${randomUUID()}`);
      tempDirs.add(worktreePath);
      ...
      const claimantWorkspacePath = path.join(path.dirname(repoRoot), `valadrien-os-claimant-${randomUUID()}`);
      const now = new Date();
  ...
  it("rejects branch reconciliation when the worktree is dirty", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-dirty-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation while the workspace lifecycle is active", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-active-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation if the workspace becomes active before the branch record update", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-race-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation while runtime services are active", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-running-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation when a runtime service starts before the locked update", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-raced-service-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects branch reconciliation when runtime service activation is already spawning", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-spawning-service-reconcile-${randomUUID()}`);
    tempDirs.add(worktreePath);
    ...
    const issueId = randomUUID();
    const executionWorkspaceId = randomUUID();
    const runtimeStartedMarker = path.join(os.tmpdir(), `valadrien-os-runtime-started-${randomUUID()}.marker`);
  ...
  it("rejects forward branch reconciliation for diverged branches", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-diverged-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("rejects forward branch reconciliation when branch ancestry is unknown", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-unknown-${randomUUID()}`);
    tempDirs.add(worktreePath);
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    const repoRoot = await createTempRepo();
    tempDirs.add(repoRoot);
    const worktreePath = path.join(path.dirname(repoRoot), `valadrien-os-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "valadrien-os-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "valadrien-os-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      cwd: worktreePath,
      providerRef: worktreePath,
      branchName: "valadrien-os-close-check",
      baseRef: "main",
      metadata: {
