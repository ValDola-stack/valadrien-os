...
import { activityLog, agents, companies, createDb, executionWorkspaces, goals, issues, projects, projectWorkspaces, type Db } from "@valadrien-os/db";
...
async function makeWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-file-resources-"));
...
describeEmbeddedPostgres("workspace file resources", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-file-resources-");
...
describeEmbeddedPostgres("file resource route guards", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-file-resource-guards-");
    db = createDb(tempDb.connectionString);
  }, 60_000);
