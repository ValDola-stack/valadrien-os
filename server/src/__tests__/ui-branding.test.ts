...
<head>
    <!-- VALADRIEN_OS_RUNTIME_BRANDING_START -->
    <!-- VALADRIEN_OS_RUNTIME_BRANDING_END -->
    <!-- VALADRIEN_OS_FAVICON_START -->
    <link rel="icon" href="/favicon.ico" sizes="48x48" />
    <link rel="icon" href="/favicon.svg" type="image/svg+xml" />
    <link rel="icon" type="image/png" sizes="32x32" href="/favicon-32x32.png" />
    <link rel="icon" type="image/png" sizes="16x16" href="/favicon-16x16.png" />
    <!-- VALADRIEN_OS_FAVICON_END -->
</head>`;

describe("ui branding", () => {
  it("detects worktree mode from VALADRIEN_OS_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ VALADRIEN_OS_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ VALADRIEN_OS_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ VALADRIEN_OS_IN_WORKTREE: "false" })).toBe(false);
  });
  ...
  it("resolves name, color, and text color for worktree branding", () => {
    ...
    expect(branding.name).toBe("valadrien-os-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="valadrien-os-worktree-name"');
    expect(meta).toContain('content="valadrien-os-pr-432"');
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="valadrien-os-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="valadrien-os-worktree-name"');
  });
});
