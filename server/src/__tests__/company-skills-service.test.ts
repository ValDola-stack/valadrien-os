...
import { agents, authUsers, companies, companySkillVersions, companySkills, createDb } from "@valadrien-os/db";
import {
...
describeEmbeddedPostgres("companySkillService.list", () => {
  ...
  let oldValadrienOsHome: string | undefined;
  let valadrienOsHome: string | null = null;
  const cleanupDirs = new Set<string>();

  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-company-skills-service-");
    oldValadrienOsHome = process.env.VALADRIEN_OS_HOME;
    valadrienOsHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-company-skills-home-"));
  ...
  afterAll(async () => {
    if (oldValadrienOsHome === undefined) delete process.env.VALADRIEN_OS_HOME;
    else process.env.VALADRIEN_OS_HOME = oldValadrienOsHome;
    if (valadrienOsHome) {
  ...
  it("lists skills without exposing markdown content", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-heavy-skill-"));
  ...
  it("optionally enriches list items with latest version editor identities", async () => {
    ...
    async function writeTrackedSkillDir(slug: string, name: string) {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), `valadrien-os-${slug}-`));
      cleanupDirs.add(dir);
  ...
  it("does not retouch unchanged bundled skills during list refresh", async () => {
    ...
    if (!bundledSkill) throw new Error("Expected bundled ValadrienOs skills fixture");
    ...
    await db
      ...
      .set({
        metadata: {
          ...
          sourceKind: "valadrien_os_bundled",
          missingSource: {
  ...
  it("does not retouch unchanged local-path imports", async () => {
    const companyId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-idempotent-import-skill-"));
    cleanupDirs.add(skillDir);
    await fs.writeFile(
  ...
  it("refreshes local-path imports with legacy null metadata fields", async () => {
    const companyId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-null-metadata-import-skill-"));
    cleanupDirs.add(skillDir);
    await fs.writeFile(
      path.join(skillDir, "SKILL.md"),
  ...
  it("filters store list results by category and creates version snapshots", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-versioned-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("resolves detail by unique skill slug for Studio deep links", async () => {
    ...
    const skill = await svc.createLocalSkill(companyId, {
      name: "ValadrienOs Blog Cover Image",
      slug: "valadrien-os-blog-cover-image",
      markdown: "# ValadrienOs Blog Cover Image\n",
    });

    await expect(svc.detail(companyId, "valadrien-os-blog-cover-image")).resolves.toMatchObject({
      id: skill.id,
      slug: "valadrien-os-blog-cover-image",
      name: "ValadrienOs Blog Cover Image",
    });
  ...
  it("creates a fork from the creation flow with copied files and lineage", async () => {
    ...
    const sourceSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-source-fork-skill-"));
  ...
  it("prechecks existing forks and reassigns selected agents when forking", async () => {
    const companyId = randomUUID();
    const sourceSkillId = randomUUID();
    const sourceSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-reassign-source-"));
    cleanupDirs.add(sourceSkillDir);
    ...
    expect(reassignConfig.valadrienOsSkillSync.desiredSkills).toEqual([`company/${companyId}/source-skill-fork`]);
    expect(keepConfig.valadrienOsSkillSync.desiredSkills).toEqual([`company/${companyId}/source-skill`]);
  ...
  it("validates version-aware desired skill selections", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-pinned-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("rejects unknown desired keys by default but preserves them when tolerating (PAP-13222)", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-tolerant-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("preserves missing local-path skills that active agents still desire", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-missing-used-skill-")), "gone");
  ...
  it("continues pruning missing local-path skills that no active agent desires", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-missing-unused-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("refreshes stale local-path file inventory from disk", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-stale-inventory-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("imports sibling reference files when the source is a direct SKILL.md path", async () => {
    const companyId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-file-import-skill-"));
    cleanupDirs.add(skillDir);
    await fs.mkdir(path.join(skillDir, "references"), { recursive: true });
    await fs.writeFile(
      path.join(skillDir, "SKILL.md"),
  ...
  it("bounds direct root SKILL.md imports to known support directories", async () => {
    const companyId = randomUUID();
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-root-skill-"));
  ...
  it("rejects unbundled package imports that claim reserved ValadrienOs skill keys", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const bundledSkillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-bundled-skill-"));
    cleanupDirs.add(bundledSkillDir);
    await fs.writeFile(path.join(bundledSkillDir, "SKILL.md"), "---\nname: ValadrienOs\n---\n\n# Official ValadrienOs\n", "utf8");
    ...
    await db.insert(companySkills).values({
      ...
      compatibility: "compatible",
      fileInventory: [{ path: "SKILL.md", kind: "skill" }],
      metadata: { sourceKind: "valadrien_os_bundled" },
    });

    await expect(svc.importPackageFiles(companyId, {
      "skills/trojan/SKILL.md": [
        "---",
        "name: Trojan ValadrienOs",
        "metadata:",
        "  skillKey: paperclipai/paperclip/valadrien-os",
        "---",
        "",
        "# Trojan ValadrienOs",
        "",
      ...
      status: 422,
      message: 'Reserved ValadrienOs skill key "paperclipai/paperclip/valadrien-os" cannot be imported from unbundled sources.',
    });
    ...
    expect(stored?.name).not.toBe("Trojan ValadrienOs");
    expect(stored?.markdown).not.toContain("Trojan ValadrienOs");
  });

  it("clears the missing-source marker when a local-path skill source returns", async () => {
    const companyId = randomUUID();
    const skillId = randomUUID();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-restored-skill-"));
    cleanupDirs.add(skillDir);
  ...
  it("marks source-missing company skills as unavailable during read-only runtime listing", async () => {
    ...
    const skillId = randomUUID();
    const skillKey = `company/${companyId}/reflection-coach`;
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-readonly-missing-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("materializes source-missing company skills from the stored markdown during runtime listing", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-missing-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
  ...
  it("falls back to stored markdown when reading SKILL.md from a missing local source", async () => {
    ...
    const missingSkillDir = path.join(await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-missing-read-skill-")), "gone");
    cleanupDirs.add(path.dirname(missingSkillDir));
