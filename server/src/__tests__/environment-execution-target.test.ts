...
describe("resolveEnvironmentExecutionTarget", () => {
  ...
  it("keeps sandbox targets on bridge mode even when lease metadata includes a ValadrienOs API URL", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      driver: "sandbox",
      config: {
        provider: "fake-plugin",
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        valadrienOsApiUrl: "https://valadrien-os.example.test",
      },
      lease: null,
      environmentRuntime: null,
    });
    ...
    expect(target).not.toHaveProperty("valadrienOsApiUrl");
    expect(target).not.toHaveProperty("valadrienOsTransport");
  });
  ...
  it("keeps sandbox targets on callback bridge execution even when lease metadata advertises SSH access", async () => {
    ...
    const target = await resolveEnvironmentExecutionTarget({
      ...
      leaseId: "lease-1",
      leaseMetadata: {
        remoteCwd: "/home/sandbox/valadrien-os-workspace",
        sshAccess: {
          type: "ssh",
  ...
  it("resolves SSH execution targets in bridge mode", async () => {
    mockResolveEnvironmentDriverConfigForRuntime.mockResolvedValue({
      ...
      config: {
        ...
        port: 22,
        username: "valadrien-os",
        remoteWorkspacePath: "/srv/valadrien-os",
