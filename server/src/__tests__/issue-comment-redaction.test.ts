...
import { companySearchQuerySchema } from "@valadrien-os/shared";
import {
...
import { buildValadrienOsWakePayload } from "../services/heartbeat.js";
...
describeEmbeddedPostgres("deleted issue comment redaction", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-comment-redaction-");
  ...
  it("redacts deleted comment bodies from ordinary reads, heartbeat context, and wake payloads", async () => {
    ...
    const wakePayload = await buildValadrienOsWakePayload({
      db,
      companyId,
      contextSnapshot: {
        issueId,
