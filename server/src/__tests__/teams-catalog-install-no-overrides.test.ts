...
import { agents, companies, createDb } from "@valadrien-os/db";
import {
...
describeEmbeddedPostgres("teams catalog install with no caller adapter overrides", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-teams-catalog-no-overrides-"));
    process.env.VALADRIEN_OS_HOME = tempHome;
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-teams-catalog-no-overrides-");
