...
import { companies, companySkills, createDb } from "@valadrien-os/db";
import {
  getEmbeddedPostgresTestSupport,
  startEmbeddedPostgresTestDatabase,
} from "./helpers/embedded-postgres.js";
import type { CatalogSkill, CatalogSkillFile } from "@valadrien-os/shared";
...
describeEmbeddedPostgres("companySkillService.installFromCatalog", () => {
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-company-skills-catalog-");
    db = createDb(tempDb.connectionString);
    svc = await createService();
  }, 20_000);

  beforeEach(async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-catalog-home-"));
  ...
  it("rejects duplicate slug conflicts", async () => {
    const companyId = await createCompany();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-existing-skill-"));
