...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "valadrien-os",
        name: "valadrien-os",
        description: "ValadrienOs coordination skill",
        markdown: "---\nname: valadrien-os\ndescription: ValadrienOs coordination skill\n---\n\n# ValadrienOs\n",
        sourceType: "github",
        sourceLocator: "https://github.com/ValDola-stack/valadrien-os/tree/master/skills/valadrien-os",
        ...
        metadata: {
          ...
          trackingRef: "master",
          repoSkillDir: "skills/valadrien-os",
        },
      },
      {
        id: "skill-2",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/valadrien-os/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      hostname: "github.com",
      owner: "valadrien-os",
      repo: "companies",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/valadrien-os/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      hostname: "github.com",
      owner: "valadrien-os",
  ...
  it("exports referenced skills as stubs by default with sanitized ValadrienOs extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "ValadrienOs"');
    ...
    const extension = asTextFile(exported.files[".valadrien-os.yaml"]);
    expect(extension).toContain('schema: "valadrien-os/v1"');
    ...
    expect(extension).not.toContain("valadrienOsSkillSync");
  ...
  it("exports agent permission grants through the ValadrienOs extension and manifest", async () => {
    const db = {
  ...
  it("exports hire approval policy only when approval is required", async () => {
    ...
    expect(asTextFile(exported.files[".valadrien-os.yaml"])).toContain("requireBoardApprovalForNewAgents: true");
  });
  ...
  it("exports default sidebar order into the ValadrienOs extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".valadrien-os.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("exports catalog skill provenance in portable ValadrienOs frontmatter", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    const catalogSkill = {
      ...
      markdown: "---\nname: review\ndescription: Catalog review skill\n---\n\n# Review\n",
      sourceType: "catalog",
      sourceLocator: "/tmp/valadrien-os/catalog/review",
      ...
      metadata: {
        ...
        packageName: "@valadrien-os/skills-catalog",
    ...
    expect(skillMarkdown).toContain("valadrien-os:");
    ...
    expect(skillMarkdown).toContain('packageName: "@valadrien-os/skills-catalog"');
  ...
  it("exports the company logo into images/ and references it from .valadrien-os.yaml", async () => {
    ...
    expect(exported.files[".valadrien-os.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        ...
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/ValDola-stack/valadrien-os/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/valadrien-os",
            repoUrl: "https://github.com/ValDola-stack/valadrien-os.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .valadrien-os-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/ValDola-stack/valadrien-os.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/valadrien-os");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported ValadrienOs",
      },
  ...
  it("normalizes invalid imported project icon names to null", async () => {
    ...
    await portability.importBundle({
      source: { type: "inline", rootPath: "valadrien-os-demo", files },
      include: { company: true, agents: false, projects: true, issues: false },
      target: { mode: "new_company", newCompanyName: "Imported ValadrienOs" },
      collisionStrategy: "rename",
    }, "user-1");
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/ValDola-stack/valadrien-os.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".valadrien-os.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/ValDola-stack/valadrien-os.git"');
    expect(extension).toContain('projectWorkspaceKey: "valadrien-os"');
  ...
  it("reads env inputs back from .valadrien-os.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("reads project env inputs back from .valadrien-os.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with ValadrienOs routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported ValadrienOs"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .valadrien-os.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported ValadrienOs");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".valadrien-os.yaml"] = `${exported.files[".valadrien-os.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
