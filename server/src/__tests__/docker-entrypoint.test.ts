...
describe("docker-entrypoint.sh", () => {
  ...
  it("remaps the node user and chowns /valadrien-os before gosu when root requests a different UID/GID", async () => {
    installStubs({ uid: 0, gid: 0 });
    ...
    expect(calls).toContain("chown -R node:node /valadrien-os");
    expect(calls).toContain("gosu node echo ENTRYPOINT-CMD-RAN");
  });
