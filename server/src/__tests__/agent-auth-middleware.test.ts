...
function craftAgentJwtWithoutResponsibleClaim(input: {
  ...
  // instance defaults to "default" (beforeEach clears VALADRIEN_OS_INSTANCE_ID),
...
describe("agent auth middleware", () => {
  const originalSecret = process.env.VALADRIEN_OS_AGENT_JWT_SECRET;
  const originalTtl = process.env.VALADRIEN_OS_AGENT_JWT_TTL_SECONDS;
  const originalInstanceId = process.env.VALADRIEN_OS_INSTANCE_ID;

  beforeEach(() => {
    process.env.VALADRIEN_OS_AGENT_JWT_SECRET = "auth-middleware-secret";
    process.env.VALADRIEN_OS_AGENT_JWT_TTL_SECONDS = "3600";
  ...
  afterEach(() => {
    if (originalSecret === undefined) delete process.env.VALADRIEN_OS_AGENT_JWT_SECRET;
    else process.env.VALADRIEN_OS_AGENT_JWT_SECRET = originalSecret;
    if (originalTtl === undefined) delete process.env.VALADRIEN_OS_AGENT_JWT_TTL_SECONDS;
    else process.env.VALADRIEN_OS_AGENT_JWT_TTL_SECONDS = originalTtl;
    if (originalInstanceId === undefined) delete process.env.VALADRIEN_OS_INSTANCE_ID;
    else process.env.VALADRIEN_OS_INSTANCE_ID = originalInstanceId;
  });
  ...
  it("uses the signed responsible_user_id claim and keeps the signed run id authoritative", async () => {
    ...
    const res = await request(createApp(db))
      ...
      .set("X-ValadrienOs-Run-Id", runId);
  ...
  it("rejects mismatched run headers for agent JWTs and audits the spoof attempt", async () => {
    ...
    const res = await request(createApp(db))
      .get("/actor")
      .set("Authorization", `Bearer ${token}`)
      .set("X-ValadrienOs-Run-Id", spoofedRunId);
  ...
  it("falls back to the run row responsible user for legacy claim-less agent JWTs", async () => {
    ...
    const token = craftAgentJwtWithoutResponsibleClaim({
      secret: process.env.VALADRIEN_OS_AGENT_JWT_SECRET!,
      agentId,
      companyId,
