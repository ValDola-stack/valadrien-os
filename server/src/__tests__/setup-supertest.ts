...
type SupertestTestConstructor = {
  prototype: {
    ...
    __valadrienOsLoopbackPatched?: boolean;
  };
};
...
if (!process.env.CODEX_HOME) {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "valadrien-os-vitest-codex-home-"));
  fs.writeFileSync(path.join(codexHome, "auth.json"), '{"OPENAI_API_KEY":"sk-vitest"}\n', { mode: 0o600 });
  process.env.CODEX_HOME = codexHome;
}

if (!SupertestTest.prototype.__valadrienOsLoopbackPatched) {
  ...
  SupertestTest.prototype.__valadrienOsLoopbackPatched = true;
}
