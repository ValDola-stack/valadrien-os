...
describe("built-in agent asset loading", () => {
  it("uses the first readable candidate path", () => {
    const dir = mkdtempSync(path.join(tmpdir(), "valadrien-os-built-in-agent-"));
    try {
...
describeEmbeddedPostgres("built-in agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-built-in-agents-");
  ...
  it("auto-provisions a paused Reflection Coach bundle with skill sync and a disabled routine", async () => {
    ...
    const [skill] = await db
      .select()
      .from(companySkills)
      .where(eq(companySkills.key, "paperclipai/bundled/valadrien-os-operations/reflection-coach"));
    expect(skill).toMatchObject({
      key: "paperclipai/bundled/valadrien-os-operations/reflection-coach",
      slug: "reflection-coach",
    });
    expect(readValadrienOsSkillSyncPreference(state.agent!.adapterConfig as Record<string, unknown>).desiredSkills).toContain(
      "paperclipai/bundled/valadrien-os-operations/reflection-coach",
    );
  ...
  it("prevents direct marker add, remove, or mutation", async () => {
    ...
    await expect(agentService(db).update(builtIn.agentId!, {
      ...
      metadata: {
        note: "allowed",
        valadrienOsBuiltInAgent: { key: "briefs", featureKeys: ["briefs"] },
      },
    });
  });
  ...
  it("automatically materializes the Reflection Coach bundle without enabling background work", async () => {
    ...
    expect(readValadrienOsSkillSyncPreference(state.agent!.adapterConfig).desiredSkills).toContain(skill!.key);
