...
describe("resolveExecutionRunAdapterConfig", () => {
  ...
  it("drops ValadrienOs runtime-owned env before resolving environment, agent, project, and routine overlays", async () => {
    ...
    const result = await resolveExecutionRunAdapterConfig({
      ...
      environmentId: "environment-1",
      environmentEnv: {
        VALADRIEN_OS_API_KEY: "environment-api-key",
        VALADRIEN_OS_AGENT_ID: "environment-agent",
        ENV_ONLY: "environment-only",
      },
      executionRunConfig: {
        env: {
...
describe("resolveExecutionRunAdapterConfig codex_local credential pre-dispatch gate", () => {
  ...
  async function stubManagedCodexEnv(options: { seedSharedAuth: boolean }) {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-gate-"));
    ...
    vi.stubEnv("VALADRIEN_OS_HOME", valadrienOsHome);
    vi.stubEnv("VALADRIEN_OS_INSTANCE_ID", "default");
