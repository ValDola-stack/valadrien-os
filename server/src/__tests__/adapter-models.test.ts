...
import { models as claudeFallbackModels } from "@valadrien-os/adapter-claude-local";
import { resetClaudeModelsCacheForTests } from "@valadrien-os/adapter-claude-local/server";
...
describe("adapter model listing", () => {
  ...
  describe("VALADRIEN_OS_ADAPTER_MODELS declared models", () => {
    afterEach(() => {
      delete process.env.VALADRIEN_OS_ADAPTER_MODELS;
    });

    it("prefers declared env models over adapter discovery", async () => {
      process.env.VALADRIEN_OS_ADAPTER_MODELS = JSON.stringify({
        opencode_local: [
    ...
    it("fails soft on malformed values: falls back to adapter models instead of throwing", async () => {
      process.env.VALADRIEN_OS_ADAPTER_MODELS = "{not json";
      process.env.VALADRIEN_OS_OPENCODE_COMMAND = "__valadrien_os_missing_opencode_command__";
