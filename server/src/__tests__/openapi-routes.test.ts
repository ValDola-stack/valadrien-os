...
describe("openapi routes", () => {
  it("serves the generated OpenAPI document", async () => {
    ...
    expect(res.body.info.title).toBe("ValadrienOs API");
  ...
  it("documents auth and reviewed response-code invariants", () => {
    ...
    expect(spec.paths["/api/plugins/install"].post["x-valadrien-os-authorization"]).toEqual({
      actor: "board",
      instanceAdmin: true,
    });
    ...
    expect(spec.paths["/api/execution-workspaces/{id}/reconcile-branch"].post["x-valadrien-os-authorization"]).toEqual({
      actor: "board",
    });
