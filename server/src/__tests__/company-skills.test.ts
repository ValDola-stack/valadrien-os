...
describe("project workspace skill discovery", () => {
  ...
  it("parses folded and literal block scalar descriptions in skill frontmatter", async () => {
    const foldedWorkspace = await makeTempDir("valadrien-os-folded-skill-yaml-");
    ...
    const literalWorkspace = await makeTempDir("valadrien-os-literal-skill-yaml-");
    ...
    const stripWorkspace = await makeTempDir("valadrien-os-strip-skill-yaml-");
  ...
  it("parses YAML block-scalar chomping variants from SKILL.md frontmatter", async () => {
    const workspace = await makeTempDir("valadrien-os-block-scalar-chomp-skill-");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(
      path.join(workspace, "SKILL.md"),
      [
