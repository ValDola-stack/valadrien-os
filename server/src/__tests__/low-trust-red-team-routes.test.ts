...
import { ISSUE_CONTINUATION_SUMMARY_DOCUMENT_KEY, LOW_TRUST_REVIEW_PRESET } from "@valadrien-os/shared";
import {
...
describeEmbeddedPostgres("low-trust red-team HTTP route regression suite", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-low-trust-red-team-routes-");
  ...
  it("redacts quarantined low-trust output from higher-trust wake and continuation contexts", async () => {
    ...
    const heartbeat = heartbeatService(db, {
      runtimeEnv: {
        ...process.env,
        VALADRIEN_OS_IN_WORKTREE: "false",
        VALADRIEN_OS_DATABASE_RESTORE_IN_PROGRESS: "false",
        VALADRIEN_OS_RESTORE_IN_PROGRESS: "false",
      },
    });
    ...
    try {
      ...
      // generated message rather than a top-level `valadrien-os` field.
      expect(payload.valadrien-os).toBeUndefined();
      ...
      expect(String(payload.message ?? "")).toContain("## ValadrienOs Wake Payload");
