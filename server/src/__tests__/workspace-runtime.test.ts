...
async function createClonedRepoWithRemote() {
  ...
  const remoteDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-worktree-remote-"));
  const remotePath = path.join(remoteDir, "valadrien-os.git");
  await execFileAsync("git", ["clone", "--bare", sourceRepo, remotePath]);

  const cloneRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-worktree-clone-"));
  const repoRoot = path.join(cloneRoot, "valadrien-os");
...
describe("realizeExecutionWorkspace", () => {
  ...
  it("reinstalls worktree-local pnpm dependencies when package metadata changes", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-worktree-stale-deps-"));
  ...
  it("regenerates stale worktree config that points at another host", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-worktree-stale-config-"));
    const baseRoot = path.join(tempRoot, "base");
    const worktreeRoot = path.join(tempRoot, "worktree");
    const fakeBin = path.join(tempRoot, "bin");
    const fakePnpmPath = path.join(fakeBin, "pnpm");
    ...
    try {
      ...
      await fs.chmod(scriptPath, 0o755);
      await fs.writeFile(
        path.join(valadrienOsDir, "config.json"),
        JSON.stringify({
          database: {
            mode: "embedded-postgres",
            embeddedPostgresDataDir: "/Users/example/.valadrien-os-worktrees/instances/stale/db",
          },
          logging: {
            mode: "file",
            logDir: "/Users/example/.valadrien-os-worktrees/instances/stale/logs",
          },
          storage: {
            provider: "local_disk",
            localDisk: {
              baseDir: "/Users/example/.valadrien-os-worktrees/instances/stale/data/storage",
            },
          },
          secrets: {
            provider: "local_encrypted",
            localEncrypted: {
              keyFilePath: "/Users/example/.valadrien-os-worktrees/instances/stale/secrets/master.key",
            },
          },
        }),
        "utf8",
      ...
      await fs.writeFile(
        path.join(valadrienOsDir, ".env"),
        [
          "VALADRIEN_OS_HOME=/Users/example/.valadrien-os-worktrees",
          "VALADRIEN_OS_INSTANCE_ID=stale",
          `VALADRIEN_OS_CONFIG=/Users/example/valadrien-os/${path.basename(worktreeRoot)}/.valadrien-os/config.json`,
          "",
        ].join("\n"),
        "utf8",
      );
      await fs.writeFile(
        ...
        [
          ...
          "fi",
          "if [ \"$1\" = \"valadrien-os\" ] && [ \"$2\" = \"worktree\" ] && [ \"$3\" = \"init\" ]; then",
          "  mkdir -p \"$PWD/.valadrien-os\"",
          "  printf '%s\\n' '{\"database\":{\"embeddedPostgresDataDir\":\"'$PWD'/.valadrien-os/runtime/db\"}}' > \"$PWD/.valadrien-os/config.json\"",
          "  printf '%s\\n' \"VALADRIEN_OS_HOME=$PWD/.valadrien-os/runtime\" \"VALADRIEN_OS_INSTANCE_ID=healthy\" \"VALADRIEN_OS_CONFIG=$PWD/.valadrien-os/config.json\" > \"$PWD/.valadrien-os/.env\"",
          "  exit 0",
          "fi",
          "exit 0",
          "",
      ...
      expect(result.stderr).toContain("Existing isolated ValadrienOs worktree config is stale for this host; regenerating.");
      await expect(fs.readFile(path.join(valadrienOsDir, ".env"), "utf8")).resolves.toContain(
        `VALADRIEN_OS_CONFIG=${worktreeRoot}/.valadrien-os/config.json`,
      );
      await expect(fs.readFile(path.join(valadrienOsDir, "config.json"), "utf8")).resolves.toContain(worktreeRoot);
    } finally {
      await fs.rm(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("repairs a clean persisted git worktree branch mismatch when both branches point at the same commit", async () => {
    ...
    const realWorktreeRoot = path.join(repoRoot, ".valadrien-os", "real-worktrees");
    const symlinkedWorktreeRoot = path.join(repoRoot, ".valadrien-os", "worktrees");
  ...
  it("reattaches a clean forward detached HEAD to the recorded persisted git worktree branch", async () => {
    ...
    const worktreePath = path.join(repoRoot, ".valadrien-os", "worktrees", branchName);
  ...
  it("rejects dirty persisted git worktree branch incoherence with bounded recovery evidence", async () => {
    ...
    const worktreePath = path.join(repoRoot, ".valadrien-os", "worktrees", expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
    await runGit(repoRoot, ["branch", expectedBranch]);
  ...
  it("routes non-reusable persisted git worktrees through workspace validation recovery", async () => {
    ...
    const detachedWorktreePath = path.join(repoRoot, ".valadrien-os", "worktrees", expectedBranch);
  ...
  it("does not reuse a missing persisted local filesystem workspace", async () => {
    const baseCwd = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-workspace-base-"));
...
describe("ensureRuntimeServicesForRun", () => {
  ...
  it("requires ValadrienOs dev runtime services to pass /api/health readiness", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-health-"));
    const workspace = buildWorkspace(workspaceRoot);
    const runId = "run-valadrien-os-health";
...
describe("readLocalServicePortOwner", () => {
  ...
  it("accepts service cwd nested within the requested workspace", async () => {
    const workspace = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-workspace-"));
  ...
  it("keeps a live registry record adoptable when cwd inspection is unsupported", async () => {
    ...
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = `unsupported-cwd-${randomUUID()}`;
    expect(port).toBeTypeOf("number");
    ...
    try {
      ...
      await fs.rm(valadrienOsHome, { recursive: true, force: true });
    }
  });
  ...
  it("refuses to adopt a listener whose real cwd belongs to another workspace", async () => {
    ...
    const targetWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-target-"));
    const ownerWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-owner-"));
    const valadrienOsHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-home-"));
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = `cross-workspace-${randomUUID()}`;
...
describeEmbeddedPostgres("workspace dirty quarantine branch repair", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-workspace-dirty-quarantine-");
  ...
  async function createDirtyMismatchRepo(input: {
    ...
    const repoRoot = await createTempRepo();
    const worktreePath = path.join(repoRoot, ".valadrien-os", "worktrees", input.expectedBranch);
    await fs.mkdir(path.dirname(worktreePath), { recursive: true });
  ...
  async function seedDirtyQuarantineRecords(input: {
    ...
    if (input.claimant && input.claimant !== "none") {
      ...
      await db.insert(executionWorkspaces).values({
        ...
        status: "active",
        cwd: path.join(input.repoRoot, ".valadrien-os", "claimants", claimantWorkspaceId),
        providerRef: path.join(input.repoRoot, ".valadrien-os", "claimants", claimantWorkspaceId),
        baseRef: "HEAD",
  ...
  it("quarantines dirty foreign-branch work into a rescue branch before restoring the recorded branch", async () => {
    ...
    expect(rescueBranch).toMatch(/^valadrien-os\/rescue\/PAP-455\/\d{8}T\d{6}Z$/);
  ...
  it("quarantines a worktree wedged mid-rebase and clears the interrupted rebase state", async () => {
    ...
    const rescueBranch = warning?.match(/"([^"]+)"/)?.[1] ?? "";
    expect(rescueBranch).toMatch(/^valadrien-os\/rescue\/PAP-456\/\d{8}T\d{6}Z$/);
  ...
  it("refuses dirty quarantine repair while the execution workspace has an active runtime service", async () => {
    ...
    await db.insert(workspaceRuntimeServices).values({
      ...
      status: "running",
      lifecycle: "shared",
      reuseKey: `execution_workspace:${ids.sourceWorkspaceId}:valadrien-os-dev`,
      command: "pnpm dev",
      cwd: worktreePath,
      port: 49195,
    ...
    await expect(readGit(repoRoot, [
      ...
      "refs/heads/valadrien-os/rescue",
    ])).resolves.toBe("");
  }, 20_000);
...
describeEmbeddedPostgres("workspace runtime service control persistence", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-workspace-runtime-control-");
    db = createDb(tempDb.connectionString);
  }, 20_000);
  ...
  it("commits a starting service row before waiting for slow readiness", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-slow-control-"));
    const valadrienOsHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-control-home-"));
    ...
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = `runtime-control-${randomUUID()}`;
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  it("does not reuse a stopped auto-port service port while another process owns it", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-unhealthy-adopt-"));
    const valadrienOsHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-home-"));
    process.env.VALADRIEN_OS_HOME = valadrienOsHome;
    process.env.VALADRIEN_OS_INSTANCE_ID = `runtime-unhealthy-adopt-${randomUUID()}`;
