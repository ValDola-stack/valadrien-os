...
describe("reconcileCodexLocalManagedHomesOnStartup", () => {
  ...
  beforeEach(async () => {
    root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-codex-startup-"));
    valadrienOsHome = path.join(root, "valadrien-os-home");
    ...
    for (const key of ["VALADRIEN_OS_HOME", "VALADRIEN_OS_INSTANCE_ID", "CODEX_HOME"]) {
