...
describeEmbeddedPostgres("issue watchdog routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-issue-watchdogs-routes-");
  ...
  async function seedCompany(name = "ValadrienOs") {
  ...
  it("routes watchdog-discovered product bugs outside the watched source tree with evidence links", async () => {
    ...
    const res = await request(app)
      ...
      .send({
        ...
        description: "Watchdog found a ValadrienOs follow-up routing bug.",
