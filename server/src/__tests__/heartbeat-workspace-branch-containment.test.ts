...
async function createGitRepo() {
  const repoRoot = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-branch-containment-repo-"));
...
function readAdapterWorkspace(input: unknown) {
  ...
  const wake = context.valadrienOsWake as { executionWorkspace?: { branchName?: string } } | undefined;
...
async function seedBranchContainmentRun(
  ...
  await db.insert(executionWorkspaces).values([
    ...
    },
    {
      ...
      status: "active",
      cwd: path.join(repoRoot, ".valadrien-os", "worktrees", "other-workspace"),
      repoUrl: null,
      baseRef: "HEAD",
      branchName: "other-workspace",
      providerType: "git_worktree",
      providerRef: path.join(repoRoot, ".valadrien-os", "worktrees", "other-workspace"),
      lastUsedAt: now,
      openedAt: now,
      createdAt: now,
      updatedAt: now,
...
describeEmbeddedPostgres("heartbeat workspace branch containment", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-branch-containment-");
