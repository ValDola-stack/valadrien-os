...
describe.sequential("plugin install and upgrade authz", () => {
  ...
  it("lists bundled monorepo plugin packages", async () => {
    ...
    expect(packageNames).toContain("@valadrien-os/plugin-workspace-diff");
    expect(packageNames).toContain("@valadrien-os/plugin-llm-wiki");
    expect(packageNames).toContain("@valadrien-os/plugin-modal");
    expect(packageNames).toContain("@valadrien-os/plugin-authoring-smoke-example");
    expect(packageNames).not.toContain("@valadrien-os/plugin-sdk");
    expect(byPackageName.get("@valadrien-os/plugin-workspace-diff")?.experimental).toBe(true);
    expect(byPackageName.get("@valadrien-os/plugin-llm-wiki")?.experimental).toBe(true);
    expect(byPackageName.get("@valadrien-os/plugin-modal")?.experimental).toBe(true);
    expect(byPackageName.get("@valadrien-os/plugin-authoring-smoke-example")?.experimental).toBe(false);
    expect(typeof byPackageName.get("@valadrien-os/plugin-workspace-diff")?.hasBuiltEntrypoints).toBe("boolean");
  }, 20_000);
    ...
    const { app } = await createApp(
      ...
      {},
      {
        ...
        toolDeps: {
          toolDispatcher: {
            listToolsForAgent: vi.fn(),
            getTool: vi.fn(() => ({ name: "valadrien-os.example:search", pluginDbId: pluginId })),
            executeTool,
          },
        },
      },
