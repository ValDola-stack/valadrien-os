...
import { agents, companies, companySkills, createDb } from "@valadrien-os/db";
import type { ValadrienOsSkillEntry } from "@valadrien-os/adapter-utils/server-utils";
import {
...
describeEmbeddedPostgres("heartbeat runtime skill version pins", () => {
  ...
  const capturedRuns: Array<{ agentId: string; skills: ValadrienOsSkillEntry[] }> = [];
  ...
  beforeAll(async () => {
    ...
    valadrienOsHome = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-runtime-skills-home-"));
    ...
    registerServerAdapter({
      ...
      execute: async (ctx) => {
        capturedRuns.push({
          ...
          skills: (ctx.config.valadrienOsRuntimeSkills ?? []) as ValadrienOsSkillEntry[],
        });
        return {
          exitCode: 0,
          signal: null,
  ...
  it("materializes different pinned skill versions for different agents at runtime", async () => {
    ...
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-versioned-runtime-skill-"));
