...
describeEmbeddedPostgres("cases routes", () => {
  ...
  const previousAgentJwtSecret = process.env.VALADRIEN_OS_AGENT_JWT_SECRET;
  ...
  beforeAll(async () => {
    process.env.VALADRIEN_OS_AGENT_JWT_SECRET = "cases-routes-test-secret";
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-cases-routes-");
  ...
  afterAll(async () => {
    ...
    if (previousAgentJwtSecret === undefined) {
      ...
      process.env.VALADRIEN_OS_AGENT_JWT_SECRET = previousAgentJwtSecret;
    }
  });
