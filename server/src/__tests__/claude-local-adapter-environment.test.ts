...
import { resetClaudeCliCapabilitiesCacheForTests, testEnvironment } from "@valadrien-os/adapter-claude-local/server";
...
const ORIGINAL_VALADRIEN_OS_HOME = process.env.VALADRIEN_OS_HOME;
const ORIGINAL_VALADRIEN_OS_INSTANCE_ID = process.env.VALADRIEN_OS_INSTANCE_ID;
...
afterEach(() => {
  ...
  if (ORIGINAL_VALADRIEN_OS_HOME === undefined) {
    delete process.env.VALADRIEN_OS_HOME;
  } else {
    process.env.VALADRIEN_OS_HOME = ORIGINAL_VALADRIEN_OS_HOME;
  }
  if (ORIGINAL_VALADRIEN_OS_INSTANCE_ID === undefined) {
    delete process.env.VALADRIEN_OS_INSTANCE_ID;
  } else {
    process.env.VALADRIEN_OS_INSTANCE_ID = ORIGINAL_VALADRIEN_OS_INSTANCE_ID;
  }
});
...
describe("claude_local environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `valadrien-os-claude-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
    ...
    expect(
      result.checks.some(
        (check) =>
          ...
          check.message === "Working directory is valid: /srv/valadrien-os/workspace",
      ),
    ).toBe(true);
  ...
  it("uses the managed Claude config seed for sandbox hello probes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-claude-envtest-managed-config-"));
...
if (!configDir.includes(".valadrien-os-runtime/claude/config")) {
...
`, "utf8");
    ...
    process.env.VALADRIEN_OS_HOME = path.join(root, "valadrien-os-home");
    process.env.VALADRIEN_OS_INSTANCE_ID = "test-instance";
  ...
  it("warns and omits --effort for sandbox probes when the installed Claude CLI does not advertise it", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-claude-envtest-sandbox-effort-"));
