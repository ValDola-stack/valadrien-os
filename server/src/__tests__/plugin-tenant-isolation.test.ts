...
describeEmbeddedPostgres("plugin tenant isolation (company_id FK)", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-plugin-tenant-isolation-");
  ...
  async function seedPlugin() {
    ...
    await db.insert(plugins).values({
      id: pluginId,
      pluginKey: "valadrien-os.tenant-isolation-test",
      packageName: "@valadrien-os/plugin-tenant-isolation-test",
      version: "0.0.1",
      apiVersion: 1,
      categories: ["automation"],
      manifestJson: {
        id: "valadrien-os.tenant-isolation-test",
        apiVersion: 1,
        version: "0.0.1",
