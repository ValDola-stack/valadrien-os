...
import type { CatalogTeam } from "@valadrien-os/shared";
...
describe("teamsCatalogService", () => {
  beforeEach(() => {
    ...
    mockCompanyPortabilityService.importBundle.mockResolvedValue({
      company: { id: "company-1", name: "ValadrienOs", action: "unchanged" },
      agents: [],
      projects: [],
      envInputs: [],
      warnings: [],
    ...
    mockCompanySkillService.installFromCatalog.mockResolvedValue({
      action: "created",
      skill: { key: "paperclipai/bundled/valadrien-os-operations/task-planning" },
      catalogSkill: { id: "paperclipai:bundled:valadrien-os-operations:task-planning" },
      warnings: [],
    });
  ...
  it("builds an inline portability source with catalog skill keys and target-manager reparenting", async () => {
    ...
    expect(prepared.source.files["agents/ceo/AGENTS.md"]).toEqual(expect.stringContaining("paperclipai/bundled/valadrien-os-operations/task-planning"));
    ...
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentId: \"manager-1\""));
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentSlug: \"engineering-manager\""));
  });
  ...
  it("resolves target-manager slug against same-company agents before rendering reparent metadata", async () => {
    ...
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentId: \"manager-1\""));
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("reportsToExistingAgentSlug: \"ceo\""));
  });

  it("preserves package-declared ValadrienOs sidecar permissions while adding generated catalog provenance", async () => {
    const svc = teamsCatalogService({} as any);

    const prepared = await svc.prepareCatalogTeamSource("company-1", "product-engineering");

    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("permissions:"));
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("canCreateAgents: true"));
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("catalogTeam:"));
    expect(prepared.source.files[".valadrien-os.yaml"]).toEqual(expect.stringContaining("catalogSlug: \"product-engineering\""));
  });
  ...
  it("uses the configured safe adapter default for bundled agents", async () => {
    const previousDefault = process.env.VALADRIEN_OS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE;
    process.env.VALADRIEN_OS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE = "opencode_local";
    try {
      const svc = teamsCatalogService({} as any);
      ...
      if (previousDefault === undefined) {
        delete process.env.VALADRIEN_OS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE;
      } else {
        process.env.VALADRIEN_OS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE = previousDefault;
      }
    }
  });
  ...
  describe("readCatalogTeamProvenance", () => {
    it("reads catalogTeam provenance from agent metadata", () => {
      expect(
        readCatalogTeamProvenance({
          valadrienOs: { catalogTeam: { catalogId: "team-x", catalogKey: "k", originHash: "sha256:1" } },
        }),
    ...
    it("returns null when there is no catalogTeam provenance", () => {
      ...
      expect(readCatalogTeamProvenance({ valadrienOs: { catalog: { skillKey: "s" } } })).toBeNull();
      expect(readCatalogTeamProvenance({ valadrienOs: { catalogTeam: { originHash: "h" } } })).toBeNull();
    });
  });
      ...
      mockAgentService.list.mockResolvedValue([
        {
          id: "removed",
          companyId: "company-1",
          metadata: { valadrienOs: { catalogTeam: { catalogId: "paperclipai:bundled:gone:removed", originHash: "sha256:x" } } },
        },
      ]);
      const svc = teamsCatalogService({} as any);
