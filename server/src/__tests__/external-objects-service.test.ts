...
import { canonicalizeExternalObjectUrl } from "@valadrien-os/shared/external-objects-server";
...
describeEmbeddedPostgres("externalObjectService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("valadrien-os-external-objects-");
  ...
  it("uses a mock plugin provider to detect and resolve non-GitHub objects", async () => {
    ...
    const manifest: ValadrienOsPluginManifestV1 = {
      id: "valadrien-os.mock-object-provider",
      apiVersion: 1,
      version: "1.0.0",
    ...
    const [plugin] = await db.insert(plugins).values({
      ...
      packageName: "@valadrien-os/mock-object-provider",
