...
async function createGitRepo() {
  ...
  await runGit(repoRoot, ["config", "user.email", "valadrien-os-test@example.com"]);
...
async function createGitRepoWithOrigin() {
  ...
  const originRoot = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-accepted-plan-origin-"));
...
describeEmbeddedPostgres("accepted plan workspace refresh", () => {
  ...
  it("keeps accepted-plan children strategy-only until first realization after the base ref moves", async () => {
    ...
    adapterExecute.mockImplementationOnce(async (input) => {
      ...
      const workspace = context.valadrienOsWorkspace as Record<string, unknown> | undefined;
  ...
  it("forces a fresh session and suppresses accepted-plan continuation when another issue owns the in-flight claim", async () => {
    ...
    expect(adapterInput.context.valadrienOsTaskMarkdown).toContain("Make the plan only.");
    expect(adapterInput.context.valadrienOsTaskMarkdown).not.toContain("Create child issues from the approved plan only");
  }, 20_000);
  ...
  it("guards cross-issue accepted-plan retries even when the waking issue is standard work mode", async () => {
    ...
    expect(adapterInput.context.valadrienOsTaskMarkdown).toContain("Issue: \"PAP-9401\"");
    expect(adapterInput.context.valadrienOsTaskMarkdown).not.toContain("Create child issues from the approved plan only");
  }, 20_000);
  ...
  it("preserves accepted-plan continuation resume state when the wake issue owns the in-flight claim", async () => {
    ...
    expect(adapterInput.context.valadrienOsTaskMarkdown).toContain("Create child issues from the approved plan only");
  }, 20_000);
});
