...
describe("agent local JWT", () => {
  ...
  const disableLegacyFallbackEnv = "VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK";
  const instanceIdEnv = "VALADRIEN_OS_INSTANCE_ID";
  ...
  // VALADRIEN_OS_INSTANCE_ID but deliberately shares VALADRIEN_OS_AGENT_JWT_SECRET
  ...
  it("defaults TTL to 1h when VALADRIEN_OS_AGENT_JWT_TTL_SECONDS is unset", () => {
  ...
  it("accepts master-secret-signed tokens when VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK is unset", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    delete process.env[disableLegacyFallbackEnv];
  ...
  it("rejects master-secret-signed tokens when VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK is enabled", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    process.env[disableLegacyFallbackEnv] = "true";
    const legacyToken = craftLegacyMasterSecretToken(process.env[secretEnv]!, "company-legacy");
    expect(verifyLocalAgentJwt(legacyToken)).toBeNull();
  ...
  it("still verifies per-company-signed tokens when VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK is enabled", () => {
    vi.setSystemTime(new Date("2026-01-01T00:00:00.000Z"));
    process.env[disableLegacyFallbackEnv] = "true";
    const token = createLocalAgentJwt("agent-1", "company-1", "claude_local", "run-1");
