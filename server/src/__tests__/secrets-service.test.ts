...
describeEmbeddedPostgres("secretService", () => {
  ...
  it("reports current-user secret rollback failures when AWS create cleanup cannot remove the reserved row", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockRejectedValueOnce(
      new SecretProviderClientError({
        ...
        rawMessage:
          "AccessDeniedException: arn:aws:sts::123456789012:assumed-role/prod/ValadrienOs cannot create secret",
      }),
    );
    ...
    await expect(
      ...
      status: 500,
      message: "Secret create failed and ValadrienOs could not roll back the local secret reservation.",
      details: {
  ...
  it("reports current-user secret persistence rollback failures when local cleanup cannot remove the reserved row", async () => {
    ...
    const externalRef =
      "arn:aws:secretsmanager:us-east-1:123456789012:secret:valadrien-os/prod-use1/user/github-token";
  ...
  it("removes user secret values and provider material when deleting a definition", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockImplementation(async (input) => {
      nextVersion += 1;
      const externalRef =
        `arn:aws:secretsmanager:us-east-1:123456789012:secret:valadrien-os/prod-use1/${input.context.secretKey}`;
      return {
        material: {
          scheme: "aws_secrets_manager_v1",
          secretId: externalRef,
  ...
  it("removes user secret values and provider material when update deletes a definition", async () => {
    ...
    vi.spyOn(awsSecretsManagerProvider, "createSecret").mockResolvedValue({
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId: "arn:aws:secretsmanager:us-east-1:123456789012:secret:valadrien-os/prod-use1/user-secret",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
      fingerprintSha256: "fingerprint-sha-1",
      externalRef: "arn:aws:secretsmanager:us-east-1:123456789012:secret:valadrien-os/prod-use1/user-secret",
      providerVersionRef: "aws-version-1",
    });
    const deleteSpy = vi.spyOn(awsSecretsManagerProvider, "deleteOrArchive").mockResolvedValue();
  ...
  it("keeps a local cleanup handle when create rollback cleanup fails", async () => {
    ...
    await expect(
      ...
      status: 500,
      message: "Secret create failed and ValadrienOs could not clean up the remote provider secret.",
      details: {
  ...
  it("reports managed secret persistence rollback failures when local cleanup cannot remove the reserved row", async () => {
    ...
    const prepared = {
      material: {
        scheme: "aws_secrets_manager_v1",
        secretId:
          "arn:aws:secretsmanager:us-east-1:123456789012:secret:valadrien-os/prod-use1/company/create-local-cleanup",
        versionId: "aws-version-1",
        source: "managed",
      },
      valueSha256: "value-sha-1",
  ...
  it("sanitizes AWS managed secret create failures and removes the reserved row", async () => {
    ...
    const rawProviderMessage =
      "AccessDeniedException: User: arn:aws:sts::123456789012:assumed-role/prod/ValadrienOs is not authorized to perform secretsmanager:CreateSecret on arn:aws:secretsmanager:us-east-1:123456789012:secret:valadrien-os/prod-use1";
  ...
  it("sanitizes AWS provider vault discovery errors before crossing the service boundary", async () => {
    ...
    expect(thrown).toMatchObject({
      ...
      details: {
        ...
        region: "us-east-1",
        credentialPath: "ValadrienOs server runtime/provider credential path",
        requiredCapability: "secretsmanager:ListSecrets",
        actionableMessage:
          "AWS discovery preview needs secretsmanager:ListSecrets in the selected region for the ValadrienOs server runtime/provider credential path.",
        safeAlternative:
