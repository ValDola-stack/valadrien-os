...
describeEmbeddedPostgres("heartbeat plugin environments", () => {
  ...
  it("inherits the instance default environment across companies while preserving explicit agent overrides", async () => {
    ...
    const workspaceRootA = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-plugin-env-company-a-"));
    const workspaceRootB = await mkdtemp(path.join(os.tmpdir(), "valadrien-os-plugin-env-company-b-"));
