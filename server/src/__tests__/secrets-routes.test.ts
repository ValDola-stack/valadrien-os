...
describe("secret routes", () => {
  ...
  it("previews provider vault discovery and logs only aggregate metadata", async () => {
      ...
      candidates: [
        {
          ...
          sampleCount: 2,
          samples: [
            { name: "valadrien-os/prod-use1/company-1/openai", hasKmsKey: false, tagKeys: ["environment"] },
          ],
          signals: {
            namespace: "prod-use1",
            secretNamePrefix: "valadrien-os",
    ...
    expect(JSON.stringify(mockLogActivity.mock.calls)).not.toContain("valadrien-os/prod-use1/company-1/openai");
  });
  ...
  it("returns actionable sanitized provider vault discovery errors", async () => {
    ...
    expect(res.body.details.actionableMessage).toContain("ValadrienOs server runtime/provider credential path");
