...
describe("claude local skill sync", () => {
  ...
  it("reports built-in ValadrienOs skills as available when no explicit selection exists", async () => {
    ...
    expect(snapshot.entries.find((entry) => entry.key === valadrienOsKey)?.state).toBe("available");
  });
  ...
  it("normalizes legacy flat ValadrienOs skill refs to canonical keys", async () => {
    const snapshot = await listClaudeSkills({
      agentId: "agent-3",
      companyId: "company-1",
      adapterType: "claude_local",
  ...
  it("shows host-level user-installed Claude skills as read-only external entries", async () => {
    const home = await makeTempDir("valadrien-os-claude-user-skills-");
    ...
    expect(snapshot.entries).toContainEqual(expect.objectContaining({
      ...
      readOnly: true,
      detail: "Installed outside ValadrienOs management in the Claude skills home.",
    }));
  });
});
