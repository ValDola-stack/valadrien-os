...
function applyDocumentFixups(document: any): any {
  ...
  document.components.securitySchemes = {
    [BOARD_SESSION_AUTH_SCHEME]: {
      type: "apiKey",
      in: "cookie",
      name: "valadrien_os_session",
      description:
        "Board session cookie in authenticated mode. ValadrienOs uses Better Auth; cookie transport may vary by deployment.",
    },
    ...
    [AGENT_BEARER_AUTH_SCHEME]: {
      ...
      description:
        "Agent API key or ValadrienOs-issued local agent JWT presented in the Authorization bearer header.",
    },
  };
  ...
  for (const [path, pathItem] of Object.entries(document.paths ?? {})) {
    for (const [method, operation] of Object.entries(pathItem as Record<string, any>)) {
      ...
      operation["x-valadrien-os-authorization"] =
...
export function buildOpenApiDocument(): any {
  return applyDocumentFixups({
    openapi: "3.0.0",
    info: {
      title: "ValadrienOs API",
      version: "1.0.0",
      description: "REST API for the ValadrienOs AI agent management platform",
    },
    servers: [{ url: "/" }],
