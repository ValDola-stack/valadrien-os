...
function requiresValadrienOsAttachmentMetadata(input: {
  type?: unknown;
  provider?: unknown;
}, fallback?: {
  type?: string | null;
  ...
  return type === "artifact" && provider === "valadrien-os";
}
  ...
  if (existing) {
    ...
    if (
      ...
      const event: IssueListStormEvent = {
        ...
        visibilityHint: input.req.header("x-valadrien-os-tab-visible") ?? null,
      };
...
export function issueRoutes(
  ...
  async function resolveRunIssueWorkspaceInheritanceSource(
    ...
    const valadrienOsIssue = context.valadrienOsIssue && typeof context.valadrienOsIssue === "object"
      ? context.valadrienOsIssue as Record<string, unknown>
      : null;
    return readNonEmptyString(context.issueId) ?? readNonEmptyString(valadrienOsIssue?.id);
  }
  ...
  async function canonicalizeValadrienOsArtifactMetadata(input: {
    issue: { id: string; companyId: string };
  ...
  router.get("/companies/:companyId/issues", async (req, res) => {
    ...
    res.setHeader("X-ValadrienOs-Request-Cache", coordinated.cacheStatus);
  ...
  router.post("/issues/:id/work-products", validate(createIssueWorkProductSchema), async (req, res) => {
    ...
    if (requiresValadrienOsAttachmentMetadata(createInput)) {
      createInput.metadata = await canonicalizeValadrienOsArtifactMetadata({
        issue,
  ...
  router.patch("/work-products/:id", validate(updateIssueWorkProductSchema), async (req, res) => {
    ...
    if (requiresValadrienOsAttachmentMetadata(patch, existing)) {
      if (patch.metadata !== undefined) {
        patch.metadata = await canonicalizeValadrienOsArtifactMetadata({
          issue,
          metadata: patch.metadata ?? null,
        });
      } else if (!requiresValadrienOsAttachmentMetadata(existing)) {
