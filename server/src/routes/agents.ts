...
import { agents as agentsTable, companies, heartbeatRuns, issues as issuesTable, projects as projectsTable } from "@valadrien-os/db";
...
import { trackAgentCreated } from "@valadrien-os/shared/telemetry";
...
import { runClaudeLogin } from "@valadrien-os/adapter-claude-local/server";
import { DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX } from "@valadrien-os/adapter-codex-local";
...
function readRunIssueId(context: Record<string, unknown> | null) {
  ...
  const valadrienOsIssue = readObject(context?.valadrienOsIssue);
  const nestedIssueId = valadrienOsIssue?.id;
...
export function agentRoutes(
   ...
   *   adapter probes the ValadrienOs host (legacy behavior).
  ...
  function buildSandboxIdentityCheck(input: {
    ...
    const detailParts = [
      `valadrienOsLeaseId=${input.lease.id}`,
  ...
  function codexLocalAgentHome(companyId: string, agentId: string): string {
    const instanceRoot = resolveValadrienOsInstanceRootForAdapter({
      homeDir: asNonEmptyString(process.env.VALADRIEN_OS_HOME) ?? undefined,
      instanceId: asNonEmptyString(process.env.VALADRIEN_OS_INSTANCE_ID) ?? undefined,
      env: process.env,
    });
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeValadrienOsSkillSyncPreference(adapterConfig, desiredSkillEntries),
      desiredSkills,
      desiredSkillEntries,
      runtimeSkillEntries,
    };
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readValadrienOsSkillSyncPreference(
  ...
  router.get("/agents/me/inbox-lite", async (req, res) => {
    ...
    const isWorktreeRuntime = isTruthyRuntimeEnvValue(process.env.VALADRIEN_OS_IN_WORKTREE);
