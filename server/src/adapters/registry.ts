...
} from "@valadrien-os/adapter-claude-local";
import {
...
} from "@valadrien-os/adapter-cursor-local";
import {
...
} from "@valadrien-os/adapter-cursor-cloud/server";
import { agentConfigurationDoc as cursorCloudAgentConfigurationDoc } from "@valadrien-os/adapter-cursor-cloud";
import {
...
} from "@valadrien-os/adapter-gemini-local";
import {
...
} from "@valadrien-os/adapter-grok-local/server";
import {
...
} from "@valadrien-os/adapter-opencode-local";
import {
  execute as openclawGatewayExecute,
  testEnvironment as openclawGatewayTestEnvironment,
} from "@valadrien-os/adapter-openclaw-gateway/server";
import {
  agentConfigurationDoc as openclawGatewayAgentConfigurationDoc,
  models as openclawGatewayModels,
} from "@valadrien-os/adapter-openclaw-gateway";
...
const externalAdaptersReady: Promise<void> = (async () => {
  try {
    ...
    for (const externalAdapter of externalAdapters) {
      ...
      if (overriding) {
        console.log(
          `[valadrien-os] External adapter "${externalAdapter.type}" overrides built-in adapter`,
        );
    ...
    console.error("[valadrien-os] Failed to load external adapters:", err);
  }
})();
...
export function setOverridePaused(type: string, paused: boolean): boolean {
  ...
  if (paused && !wasPaused) {
    ...
    console.log(`[valadrien-os] Override paused for "${type}" — builtin adapter restored`);
    return true;
  }
  if (!paused && wasPaused) {
    pausedOverrides.delete(type);
    console.log(`[valadrien-os] Override resumed for "${type}" — external adapter active`);
    return true;
  }
  return false;
}
