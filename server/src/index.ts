...
export async function startServer(): Promise<StartedServer> {
  ...
  const databaseBackupMaxAgeHours = Math.max(
    1,
    Number(process.env.VALADRIEN_OS_DB_BACKUP_MAX_AGE_HOURS) ||
      Math.max(26, Math.ceil((config.databaseBackupIntervalMinutes / 60) * 2)),
  );
  const databaseBackupAlertFile =
    process.env.VALADRIEN_OS_DB_BACKUP_ALERT_FILE ||
  ...
  // env (VALADRIEN_OS_EXECUTION_MODE=kubernetes). Runs BEFORE the heartbeat resumes
  // queued runs so the policy + managed k8s environments are in place. A bad
  // VALADRIEN_OS_EXECUTION_MODE / VALADRIEN_OS_K8S_* value throws and fails startup
  ...
  // set (VALADRIEN_OS_ADAPTERS). Must run after external adapters are loaded so the
  ...
  try {
    ...
    logger.error({ err }, "failed to reconcile adapter availability from VALADRIEN_OS_ADAPTERS");
    throw err;
  }
