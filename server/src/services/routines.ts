...
import { trackRoutineRun } from "@valadrien-os/shared/telemetry";
...
function resolveRoutineVariableValues(
  ...
  for (const variable of variables) {
    ...
    // ValadrienOs manages from execution context, so callers cannot override them.
...
export function routineService(
  ...
  async function getAutomaticRoutineDispatchEligibility(
    ...
    if (!isTruthyRuntimeEnvValue(runtimeEnv.VALADRIEN_OS_IN_WORKTREE)) return { eligible: true };
  ...
  return {
    ...
    create: async (companyId: string, input: CreateRoutine, actor: Actor): Promise<Routine> => {
      ...
      const env = input.env === undefined || input.env === null
        ...
        : await secretsSvc.normalizeEnvBindingsForPersistence(companyId, input.env, {
            strictMode: process.env.VALADRIEN_OS_SECRETS_STRICT_MODE === "true",
            fieldPath: "env",
          });
    ...
    createTrigger: async (
      ...
      if (input.kind === "webhook") {
        ...
        secretMaterial = {
          webhookUrl: `${process.env.VALADRIEN_OS_API_URL}/api/routine-triggers/public/${publicId}/fire`,
    ...
    rotateTriggerSecret: async (
      ...
      return {
        trigger: trigger as RoutineTrigger,
        secretMaterial: {
          webhookUrl: `${process.env.VALADRIEN_OS_API_URL}/api/routine-triggers/public/${existing.publicId}/fire`,
    ...
    firePublicTrigger: async (publicId: string, input: {
      ...
      if (trigger.signingMode === "none") {
        ...
        // generic X-ValadrienOs-Signature header so operators can use github_hmac
    ...
    tickScheduledTriggers: async (now: Date = new Date()) => {
      const worktreeActivation = isTruthyRuntimeEnvValue(runtimeEnv.VALADRIEN_OS_IN_WORKTREE)
