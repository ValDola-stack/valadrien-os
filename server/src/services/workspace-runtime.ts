...
import { executionWorkspaces, issueComments, issues, projectWorkspaces, workspaceRuntimeServices } from "@valadrien-os/db";
import {
...
function buildDirtyQuarantineRescueBranch(sourceIssue: ExecutionWorkspaceIssueRef | null) {
  ...
  return sanitizeBranchName(`valadrien-os/rescue/${issueComponent}/${formatUtcBranchTimestamp()}`);
}
...
function explainGitWorktreeBranchIncoherence(input: {
  ...
  if (!input.expectedHeadSha || !input.actualHeadSha) {
    return `ValadrienOs could not determine branch ancestry because the recorded branch "${input.expectedBranchName}" or checked-out branch "${actualBranch}" is missing a resolvable HEAD commit.`;
  }
  if (input.sameHead) {
  ...
  if (input.ancestryVerdict === "diverged") {
    return `The recorded branch "${input.expectedBranchName}" is not an ancestor of the checked-out branch "${actualBranch}", so ValadrienOs cannot prove a forward-only reconciliation.`;
  }
  return `ValadrienOs could not determine whether the checked-out branch "${actualBranch}" is forward of the recorded branch "${input.expectedBranchName}".`;
}
  ...
  try {
    ...
    await recordGitOperation(input.recorder, {
      ...
      args: [
        "commit",
        "-m",
        "ValadrienOs dirty workspace rescue",
        "-m",
        [
...
export async function ensureGitWorktreeBranchCoherent(input: {
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The checked-out branch contains the recorded branch plus newer commits, so ValadrienOs adopted it for subsequent runs.`,
      ],
    };
  }
  ...
  if (
    ...
    return {
      ...
      reconciledForward: false,
      warnings: [
        `${warningPrefix} The detached HEAD contained the recorded branch plus newer commits, so ValadrienOs moved the recorded branch to that HEAD.`,
      ],
    };
  }
...
function isValadrienOsDevRuntimeService(input: { serviceName?: string | null; command?: string | null }) {
  ...
  return (
    ...
    || serviceName === "valadrien-os-dev-once"
...
function resolveRuntimeServiceHealthUrl(
  ...
  if (!url || !isValadrienOsDevRuntimeService(input ?? {})) return url;
  try {
