...
export const HEARTBEAT_RUN_SCRATCH_MARKER = ".valadrien-os-run-scratch.json";
...
export async function prepareHeartbeatRunScratch(input: {
  ...
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), `valadrien-os-run-${issueSegment}-${runSegment}-`));
...
export function buildHeartbeatRunScratchEnv(
  ...
  const env: Record<string, string> = {
    VALADRIEN_OS_RUN_SCRATCH_DIR: scratch.dir,
    VALADRIEN_OS_TASK_SCRATCH_DIR: scratch.dir,
    VALADRIEN_OS_SCRATCH_DIR: scratch.dir,
    VALADRIEN_OS_TMPDIR: scratch.dir,
  };
...
export async function cleanupHeartbeatRunScratch(input: {
  ...
  if (!isPathInside(tmpRoot, dir) || !path.basename(dir).startsWith("valadrien-os-run-")) {
