...
describe("parseAdapterRegistryEnv", () => {
  ...
  it("parses inline VALADRIEN_OS_ADAPTERS JSON", () => {
    const r = parseAdapterRegistryEnv({ VALADRIEN_OS_ADAPTERS: ENTRY });
  ...
  it("throws on malformed JSON (fail loud)", () => {
    expect(() => parseAdapterRegistryEnv({ VALADRIEN_OS_ADAPTERS: "{not json" })).toThrow(
      /VALADRIEN_OS_ADAPTERS/,
    );
  });

  it("throws on schema-invalid content (fail loud)", () => {
    expect(() =>
      parseAdapterRegistryEnv({ VALADRIEN_OS_ADAPTERS: JSON.stringify([{ enabled: true }]) }),
    ).toThrow(/VALADRIEN_OS_ADAPTERS/);
  });
});
