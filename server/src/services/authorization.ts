...
import { LOW_TRUST_REVIEW_PRESET, extractAgentMentionIds, isFoundingAgentRole, type LowTrustBoundary } from "@valadrien-os/shared";
import {
...
function responsibleUserSnapshotTtlMs() {
  const raw = process.env.VALADRIEN_OS_RESPONSIBLE_USER_AUTHZ_CACHE_TTL_MS?.trim();
  if (!raw) return 5_000;
...
export function responsibleUserAuthzShadowMode() {
  const mode = process.env.VALADRIEN_OS_RESPONSIBLE_USER_AUTHZ_MODE?.trim().toLowerCase();
  const shadow = process.env.VALADRIEN_OS_RESPONSIBLE_USER_AUTHZ_SHADOW?.trim().toLowerCase();
