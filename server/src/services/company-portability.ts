...
import { builtInManagedResources, principalPermissionGrants, type Db } from "@valadrien-os/db";
import type {
...
import { requireOpenCodeModelId } from "@valadrien-os/adapter-opencode-local/server";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".valadrien-os.yaml" || normalized === ".valadrien-os.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const valadrienOs = isPlainRecord(metadata?.valadrienOs) ? metadata?.valadrienOs as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.valadrienOsSkillKey)
    ?? asString(valadrienOs?.skillKey)
    ?? asString(valadrienOs?.key),
  );
}
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "valadrien-os" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; ValadrienOs will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .valadrien-os.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[valadrienOsExtensionPath];
  if (selectedFiles.has(valadrienOsExtensionPath) && typeof extensionEntry === "string") {
    filtered[valadrienOsExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findValadrienOsExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".valadrien-os.yaml"] === "string") return ".valadrien-os.yaml";
  if (typeof files[".valadrien-os.yml"] === "string") return ".valadrien-os.yml";
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "valadrien_os_bundled") {
    ...
    return {
      ...
      commit,
      trackingRef: "master",
      url: `https://github.com/ValDola-stack/valadrien-os/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "valadrien_os_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.valadrienOsSkillKey = skill.key;
  metadata.valadrienOs = {
    ...(isPlainRecord(metadata.valadrienOs) ? metadata.valadrienOs : {}),
    skillKey: skill.key,
    slug: skill.slug,
...
function buildManifestFromPackageFiles(
  ...
  const valadrienOsExtensionPath = findValadrienOsExtensionPath(normalizedFiles);
  const valadrienOsExtension = valadrienOsExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, valadrienOsExtensionPath) ?? "")
    : {};
  const valadrienOsCompany = isPlainRecord(valadrienOsExtension.company) ? valadrienOsExtension.company : {};
  const valadrienOsSidebar = normalizePortableSidebarOrder(valadrienOsExtension.sidebar);
  const valadrienOsAgents = isPlainRecord(valadrienOsExtension.agents) ? valadrienOsExtension.agents : {};
  const valadrienOsProjects = isPlainRecord(valadrienOsExtension.projects) ? valadrienOsExtension.projects : {};
  const valadrienOsTasks = isPlainRecord(valadrienOsExtension.tasks) ? valadrienOsExtension.tasks : {};
  const valadrienOsRoutines = isPlainRecord(valadrienOsExtension.routines) ? valadrienOsExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(valadrienOsCompany.brandColor),
      logoPath: asString(valadrienOsCompany.logoPath) ?? asString(valadrienOsCompany.logo),
      attachmentMaxBytes:
        typeof valadrienOsCompany.attachmentMaxBytes === "number" && Number.isFinite(valadrienOsCompany.attachmentMaxBytes)
          ? Math.max(1, Math.floor(valadrienOsCompany.attachmentMaxBytes))
          : null,
      requireBoardApprovalForNewAgents:
        typeof valadrienOsCompany.requireBoardApprovalForNewAgents === "boolean"
          ? valadrienOsCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
      feedbackDataSharingEnabled:
        typeof valadrienOsCompany.feedbackDataSharingEnabled === "boolean"
          ? valadrienOsCompany.feedbackDataSharingEnabled
          : false,
      feedbackDataSharingConsentAt:
        typeof valadrienOsCompany.feedbackDataSharingConsentAt === "string"
          ? valadrienOsCompany.feedbackDataSharingConsentAt
          : null,
      feedbackDataSharingConsentByUserId:
        asString(valadrienOsCompany.feedbackDataSharingConsentByUserId),
      feedbackDataSharingTermsVersion:
        asString(valadrienOsCompany.feedbackDataSharingTermsVersion),
    },
    sidebar: valadrienOsSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(valadrienOsAgents[slug]) ? valadrienOsAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(valadrienOsProjects[slug]) ? valadrienOsProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(valadrienOsTasks[slug]) ? valadrienOsTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(valadrienOsRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(valadrienOsRoutines[slug]) ? valadrienOsRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function prepareImportedAgentAdapter(
    ...
    const nextAdapterConfig = writeValadrienOsSkillSyncPreference(
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".valadrien-os.yaml" ||
          relative === ".valadrien-os.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const valadrienOsAgentsOut: Record<string, Record<string, unknown>> = {};
    const valadrienOsProjectsOut: Record<string, Record<string, unknown>> = {};
    const valadrienOsTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const valadrienOsRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readValadrienOsSkillSyncPreference(
        ...
        valadrienOsAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      valadrienOsProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      valadrienOsTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      valadrienOsRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const valadrienOsExtensionPath = ".valadrien-os.yaml";
    const valadrienOsAgents = Object.fromEntries(
      Object.entries(valadrienOsAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const valadrienOsProjects = Object.fromEntries(
      Object.entries(valadrienOsProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const valadrienOsTasks = Object.fromEntries(
      Object.entries(valadrienOsTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const valadrienOsRoutines = Object.fromEntries(
      Object.entries(valadrienOsRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[valadrienOsExtensionPath] = buildYamlFile(
      {
        ...
        agents: Object.keys(valadrienOsAgents).length > 0 ? valadrienOsAgents : undefined,
        projects: Object.keys(valadrienOsProjects).length > 0 ? valadrienOsProjects : undefined,
        tasks: Object.keys(valadrienOsTasks).length > 0 ? valadrienOsTasks : undefined,
        routines: Object.keys(valadrienOsRoutines).length > 0 ? valadrienOsRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, valadrienOsExtensionPath);
