...
export interface KubernetesEnvironmentConfigInput {
   ...
   */
  timeoutMs?: number;
  adapters?: import("@valadrien-os/shared").AdapterRegistryEntry[];
  [key: string]: unknown;
}
...
export function environmentService(db: Db) {
  return {
    ...
    ensureKubernetesEnvironment: async (
      ...
      // "at most one ValadrienOs-managed sandbox row per instance" at the DB
      ...
      const inserted = await db
        ...
        .onConflictDoNothing({
          target: [environments.driver],
          where:
            sql`${environments.driver} = 'sandbox' AND (${environments.metadata} ->> 'managedByValadrienOs')::boolean = true`,
        })
        .returning()
        .then((rows) => rows[0] ?? null)
        .catch((error) => {
