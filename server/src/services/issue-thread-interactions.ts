...
import { trackInteractionResolved } from "@valadrien-os/shared/telemetry";
import type {
  ...
  try {
    ...
    if (!args?.creatorRoleByAgentId) {
      try {
        ...
        console.error("[valadrien-os] Failed to load interaction.resolved creator role", error);
      }
    }
    ...
    console.error("[valadrien-os] Failed to emit interaction.resolved telemetry", error);
  }
}
...
async function emitResolvedInteractionsTelemetry(
  ...
  try {
    ...
    console.error("[valadrien-os] Failed to load interaction.resolved creator roles", error);
  }
