...
export function readPortableCatalogProvenance(
  ...
  const valadrienOs = isCatalogRecord(metadata?.valadrienOs) ? metadata.valadrienOs : null;
  const catalog = isCatalogRecord(valadrienOs?.catalog) ? valadrienOs.catalog : null;
