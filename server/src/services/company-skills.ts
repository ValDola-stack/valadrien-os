...
import { readValadrienOsSkillSyncPreference, writeValadrienOsSkillSyncPreference } from "@valadrien-os/adapter-utils/server-utils";
import type { ValadrienOsDesiredSkillEntry, ValadrienOsSkillEntry } from "@valadrien-os/adapter-utils/server-utils";
import type {
...
import { isUuidLike, normalizeAgentUrlKey, parseFrontmatterMarkdown } from "@valadrien-os/shared";
...
function assertImportedSkillKeyAllowed(skill: ImportedSkill) {
  ...
  if (sourceKind === "valadrien_os_bundled") return;
  throw unprocessable(
    `Reserved ValadrienOs skill key "${skill.key}" cannot be imported from unbundled sources.`,
    {
      skillKey: skill.key,
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("ValDola-stack/valadrien-os/")) return slug;
...
function isValadrienOsBundledSkillKey(key: string) {
  return key.startsWith("paperclipai/paperclip/");
}

function stripDerivedValadrienOsBundledMetadata(key: string, metadata: unknown): unknown {
  ...
  if (out.sourceKind === "valadrien_os_bundled") delete out.sourceKind;
...
function importedSkillMetadataEqual(existing: CompanySkill, values: ImportedSkillPersistValues) {
  ...
  if (isValadrienOsBundledSkillKey(values.key) && asString(incomingMetadata?.sourceKind) === "valadrien_os_bundled") {
    return JSON.stringify(stripDerivedValadrienOsBundledMetadata(existing.key, existing.metadata))
      === JSON.stringify(stripDerivedValadrienOsBundledMetadata(values.key, values.metadata));
  }
...
const BUILT_IN_SKILL_TEST_RUN_TEMPLATE_BODY = [
  ...
  "",
  "Invoke and use the selected skill under test: `{{skillInvocation}}`. Use the pinned skill revision supplied by ValadrienOs as the source of truth, regardless of any other runtime skills.",
  "",
...
function builtInSkillTestRunTemplate(companyId: string): CompanySkillTestRunTemplate {
  return {
    ...
    description: "ValadrienOs's read-only default harness instructions for Skills Studio runs.",
...
function normalizeRequestedDesiredSkillSelection(value: string | AgentDesiredSkillEntry): ValadrienOsDesiredSkillEntry {
...
export interface ResolvedRequestedSkillEntries {
  ...
  resolved: ValadrienOsDesiredSkillEntry[];
  /**
...
async function resolveRequestedSkillEntriesOrThrow(
  ...
  const missing = new Set<string>();
  const ambiguous = new Set<string>();
  const resolved = new Map<string, ValadrienOsDesiredSkillEntry>();
...
function resolveDesiredSkillEntries(
  ...
  const preference = readValadrienOsSkillSyncPreference(config);
  const out = new Map<string, ValadrienOsDesiredSkillEntry>();
...
function buildMissingRuntimeSourceDetail(skill: Pick<CompanySkill, "name" | "sourceLocator" | "metadata">) {
  ...
  if (sourcePath) {
    return `Company skill "${skill.name}" is in the library, but ValadrienOs cannot find its local source at ${sourcePath}.`;
  }
  return `Company skill "${skill.name}" is in the library, but ValadrienOs cannot find a valid local runtime source for it.`;
}
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveValadrienOsInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: SkillSourceInfoTarget): {
  ...
  if (metadata.sourceKind === "valadrien_os_bundled") {
    return {
      editable: false,
      editableReason: "Bundled ValadrienOs skills are read-only.",
      sourceLabel: "ValadrienOs bundled",
  ...
  if (skill.sourceType === "local_path") {
    ...
    if (localSkillDir && localSkillDir.startsWith(managedRoot)) {
      return {
        editable: true,
        editableReason: null,
        sourceLabel: "ValadrienOs workspace",
        sourceBadge: "valadrien-os",
...
export function companySkillService(db: Db) {
  ...
  async function reconcileLocalPathSkillSources(companyId: string) {
    ...
    for (const skill of skills) {
      if (skill.sourceType !== "local_path") continue;
      if (isValadrienOsBundledSkillKey(skill.key) || asString(skill.metadata?.sourceKind) === "valadrien_os_bundled") continue;
    ...
    await db.transaction(async (tx) => {
      for (const item of planned) {
        ...
        const updated = await tx
          .update(agentsTable)
          .set({
            adapterConfig: writeValadrienOsSkillSyncPreference(adapterConfig, nextEntries),
            updatedAt: new Date(),
          })
          .where(and(eq(agentsTable.companyId, companyId), eq(agentsTable.id, item.agentId)))
  ...
  async function installFromCatalog(
    ...
    const values = {
      ...
      authorName: storeMetadata.authorName ?? existingByKey?.authorName ?? "ValadrienOs",
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "valadrien_os_bundled"
        && incomingKind === "github"
        && incomingOwner === "valadrien-os"
        && incomingRepo === "valadrien-os"
      ) {
        out.push(existing);
        continue;
      }
  ...
  function snapshotAgentConfig(agent: Awaited<ReturnType<typeof agents.getById>>) {
    ...
    return {
      ...
      adapterConfig,
      runtimeConfig,
      assignedSkills: isPlainRecord(adapterConfig.valadrienOsSkillSync)
        ? adapterConfig.valadrienOsSkillSync
        : null,
      instructionsRef:
