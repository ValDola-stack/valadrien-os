...
import type { IssueWatchdog, IssueWatchdogSummary } from "@valadrien-os/shared";
...
function issueIdFromWakePayload(payload: unknown) {
  ...
  const nested = parseObject(parsed._valadrienOsWakeContext);
      ...
      db
        ...
        .where(and(
          ...
          or(
            ...
            inArray(sql`${agentWakeupRequests.payload}->'_valadrienOsWakeContext'->>'issueId'`, subtreeIssueIds),
            inArray(sql`${agentWakeupRequests.payload}->'_valadrienOsWakeContext'->>'taskId'`, subtreeIssueIds),
          ),
        )),
      db
        .select({
      ...
      db
        ...
        .where(and(
          ...
          sql`(${agentWakeupRequests.payload}->>'issueId' = ${issueId}
            ...
            OR ${agentWakeupRequests.payload}->'_valadrienOsWakeContext'->>'issueId' = ${issueId}
            OR ${agentWakeupRequests.payload}->'_valadrienOsWakeContext'->>'taskId' = ${issueId})`,
        ))
        .limit(1)
        .then((rows) => rows[0] ?? null),
    ]);
