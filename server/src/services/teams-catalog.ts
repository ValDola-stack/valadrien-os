...
import { normalizeAgentUrlKey } from "@valadrien-os/shared";
import { parseFrontmatterMarkdown } from "@valadrien-os/shared/frontmatter";
...
function buildCatalogPackageRootCandidates() {
  const configuredRoot = process.env.VALADRIEN_OS_TEAMS_CATALOG_DIR?.trim();
  const candidates = [
...
async function statCatalogManifest() {
  ...
  throw new Error(
    `Teams catalog manifest not found. Checked: ${catalogPackageRootCandidates.map((root) => path.join(root, "generated/catalog.json")).join(", ")}. Run pnpm --filter @valadrien-os/teams-catalog build:manifest.`,
  );
}
...
/**
 * Extract `metadata.valadrien-os.catalogTeam` provenance written by the team
...
export function readCatalogTeamProvenance(
  ...
  const valadrienOs = isPlainRecord(metadata.valadrienOs) ? metadata.valadrienOs : null;
  const catalogTeam = valadrienOs && isPlainRecord(valadrienOs.catalogTeam) ? valadrienOs.catalogTeam : null;
...
function defaultSafeCatalogAdapterType() {
  return process.env.VALADRIEN_OS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE?.trim() || FALLBACK_SAFE_CATALOG_ADAPTER_TYPE;
}
...
export function teamsCatalogService(db: Db) {
  ...
  async function prepareCatalogTeamSource(
    ...
    const existingExtension =
      typeof files[".valadrien-os.yaml"] === "string"
        ? parseYamlDocument(files[".valadrien-os.yaml"])
        : {};
    const generatedExtension = parseYamlDocument(await renderCatalogProvenanceYaml(team, targetManager));
    files[".valadrien-os.yaml"] = renderYamlFile(mergePlainRecords(existingExtension, generatedExtension));
  ...
  async function installCatalogTeam(
    ...
    const warnings = [
      ...
      ...(defaultedAdapterSlugs.length > 0
        ? [
            `Catalog agents without explicit overrides (${defaultedAdapterSlugs.join(", ")}) default to ${defaultAdapterType}. Pass adapterOverrides or VALADRIEN_OS_TEAMS_CATALOG_DEFAULT_ADAPTER_TYPE to use a different supported adapter.`,
          ]
        : []),
    ];
