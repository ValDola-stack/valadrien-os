...
function buildExecutionReviewParticipantUnavailableComment(latestRun: LatestIssueRun) {
  const failureSummary = summarizeRunFailureForIssueComment(latestRun);
  return (
    "ValadrienOs cannot continue the pending execution-review participant because the participant is not invokable " +
  ...
  function buildStrandedIssueRecoveryDescription(input: {
    ...
    return [
      isReviewParticipantRecovery
        ? "ValadrienOs exhausted automatic recovery for a pending execution-review participant and created this explicit recovery task."
  ...
  async function resolveContinuationWaitingOnReview(issue: typeof issues.$inferSelect) {
    ...
    await issuesSvc.addComment(
      ...
      `This task is waiting on ${waitingOn} to finish. ` +
        ...
        "ValadrienOs turned that into a normal dependency wait instead of flagging it as stuck.)",
      {},
      { authorType: "system" },
    );
    await logActivity(db, {
      ...
      if (isUnsuccessfulTerminalIssueRun(latestRun)) {
        ...
        if (classification.kind === "non_retryable") {
          ...
          const updated = await escalateStrandedAssignedIssue({
            ...
            latestRun,
            comment:
              "ValadrienOs detected a non-retryable failure on this issue's continuation run " +
