...
async function throwProviderWriteOrReservedRowRollbackError(input: {
  ...
  try {
    ...
    throw new HttpError(500, "Secret create failed and ValadrienOs could not roll back the local secret reservation.", {
...
function throwProviderCleanupFailedAfterCreateRollback(input: {
  ...
  const providerConfigId = providerConfigIdentifier(input);
  throw new HttpError(500, "Secret create failed and ValadrienOs could not clean up the remote provider secret.", {
...
function safeRemoteProviderErrorDetails(
  ...
  if (
    ...
    details.credentialPath = "ValadrienOs server runtime/provider credential path";
    if (error?.code === "access_denied") {
      if (context.operation === "secret.create") {
        ...
        details.safeAlternative =
          "If the secret already exists in AWS, link it as an external reference instead of creating a ValadrienOs-managed value.";
...
export function secretService(db: Db) {
  ...
  async function createUserSecretValueInternal(
    ...
    const managedMode = definition.managedMode as "valadrien_os_managed" | "external_reference";
      ...
      try {
        return await db
          ...
          .values({
            ...
            managedMode: input.managedMode ?? "valadrien_os_managed",
