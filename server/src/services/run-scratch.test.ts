...
describe("heartbeat run scratch cleanup", () => {
  ...
  it("preserves valadrien-os-named directories without the ownership marker", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "valadrien-os-run-unmarked-"));
    cleanupDirs.add(dir);
  ...
  it("builds explicit scratch env without clobbering configured temp dirs", async () => {
    ...
    expect(result.env.VALADRIEN_OS_RUN_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.VALADRIEN_OS_TASK_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.VALADRIEN_OS_SCRATCH_DIR).toBe(scratch.dir);
    expect(result.env.VALADRIEN_OS_TMPDIR).toBe(scratch.dir);
