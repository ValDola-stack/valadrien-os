...
import { agents, builtInManagedResources, companies, issueThreadInteractions, issues, routines, routineTriggers } from "@valadrien-os/db";
import { syncRoutineVariablesWithTemplate } from "@valadrien-os/shared";
import type { Agent, Approval, CompanySkill, PermissionKey, Routine, RoutineTrigger, RoutineVariable } from "@valadrien-os/shared";
...
const FALLBACK_REFLECTION_COACH_INSTRUCTIONS = [
  "# Reflection Coach",
  "",
  "You are ValadrienOs's built-in Reflection Coach.",
  ...
  "Do not apply changes in the same run. Present a reviewable diff and wait for the required ValadrienOs issue-thread approval before any follow-up applies it.",
  "",
].join("\n");
...
const FALLBACK_REFLECTION_COACH_SKILL = [
  ...
  "key: paperclipai/bundled/valadrien-os-operations/reflection-coach",
  "---",
  "",
  "# Reflection Coach",
  "",
...
export function readBuiltInTextWithFallback(
    ...
    try {
      ...
      if (code !== "ENOENT") {
        ...
        if (!warnedBuiltInTextReadErrors.has(warningKey)) {
          ...
          console.warn(
            "[valadrien-os] Built-in agent asset " + label + " read error on " + candidatePath + ": " + codeLabel,
          );
        }
      }
  ...
  if (!warnedBuiltInTextFallbacks.has(label)) {
    ...
    console.warn(
      `[valadrien-os] Built-in agent asset ${label} was not readable; using bundled fallback text. `
...
const skillsCatalogRoot = resolvePackageRoot("@valadrien-os/skills-catalog");
...
const REFLECTION_COACH_SKILL = readBuiltInTextWithFallback(
  ...
  [
    path.resolve(
      moduleDir,
      "../../../packages/skills-catalog/catalog/bundled/valadrien-os-operations/reflection-coach/SKILL.md",
    ),
    ...(skillsCatalogRoot
      ? [path.join(skillsCatalogRoot, "catalog/bundled/valadrien-os-operations/reflection-coach/SKILL.md")]
      : []),
  ],
...
const DEFINITIONS = validateBuiltInAgentDefinitions([
  {
    ...
    defaultInstructions:
      "You are ValadrienOs's built-in Briefs agent. Produce concise, sourced operational briefs that help the board understand current company work, risks, and next actions.",
    defaultRole: "general",
  ...
  },
  {
    ...
    defaultInstructions:
      "You are ValadrienOs's built-in Learning agent. Extract durable lessons from completed work, preserve useful patterns, and keep learning artifacts grounded in source context.",
    defaultRole: "general",
    allowedAdapterTypes: ["codex_local", "claude_local", "gemini_local", "opencode_local", "process"],
    defaultBudgetMonthlyCents: 0,
  },
  {
    ...
    bundle: {
      ...
      skill: {
        ...
        canonicalKey: "paperclipai/bundled/valadrien-os-operations/reflection-coach",
        files: {
...
export function builtInAgentService(db: Db) {
  ...
  async function syncBundledSkillToAgent(agent: Agent, skill: CompanySkill) {
    const desired = readValadrienOsSkillSyncPreference(agent.adapterConfig as Record<string, unknown>).desiredSkillEntries;
    const nextDesired = [
      ...desired.filter((entry) => entry.key !== skill.key),
      { key: skill.key, versionId: skill.currentVersionId ?? null },
    ];
    const adapterConfig = writeValadrienOsSkillSyncPreference(agent.adapterConfig as Record<string, unknown>, nextDesired);
