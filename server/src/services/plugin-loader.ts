...
const STANDALONE_BUNDLED_PLUGIN_SDK_PACKAGE = "@valadrien-os/plugin-sdk";
 ...
 * `@valadrien-os/plugin-kubernetes`) are allowed to read from the
 ...
 * runs in-cluster (e.g. `@valadrien-os/plugin-kubernetes` with inCluster=true)
...
export function resolveDeclaredPluginEntrypoints(
  ...
  for (const key of ["manifest", "worker", "ui"] as const) {
    const relativePath = (valadrienOsPlugin as Record<string, unknown>)[key];
...
function formatLocalPluginManualBuildHint(
  ...
  const autoBuildDisabled = (options.processEnv ?? process.env)["VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD"] === "1"
    ? " Auto-build is disabled by VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD=1."
    : "";
...
export async function ensureLocalPluginBuilt(
  ...
  if (processEnv["VALADRIEN_OS_DISABLE_PLUGIN_AUTOBUILD"] === "1") return;
  if (!isRepoBundledPluginPath(packageRoot, { repoRoot: options.repoRoot })) return;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      const manualBuildHint = localPath
        ...
        : "";
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a ValadrienOs plugin (no manifest found).${manualBuildHint}`,
      );
    }
