...
import { normalizeAgentApiKeyScope, type AgentApiKeyScope } from "@valadrien-os/shared";
...
function jwtConfig() {
  const secret = process.env.VALADRIEN_OS_AGENT_JWT_SECRET?.trim() || process.env.BETTER_AUTH_SECRET?.trim();
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.VALADRIEN_OS_AGENT_JWT_TTL_SECONDS, 60 * 60),
    issuer: process.env.VALADRIEN_OS_AGENT_JWT_ISSUER ?? "valadrien-os",
    audience: process.env.VALADRIEN_OS_AGENT_JWT_AUDIENCE ?? "valadrien-os-api",
    ...
    // name) even though it deliberately shares VALADRIEN_OS_AGENT_JWT_SECRET with
    ...
    instanceId: resolveValadrienOsInstanceId(),
    disableLegacyFallback: parseBooleanEnv(process.env.VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK),
  };
}
 ...
 * have expired (set VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true). Normal
...
export function verifyLocalAgentJwt(token: string): LocalAgentJwtClaims | null {
  ...
  //
  // Operators should set `VALADRIEN_OS_AGENT_JWT_DISABLE_LEGACY_FALLBACK=true`
  // approximately one JWT TTL (~1h by default, see VALADRIEN_OS_AGENT_JWT_TTL_SECONDS)
