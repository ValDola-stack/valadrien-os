...
const VALID_HEADERS = {
  "x-valadrien-os-cloud-tenant-token": "test-server-token",
  "x-valadrien-os-cloud-user-id": "user-123",
  "x-valadrien-os-cloud-user-email": "Owner@Example.com",
  "x-valadrien-os-cloud-stack-id": "stack-abc",
  "x-valadrien-os-cloud-stack-role": "owner",
};

describe("resolveCloudTenantActor (shared-pool hardening)", () => {
  beforeEach(() => {
    process.env.VALADRIEN_OS_CLOUD_TENANT_SERVER_TOKEN = "test-server-token";
  });
  afterEach(() => {
    delete process.env.VALADRIEN_OS_CLOUD_TENANT_SERVER_TOKEN;
  });
  ...
  it("maps a non-owner stack role through to the membership without elevating", async () => {
    ...
    const actor = await resolveCloudTenantActor(
      db,
      fakeReq({ ...VALID_HEADERS, "x-valadrien-os-cloud-stack-role": "member" }),
    );
    expect(actor!.isInstanceAdmin).toBe(false);
