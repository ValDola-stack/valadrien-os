...
import type { ValadrienOsConfig } from "@valadrien-os/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by ValadrienOs worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.VALADRIEN_OS_IN_WORKTREE !== "true") return null;
...
function writeConfigFile(configPath: string, config: ValadrienOsConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".valadrien-os", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as ValadrienOsConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: ValadrienOsConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: ValadrienOsConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.VALADRIEN_OS_HOME = context.homeDir;
  process.env.VALADRIEN_OS_INSTANCE_ID = context.instanceId;
  process.env.VALADRIEN_OS_CONFIG = context.configPath;
  process.env.VALADRIEN_OS_CONTEXT = context.contextPath;
  process.env.VALADRIEN_OS_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ValadrienOsConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    VALADRIEN_OS_HOME: context.homeDir,
    VALADRIEN_OS_INSTANCE_ID: context.instanceId,
    VALADRIEN_OS_CONFIG: context.configPath,
    VALADRIEN_OS_CONTEXT: context.contextPath,
    VALADRIEN_OS_IN_WORKTREE: "true",
    VALADRIEN_OS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: ValadrienOsConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as ValadrienOsConfig;
  } catch {
    return;
  }
