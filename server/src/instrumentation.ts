...
export function shutdownInstrumentation(): Promise<void> {
  shutdownPromise ??= (async () => {
    ...
    try {
      ...
      console.error("[valadrien-os] OpenTelemetry shutdown failed", err);
    }
  })();
...
export function resolveProtocol(): {
  ...
  switch (raw) {
    ...
    default:
      // eslint-disable-next-line no-console
      console.warn(
        `[valadrien-os] Unknown OTEL_EXPORTER_OTLP_PROTOCOL=${raw}; falling back to grpc. ` +
  ...
  try {
    ...
    const sdk = new NodeSDK({
      resource: resourceFromAttributes({
        [ATTR_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || "valadrien-os",
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        "[valadrien-os] OpenTelemetry SDK failed to start; continuing without tracing",
        err,
      );
      return;
    }
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[valadrien-os] OTEL_EXPORTER_OTLP_ENDPOINT is set but the @opentelemetry/* " +
