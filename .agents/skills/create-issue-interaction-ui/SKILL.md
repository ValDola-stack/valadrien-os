...
description: >
  Add a new ValadrienOs issue-thread interaction kind end-to-end. Use when repo
...
Developer/maintainer skill. Do NOT install this on production ValadrienOs agents.

This skill walks a ValadrienOs contributor through introducing a new issue-thread
...
the operational agents that run inside a deployed ValadrienOs company.

Do NOT install this on production ValadrienOs agents. This guide is for repository contributors changing ValadrienOs itself.
...
- Changing how ValadrienOs agents *call* interactions. Update `skills/valadrien-os`
...
| Helpers/Docs | CLI command, MCP tool, plugin SDK type+host+testing path, `skills/valadrien-os` guidance.            |
...
your own plan document if you are running this work through ValadrienOs itself.
...
- `skills/valadrien-os/SKILL.md` — add a row to the interaction-kinds table:
  *when to use*, *when not to use*, plus a copyable payload example.
- `skills/valadrien-os/references/api-reference.md` — full payload and result
...
- [ ] CLI, MCP, and plugin SDK helpers all accept the new payload shape and
      have test coverage.
- [ ] `skills/valadrien-os/SKILL.md` and `references/api-reference.md` updated.
