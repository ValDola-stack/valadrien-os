...
The opening line of the changelog must be an H1 of the format `# ValadrienOs {version}`
(no braces), e.g. `# ValadrienOs v2026.618.0`. Always include the `ValadrienOs ` prefix and
the `v` on the version.

Template:

```markdown
# ValadrienOs vYYYY.MDD.P
...
case when the run has ValadrienOs API context. Use `skills/valadrien-os/references/cases.md`
...
{
  "caseType": "release",
  "key": "valadrien-os-release:vYYYY.MDD.P",
  "title": "ValadrienOs vYYYY.MDD.P release",
  "summary": "Stable ValadrienOs release notes for vYYYY.MDD.P.",
  "status": "in_progress",
  "fields": {
    "schema_version": 1,
    "version": "vYYYY.MDD.P",
...
{
  "title": "ValadrienOs vYYYY.MDD.P changelog",
  "format": "markdown",
...
1. confirm the H1 heading is `# ValadrienOs {version}` (e.g. `# ValadrienOs v2026.618.0`) with the stable version only
