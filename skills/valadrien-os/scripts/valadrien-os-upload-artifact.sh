...
usage() {
  cat <<'EOF'
Usage:
  valadrien-os-upload-artifact.sh FILE [options]

Uploads a generated file from the current workspace to the current ValadrienOs
issue, then creates an attachment-backed artifact work product by default.

Required environment for live uploads:
  VALADRIEN_OS_API_URL, VALADRIEN_OS_API_KEY, VALADRIEN_OS_COMPANY_ID, VALADRIEN_OS_TASK_ID, VALADRIEN_OS_RUN_ID

Options:
  --issue-id ID          Issue id to attach to (default: VALADRIEN_OS_TASK_ID)
  --company-id ID        Company id (default: VALADRIEN_OS_COMPANY_ID)
...
Examples:
  scripts/valadrien-os-upload-artifact.sh dist/demo.mp4 \
    --title "Demo video render" \
    --summary "MP4 render for board review"

  scripts/valadrien-os-upload-artifact.sh out/walkthrough.webm \
...
company_id="${VALADRIEN_OS_COMPANY_ID:-}"
title=""
summary=""
content_type=""
...
if [[ -z "$issue_id" || -z "$company_id" ]]; then
  printf 'Missing issue or company id. Pass --issue-id/--company-id or set VALADRIEN_OS_TASK_ID/VALADRIEN_OS_COMPANY_ID.\n' >&2
  exit 1
fi
