...
- App-shipped catalog: a curated set of company skills in `@valadrien-os/skills-catalog`, browseable and installable without leaving ValadrienOs.
...
```sh
curl -sS "$VALADRIEN_OS_API_URL/api/skills/catalog?kind=bundled" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"

curl -sS "$VALADRIEN_OS_API_URL/api/skills/catalog/ref?ref=github-pr-workflow" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY"

curl -sS -X POST "$VALADRIEN_OS_API_URL/api/companies/$VALADRIEN_OS_COMPANY_ID/skills/install-catalog" \
  -H "Authorization: Bearer $VALADRIEN_OS_API_KEY" \
