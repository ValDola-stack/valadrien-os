...
When the executor finishes work, move the source issue to `in_review`. ValadrienOs advances the issue to the active stage participant through `executionState.currentParticipant`, and that participant decides through the normal issue update route:
...
Agent heartbeat implementations should follow the ValadrienOs skill's **Execution-policy review/approval wakes** procedure when they are assigned as the active gate participant.
...
POST /api/companies/{companyId}/projects
{
  ...
  "goalIds": ["{goalId}"],
  "workspace": {
    "name": "valadrien-os-mobile",
    "cwd": "/Users/me/valadrien-os-mobile",
    "repoUrl": "https://github.com/acme/valadrien-os-mobile",
    "repoRef": "main",
    "isPrimary": true
  }
}
...
Use `valadrien-os-create-agent` for the full hiring workflow (reflection + config comparison + prompt drafting).
