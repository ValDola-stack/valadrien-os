...
function makeFixture() {
  const fixtureDir = mkdtempSync(join(tmpdir(), "valadrien-os-release-registry-"));
...
test("fetch prints a JSON version map and treats missing packages as empty", () => {
  ...
  const result = runScript(["fetch", "@valadrien-os/present", "@valadrien-os/missing"], fixture);

  assert.equal(result.status, 0);
  const map = JSON.parse(result.stdout);
  assert.deepEqual(map["@valadrien-os/present"], [
    "1.0.0",
    "2026.707.0",
    "2026.707.1",
    "2026.707.1-canary.4",
  ]);
  assert.deepEqual(map["@valadrien-os/missing"], []);
...
test("assert-absent succeeds when no package has the version", () => {
  ...
  const result = runScript(
    ["assert-absent", "2026.707.2", "@valadrien-os/absent", "@valadrien-os/absent"],
    fixture,
  );
...
test("assert-absent fails and names packages that already have the version", () => {
  const fixture = makeFixture();
  const result = runScript(
    ["assert-absent", "2026.707.2", "@valadrien-os/present", "@valadrien-os/absent"],
    fixture,
  );
...
test("invalid concurrency fails instead of skipping registry checks", () => {
  const fixture = makeFixture();
  const result = runScript(["assert-absent", "2026.707.2", "@valadrien-os/present"], fixture, {
...
test("next_stable_version reads RELEASE_PACKAGE_VERSIONS_FILE without calling npm", () => {
  ...
  writeFileSync(
    versionsFile,
    JSON.stringify({
      "@valadrien-os/a": ["2026.707.0", "2026.707.1", "2026.707.1-canary.4"],
      "@valadrien-os/b": [],
    }),
  );

  const result = runReleaseLibHelper(
    'next_stable_version 2026-07-07 "@valadrien-os/a" "@valadrien-os/b"',
    fixture,
...
test("next_canary_version reads RELEASE_PACKAGE_VERSIONS_FILE without calling npm", () => {
  ...
  const result = runReleaseLibHelper('next_canary_version 2026.707.1 "@valadrien-os/a"', fixture, {
    RELEASE_PACKAGE_VERSIONS_FILE: versionsFile,
  });
...
test("next_stable_version falls back to npm view without a versions file", () => {
  const fixture = makeFixture();
  const result = runReleaseLibHelper('next_stable_version 2026-07-07 "@valadrien-os/present"', fixture);
