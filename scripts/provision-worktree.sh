...
const envConfigPath = expandHomePrefix(env.VALADRIEN_OS_CONFIG);
if (envConfigPath && path.resolve(envConfigPath) !== configPath) {
  fail(`existing worktree env points at ${envConfigPath}, not ${configPath}`);
}

const homeDir = expandHomePrefix(env.VALADRIEN_OS_HOME);
const instanceId = env.VALADRIEN_OS_INSTANCE_ID;
if (!homeDir || !instanceId) {
  fail("existing worktree env is missing VALADRIEN_OS_HOME or VALADRIEN_OS_INSTANCE_ID");
}
...
if [[ -e "$worktree_config_path" && -e "$worktree_env_path" ]] && existing_worktree_config_is_usable; then
  echo "Reusing existing isolated ValadrienOs worktree config at $worktree_config_path" >&2
else
  if [[ -e "$worktree_config_path" || -e "$worktree_env_path" ]]; then
    echo "Existing isolated ValadrienOs worktree config is stale for this host; regenerating." >&2
  fi
...
const ignoredDirs = new Set([".git", ".valadrien-os", "node_modules", "dist", "storybook-static"]);
const files = [];
...
if [[ -f "$worktree_cwd/package.json" && -f "$worktree_cwd/pnpm-lock.yaml" ]]; then
  needs_install=0
  install_fingerprint_path="$valadrien_os_dir/pnpm-install-fingerprint"
