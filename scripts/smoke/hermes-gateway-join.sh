...
Creates a Hermes gateway agent from an agent-only ValadrienOs invite, approves the
join request, claims the one-time ValadrienOs API key, and verifies the stored
...
Notes:
  HERMES_GATEWAY_API_BASE_URL is stored on the ValadrienOs adapter and must be
  reachable by the ValadrienOs server. HERMES_GATEWAY_PROBE_URL is only used by
  this operator shell to preflight /health, which is useful when ValadrienOs talks
  ...
  claimed ValadrienOs agent API key and is written chmod 600.
...
redact_text() {
  local text="$1"
  local secret
  for secret in "${HERMES_GATEWAY_API_KEY:-}" "${CLAIM_SECRET:-}" "${AGENT_API_KEY:-}" "${VALADRIEN_OS_AUTH_HEADER:-}" "${VALADRIEN_OS_COOKIE:-}" "${VALADRIEN_OS_API_KEY:-}"; do
    if [[ -n "$secret" ]]; then
...
probe_hermes_gateway() {
  ...
  [[ -n "$HERMES_GATEWAY_API_KEY" ]] || fail "HERMES_GATEWAY_API_KEY or API_SERVER_KEY is required before any ValadrienOs state is mutated"
  ...
  if [[ "$code" != "200" ]]; then
    fail "Hermes gateway health probe failed before mutating ValadrienOs state: ${health_url} returned HTTP ${code}. Start Hermes with API_SERVER_ENABLED=true API_SERVER_KEY=<key> hermes gateway run --replace --accept-hooks, or set HERMES_GATEWAY_API_BASE_URL/HERMES_GATEWAY_API_KEY."
  fi
}

log "checking ValadrienOs health"
...
JOIN_PAYLOAD="$(jq -nc \
  ...
  --arg valadrienOsApiUrl "$VALADRIEN_OS_API_URL_FOR_HERMES" \
  ...
  | if $valadrienOsApiUrl != "" then .agentDefaultsPayload.valadrienOsApiUrl = $valadrienOsApiUrl else . end
...
if [[ -n "$VALADRIEN_OS_API_URL_FOR_HERMES" ]]; then
  STORED_VALADRIEN_OS_API_URL="$(jq -r '.adapterConfig.valadrienOsApiUrl // empty' <<<"$RESPONSE_BODY")"
  [[ "$STORED_VALADRIEN_OS_API_URL" == "$VALADRIEN_OS_API_URL_FOR_HERMES" || "$(strip_trailing_slash "$STORED_VALADRIEN_OS_API_URL")" == "$(strip_trailing_slash "$VALADRIEN_OS_API_URL_FOR_HERMES")" ]] \
    || fail "stored valadrienOsApiUrl mismatch"
fi
