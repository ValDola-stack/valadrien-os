...
VALADRIEN_OS_API_URL="${VALADRIEN_OS_API_URL:-http://127.0.0.1:3100}"
...
HERMES_IMAGE="${HERMES_IMAGE:-valadrien-os-hermes-gateway-smoke:local}"
...
HERMES_CONTAINER_NAME="${HERMES_CONTAINER_NAME:-valadrien-os-hermes-gateway-smoke-${RUN_SUFFIX}}"
...
HERMES_SMOKE_STATE_DIR="${HERMES_SMOKE_STATE_DIR:-${TMPDIR:-/tmp}/valadrien-os-hermes-gateway-smoke-${RUN_SUFFIX}}"
HERMES_SMOKE_DIAG_DIR="${HERMES_SMOKE_DIAG_DIR:-${TMPDIR:-/tmp}/valadrien-os-hermes-gateway-e2e-diag-${RUN_SUFFIX}}"
...
VALADRIEN_OS_API_URL_FOR_HERMES="${VALADRIEN_OS_API_URL_FOR_HERMES:-http://host.docker.internal:3100}"
...
joins it to ValadrienOs as a hermes_gateway agent, wakes that agent on a smoke
issue, verifies the issue result, captures redacted diagnostics, and cleans up
ValadrienOs and Docker state unless HERMES_SMOKE_KEEP=1.

Required:
  VALADRIEN_OS_API_URL=http://127.0.0.1:3100
  VALADRIEN_OS_AUTH_HEADER='Bearer <board-token>'     # or VALADRIEN_OS_COOKIE

Common flags:
  COMPANY_ID=<uuid> or COMPANY_SELECTOR=<prefix|name|uuid>
  HERMES_VERSION=0.17.0
  HERMES_IMAGE=valadrien-os-hermes-gateway-smoke:local
  HERMES_GATEWAY_PORT=8642
  HERMES_GATEWAY_API_BASE_URL=http://127.0.0.1:8642
  HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642
  VALADRIEN_OS_API_URL_FOR_HERMES=http://host.docker.internal:3100
...
Mode notes:
  HERMES_GATEWAY_API_BASE_URL is the URL stored on the ValadrienOs adapter and
  must be reachable by the ValadrienOs server. HERMES_GATEWAY_PROBE_URL is the URL
  this operator shell uses for direct gateway checks. They can differ for Docker
  network and reverse-proxy smoke runs.

  Raw Hermes and ValadrienOs API keys are redacted from logs and diagnostic files.
...
elif [[ -n "${VALADRIEN_OS_API_KEY:-}" ]]; then
  AUTH_HEADERS+=(-H "Authorization: Bearer ${VALADRIEN_OS_API_KEY}")
fi
if [[ -n "${VALADRIEN_OS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=(-H "Cookie: ${VALADRIEN_OS_COOKIE}")
  VALADRIEN_OS_BROWSER_ORIGIN="${VALADRIEN_OS_BROWSER_ORIGIN:-${VALADRIEN_OS_API_URL%/}}"
  AUTH_HEADERS+=(-H "Origin: ${VALADRIEN_OS_BROWSER_ORIGIN}" -H "Referer: ${VALADRIEN_OS_BROWSER_ORIGIN}/")
fi
...
redact_text() {
  ...
  for secret in \
    ...
    "${VALADRIEN_OS_AUTH_HEADER:-}" \
    "${VALADRIEN_OS_COOKIE:-}"; do
    if [[ -n "$secret" ]]; then
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board/operator auth required. Set VALADRIEN_OS_COOKIE, VALADRIEN_OS_AUTH_HEADER, or a board-capable VALADRIEN_OS_API_KEY."
  fi
...
capture_diagnostics() {
  mkdir -p "$HERMES_SMOKE_DIAG_DIR"
  {
    ...
    echo "valadrienOsApiUrl=${VALADRIEN_OS_API_URL}"
    echo "valadrienOsApiUrlForHermes=${VALADRIEN_OS_API_URL_FOR_HERMES}"
  ...
  capture_container_logs
  capture_issue_diagnostics "$SMOKE_ISSUE_ID" "valadrien-os-smoke"
  capture_run_diagnostics "$RUN_ID" "valadrien-os-smoke"
}

cleanup_valadrien_os_state() {
...
on_exit() {
  ...
  cleanup_valadrien_os_state || true
...
start_container() {
  ...
  local args=(
    ...
    -e VALADRIEN_OS_API_URL="$VALADRIEN_OS_API_URL_FOR_HERMES"
    -e NO_COLOR=1
...
assert_fresh_container_state() {
  ...
  docker exec "$HERMES_CONTAINER_NAME" sh -lc 'env | sort | grep -E "^(HOME|HERMES_HOME|XDG_|API_SERVER_|VALADRIEN_OS_API_URL)=" || true' > "${HERMES_SMOKE_DIAG_DIR}/container-env.txt"
  ...
  if docker exec "$HERMES_CONTAINER_NAME" sh -lc 'env | grep -q "^VALADRIEN_OS_API_KEY="'; then
    fail "container unexpectedly has VALADRIEN_OS_API_KEY before join/key claim"
  fi
}

probe_container_to_valadrien-os() {
  log "probing container-to-ValadrienOs connectivity at ${VALADRIEN_OS_API_URL_FOR_HERMES}/api/health"
  if ! docker exec "$HERMES_CONTAINER_NAME" curl -fsS --max-time 8 "${VALADRIEN_OS_API_URL_FOR_HERMES%/}/api/health" > "${HERMES_SMOKE_DIAG_DIR}/container-valadrien-os-health.json"; then
    fail "Hermes container cannot reach ValadrienOs. Set VALADRIEN_OS_API_URL_FOR_HERMES to a URL reachable from inside Docker, or keep HERMES_DOCKER_ADD_HOST=1 for Linux host.docker.internal."
  fi
}
...
probe_gateway_readiness() {
  ...
  if [[ "$HERMES_GATEWAY_PROBE_URL" != "$HERMES_GATEWAY_API_BASE_URL" ]]; then
    log "ValadrienOs will store Hermes gateway URL ${HERMES_GATEWAY_API_BASE_URL}"
  fi
...
assert_direct_gateway_run() {
  ...
  payload="$(jq -nc \
    --arg marker "$marker" \
    --arg session "valadrien-os-smoke-direct-${RUN_SUFFIX}" \
    '{input: ("Reply with exactly " + $marker + " and no other text."), instructions: "You are running a ValadrienOs Hermes gateway smoke direct API assertion.", session_id: $session}')"
...
assert_stop_behavior_if_deterministic() {
  ...
  local payload
  payload="$(jq -nc \
    --arg session "valadrien-os-smoke-stop-${RUN_SUFFIX}" \
...
join_hermes_agent() {
  ...
  VALADRIEN_OS_API_URL_FOR_HERMES="$VALADRIEN_OS_API_URL_FOR_HERMES" \
  VALADRIEN_OS_AUTH_HEADER="${VALADRIEN_OS_AUTH_HEADER:-}" \
  VALADRIEN_OS_API_KEY="${VALADRIEN_OS_API_KEY:-}" \
  VALADRIEN_OS_COOKIE="${VALADRIEN_OS_COOKIE:-}" \
...
install_claimed_key_in_container() {
  log "placing newly claimed ValadrienOs key in container workspace"
  local key_file="${HERMES_SMOKE_STATE_DIR}/workspace/valadrien-os-claimed-api-key.json"
  ...
  chmod 644 "$key_file"
  docker exec "$HERMES_CONTAINER_NAME" sh -lc 'test -f /home/hermes/workspace/valadrien-os-claimed-api-key.json && test ! -e "$HERMES_HOME/host-sentinel.txt"'
}

patch_agent_instructions_with_claimed_key() {
  log "patching Hermes agent instructions with claimed ValadrienOs API context"
  api_request "GET" "/agents/${AGENT_ID}"
  assert_status "200"

  local instructions patch_payload
  instructions="For this smoke run only, call ValadrienOs at ${VALADRIEN_OS_API_URL_FOR_HERMES}. Read /home/hermes/workspace/valadrien-os-claimed-api-key.json and use its token as VALADRIEN_OS_API_KEY for ValadrienOs API requests. Do not reveal this key. When mutating ValadrienOs, include X-ValadrienOs-Run-Id with the current ValadrienOs run id when available."
...
create_smoke_issue() {
  local marker="HERMES_VALADRIEN_OS_E2E_OK_${RUN_SUFFIX}"
  local title="[Hermes Gateway Smoke] ${RUN_SUFFIX}"
  local description
  description="Hermes gateway full Docker e2e smoke.\n\n1. Read this issue.\n2. Post a ValadrienOs issue comment containing exactly: ${marker}\n3. Mark this issue done.\n\nUse the ValadrienOs API URL and key provided in your run instructions. Do not reveal secrets."
  ...
  echo "$marker" > "${HERMES_SMOKE_DIAG_DIR}/valadrien-os-marker.txt"
}
...
trigger_wakeup() {
  ...
  log "triggered ValadrienOs run ${RUN_ID}"
}
...
assert_valadrien_os_wake_success() {
  local marker
  marker="$(cat "${HERMES_SMOKE_DIAG_DIR}/valadrien-os-marker.txt")"

  trigger_wakeup
  local run_status issue_status marker_found
  run_status="$(wait_for_run_terminal "$RUN_ID" "$RUN_TIMEOUT_SEC")"
  log "ValadrienOs run ${RUN_ID} status=${run_status}"
  ...
  if [[ "$STRICT_CASES" == "1" ]]; then
    [[ "$run_status" == "succeeded" ]] || fail "ValadrienOs Hermes gateway run did not succeed"
...
main() {
  ...
  assert_status "200"
  log "ValadrienOs health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
  require_board_auth
  resolve_company_id
