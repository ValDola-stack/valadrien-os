...
: "${VALADRIEN_OS_API_URL:?Set VALADRIEN_OS_API_URL for the target dev instance.}"
: "${VALADRIEN_OS_API_KEY:?Set VALADRIEN_OS_API_KEY for the target dev instance.}"
: "${VALADRIEN_OS_COMPANY_ID:?Set VALADRIEN_OS_COMPANY_ID for the target dev company.}"
...
pc_json() {
  "${PC_CMD[@]}" "$@" --json -C "$VALADRIEN_OS_COMPANY_ID"
}
