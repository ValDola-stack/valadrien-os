#!/usr/bin/env bash
#
# Kill all local ValadrienOs workspace runtime service processes.
#
# This targets managed workspace services such as preview/dev commands started
# from project or execution workspaces. Use scripts/kill-dev.sh for ValadrienOs
# server processes.
#
# Usage:
...
valadrien_os_home="$(expand_home "${VALADRIEN_OS_HOME:-$HOME/.valadrien-os}")"
valadrien_os_instance_id="${VALADRIEN_OS_INSTANCE_ID:-default}"
append_runtime_dir "$valadrien_os_home/instances/$valadrien_os_instance_id/runtime-services"

if [[ "${VALADRIEN_OS_KILL_WORKSPACES_ONLY_CURRENT:-}" != "1" ]]; then
  for dir in \
    "$HOME"/.valadrien-os/instances/*/runtime-services \
    "$HOME"/.valadrien-os-worktrees/instances/*/runtime-services \
    "$REPO_ROOT"/.valadrien-os/instances/*/runtime-services \
    "$REPO_ROOT"/.valadrien-os/runtime-services/instances/*/runtime-services
  do
    append_runtime_dir "$dir"
  done

  for sibling_root in "$REPO_PARENT"/valadrien-os*; do
    [[ -d "$sibling_root" ]] || continue
    for dir in \
      "$sibling_root"/.valadrien-os/instances/*/runtime-services \
      "$sibling_root"/.valadrien-os/runtime-services/instances/*/runtime-services
    do
      append_runtime_dir "$dir"
    done
  done
...
if [[ ${#active_files[@]} -eq 0 && ${#stale_files[@]} -eq 0 ]]; then
  echo "No ValadrienOs workspace runtime services found."
  exit 0
fi

if [[ ${#active_files[@]} -gt 0 ]]; then
  echo "Found ${#active_files[@]} ValadrienOs workspace runtime service record(s):"
  echo ""
  printf '%s\n' "${display_lines[@]}"
  echo ""
fi
