#!/usr/bin/env bash
# Fork: release scripts are disabled pending a publishing decision for the
# @valadrien-os scope. The upstream release flow is kept below for when
# publishing is re-enabled.
echo "release scripts disabled pending publishing decision for @valadrien-os scope" >&2
exit 1
...
export VALADRIEN_OS_RELEASE_REUSE_UI_DIST=1
